...
这说明系统正在从“`users.role` + `employees.dept_id` 驱动”迁移到“显式绑定表驱动”的权限模型，但收敛还没有完成。
...
- 旧 primary binding 会被置为 inactive。
...
2. 再取账号/员工的显式绑定角色
   ...
   - `employee_roles`
3. 再取任职带来的绑定角色
   - `employee_assignments -> position_roles`
   - `employee_assignments -> org_default_roles`
4. 如果上面都没有，再走兼容回退
   - `employees.dept_id -> org_<deptId>` 角色族
   - 同时兼容 `org_<deptId>|xxx` 这类扩展角色
5. 最后再把 `users.role` 作为 legacy fallback 参与合并

也就是说，普通员工的权限来源已经不是单一字段，而是“绑定表优先，部门回退兜底，`users.role` 最后兼容”。
...
- 普通员工权限主要来自绑定关系与组织任职。
- `users.role` 只保留给管理员账号、服务账号或兼容场景。
...
- 默认授权：`org_default_roles` / `position_roles`
