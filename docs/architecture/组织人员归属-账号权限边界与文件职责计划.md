...
3. `users.permission_preset_id` 在 UI 中只允许表达为“权限预设”，不要叫人员角色、职位或岗位。
...
- 用户管理中的 `users.permission_preset_id` 展示文案已只表达为“权限预设”，不再叫“角色绑定”或人员角色；
- `server/services/user_role_assignment_service.go` 已收敛为 `server/services/account_permission_preset_service.go`，避免文件名误导为人员职位或岗位授权；
