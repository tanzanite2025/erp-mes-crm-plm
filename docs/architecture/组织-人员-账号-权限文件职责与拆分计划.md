# 组织 / 人员 / 账号 / 权限文件职责与拆分计划

> 状态：当前权威分析。
> 日期：2026-07-25
> 范围：只整理真实代码链路、文件职责和后续拆分承接位置；本文件不代表已完成代码改造。

## 1. 当前结论

组织、人员、账号、权限现在不是一条完整统一链，而是三条已经相互接触但尚未完全收敛的链：

1. 组织人员链：`organizations` + `employees.dept_id` 仍是当前页面和接口的主链。
2. 任职投影链：`org_units` + `employee_assignments` 已建模，但主要作为投影/命令承接，还没有接管组织页面主写入口。
3. 账号权限链：`users.role` + `roles.permissions` + `user_permissions` 是当前运行时鉴权真相源。

因此后续不要继续往 `OrganizationService` 或某个页面里补条件分支。应先把职责拆清楚，再决定是否把组织主链切到 `org_units`。

```mermaid
flowchart LR
    A["organizations"] --> B["employees.dept_id"]
    B --> C["users.employee_id"]
    C --> D["IdentityAccessService"]
    E["users.role"] --> F["roles.permissions"]
    G["user_permissions"] --> D
    F --> D
    D --> H["/auth/login /auth/snapshot middleware"]

    I["org_units"] --> J["employee_assignments"]
    J -. "当前主要是任职投影 / 职位保存" .-> B
```
...
| 环节 | 当前文件 | 当前职责 | 长期判断 |
| --- | --- | --- | --- |
| 登录 | `server/handlers/auth.go` | 校验账号密码、调用 `IdentityAccessService`、返回 token 和权限 | 职责基本正确，但返回 DTO 应继续由统一 snapshot 定义驱动 |
| 请求鉴权 | `server/middleware/auth.go` | 解析 JWT、回库查账号、调用 `IdentityAccessService`、写入 gin context | 正确方向：JWT 不是权限真相源 |
| 权限快照 | `server/dependencies/identity_access.go` | 包装用户身份和权限，返回 `/auth/snapshot` 数据 | 包名不长期；业务访问画像不应放在 `dependencies` |
| 有效权限计算 | `server/dependencies/effective_access.go` | 合并角色权限与账号直接权限 | 核心逻辑正确，但应迁到明确的访问控制 service |
...
这会造成一个隐患：如果 `org_units` 和 `organizations` 的 ID 不完全一致，调组织、人员列表和职位归属会各读各的。
...
不能继续“参数叫 orgUnit，显示读 organization，归属写 deptId”。

### 4.5 访问控制 service 不应放在 `dependencies`
...
- `server/services/access/effective_permission_service.go`
- `server/services/access/access_snapshot_dto.go`

迁移时保持对 Handler/Middleware 的单一入口，不让前端或 Handler 自己拼权限。
...
目标：不再让 `organizations` 与 `org_units` 双轨争夺语义。
...
- `IdentityAccessService` / `EffectiveAccessService` 仍在 `server/dependencies`，后续应迁到更明确的访问控制 service 位置；
- 组织主模型仍未二选一，`organizations` 与 `org_units` 的语义错位仍然存在；
- 前端字段仍未从 `deptId/lineId/processId/positionId` 收敛到最终任职模型。

后续不要先改权限算法。应继续完成 Phase 1 的无行为变化拆分：

1. 移动访问控制 service；
2. 保持接口与行为不变；
3. 补最基本的 `go test ./...`。

拆完以后，再进入组织主模型二选一。否则直接改业务逻辑，会继续把旧链路和新链路缠在一起。
