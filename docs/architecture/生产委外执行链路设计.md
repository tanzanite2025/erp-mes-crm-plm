...
| 扫码入口与路线推进 | 还不能从 USB/扫码 UI 自动解析账号职位并推进下一路线步骤 |
...
```text
process_step_allowed_positions
  processStepId
  positionId
```

`Position` 与产线无关，属于组织人事域。工序只引用职位 ID，不直接绑定员工账号、人员档案或账号。当前基础模型和 L3/工序资源弹窗已落地这条关联，后续扫码执行只需要读取当前账号的有效职位再校验是否命中该工序允许职位集合。
...
对应 API 为 `GET /production/product-barcode-states/:productBarcode` 和 `POST /production/product-barcode-states`。这一步只负责“某个一维码当前在哪个工序”和“状态事件历史”，还没有实现扫码执行动作、路线步骤自动推进和持有位置转移。
...
对应 API 为 `GET /production/operation-executions` 和 `POST /production/operation-executions`。写入时会校验工序存在，并在工序配置了允许职位时校验 `operatorPositionId` 是否在允许集合内；写入成功后同步追加 `product_barcode_state_events`。尚未完成的是账号自动解析当前职位、扫码 UI/USB 入口、路线下一步骤推进和持有位置转移。
...
下一步应先完成一维码状态链，再实现扫码驱动的工序执行、持有位置/转移事件。委外真正开始写代码时，才会有稳定锚点，不会做成漂浮在系统外面的孤岛。
