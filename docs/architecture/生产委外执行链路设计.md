...
> 文档状态：生产路线、条码状态、工序执行、统一扫码命令、路线推进和转移事件基础已落地；委外单位、委外任务与发出/回收/检验尚未实施<br>
> 确认日期：2026-07-24<br>
...
现在仍不能直接上委外 UI。当前已经补齐“扫码入口 + 路线推进 + 持有位置/转移事件基础”，但委外单位、委外任务、发出/回收和回收检验还没有实施；下一步应先做委外基础资料，再做委外执行状态机。
...
下一步可以进入 `OutsourcePartner`、可执行工序范围、委外任务、发出/回收/检验。委外真正开始写代码时必须继续复用这条扫码主链，不能另起一套“公司 + 数量 + 备注”的孤立表单。
