...
> 文档状态：生产路线、条码状态与工序执行基础骨架已落地；扫码推进、转移事实与委外执行尚未实施<br>
> 确认日期：2026-07-24<br>
...
现在不能直接上委外 UI。正确顺序是先把“扫码入口 + 路线推进 + 持有位置/转移事实”补齐，再实现委外单位和委外执行。
...
| 缺口 | 影响 |
| --- | --- |
| 扫码入口与账号权限落点 | 还不能从 USB/扫码 UI 按当前账号的页面/命令/动作权限触发执行 |
| 路线步骤推进 | 工序执行后还不能稳定计算下一路线步骤、返工步骤或保持当前步骤 |
...
工序不能配置按职位授权。职位属于组织人事资料，不参与生产执行授权。是否能扫码、转移、发出、回收或检验，应由权限系统按账号/人员分配到页面、命令或动作上。
...
对应 API 为 `GET /production/operation-executions` 和 `POST /production/operation-executions`。写入时会校验工序存在；是否允许调用该接口、按钮或扫码命令，由账号/人员权限体系在入口侧处理。写入成功后同步追加 `product_barcode_state_events`。尚未完成的是扫码 UI/USB 入口、路线下一步骤推进和持有位置转移。
...
- [ ] 明确扫码命令入口：Web、PDA、USB 扫码都进入同一条执行命令；
- [ ] 扫码入口接入账号/命令/动作权限校验，不从工序读取人员限制；
- [ ] 从一维码扫描入口自动触发工序执行；
- [ ] 实现当前路线步骤解析与推进；
- [ ] 增加扫码确认的持有位置和转移事件基础模型；
- [ ] 工序执行、路线推进、转移事件全部写入审计与消息事件源。
...
这条主链稳定以后，再进入 `OutsourcePartner`、可执行工序范围、委外任务、发出/回收/检验。委外真正开始写代码时才会有稳定锚点，不会做成漂浮在系统外面的孤岛。
