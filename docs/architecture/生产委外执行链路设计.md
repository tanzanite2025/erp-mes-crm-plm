...
> 文档状态：生产路线、条码状态、工序执行、统一扫码命令、路线推进、转移事件基础和委外单位档案已落地；委外可执行工序范围、委外任务与发出/回收/检验尚未实施<br>
> 确认日期：2026-07-25<br>
...
当前已先上线委外管理域中的“委外单位”档案，用于维护生产域外部加工单位主数据。仍不能直接上委外执行 UI：可执行工序范围、委外任务、发出/回收和回收检验还没有实施；下一步应先补委外单位与工序范围映射，再做委外执行状态机。
...
下一步应进入 `OutsourcePartnerProcessScope`，把委外单位和可执行工序范围映射起来；之后再做委外任务、发出/回收/检验。后续委外执行必须继续复用这条扫码主链，不能另起一套“公司 + 数量 + 备注”的孤立表单。
