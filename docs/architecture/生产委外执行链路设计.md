...
> 文档状态：生产路线、条码状态、工序执行、统一扫码命令、路线推进、转移事件基础、委外单位档案和委外任务首期已落地；发出/回收/检验尚未实施<br>
> 确认日期：2026-07-25<br>
...
当前已上线委外管理域中的“委外单位”档案和“委外任务”首期能力。委外任务必须从销售订单或生产计划创建，因此已经支持“销售订单 500 件，本厂做不过来，直接从开始就委外”的基础登记链路。这里要明确：当前只是委外任务骨架，不是完整委外闭环；扫码发出、扫码回收、回收检验和质量判定后的路线跳转还没有实施。委外不维护第二套工序能力主数据，产品当前工序只认一维码状态中的 `routeStepId`。
...
当前委外单位和委外任务只保存各自领域的基础资料与来源快照；产品当前工序统一读取一维码状态中的 `routeStepId`。下一步补发出、回收、检验和质量判定后的路线跳转，且必须继续复用这条扫码主链，不能另起一套“公司 + 数量 + 工序能力映射”的孤立事实源。
