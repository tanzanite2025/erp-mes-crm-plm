...
> 文档状态：架构分析完成，产线管理 TAB 尚未删除，委外功能尚未实施  
> 确认日期：2026-07-23  
> 适用范围：生产架构、产线脑图、生产路线、在制品执行、委外/外协  
> 当前原则：先收敛领域边界和唯一入口，再实施代码迁移；项目尚未正式接入生产，不保留旧路由兼容跳转
...
| `src/routes/_authenticated/production-architecture/line.tsx` | 独立产线管理路由 | 迁移完成后删除 |
| `src/routes/_authenticated/production-architecture/mindmap.tsx` | 产线脑图路由 | 保留，作为唯一产线入口 |
| `src/routes/_authenticated/production-architecture/index.tsx` | 当前根路径跳转到 `line` | 删除旧跳转；根路径改为正式进入 `mindmap` |
...
```mermaid
flowchart LR
  A["LineMgmt 或 LineMindmap"] --> B["useProductionLinesQuery"]
...
flowchart LR
  ...
  B --> C["useLineMgmtLines.handleUpdateLine"]
...
当前 `useLineMgmtLines` 名称虽然带有旧页面语义，但实际上已被脑图使用。迁移时不能删除这个 hook 的业务能力，应该将其重命名为 `use-production-lines`，再统一更新引用。
...
- [ ] 将 `LineDialog` 整理为长期命名的产线资料弹窗；
- [ ] 在脑图 Toolbar 增加“新增产线”；
- [ ] 在当前产线操作菜单增加“编辑资料、启用/停用、删除”；
- [ ] 复用现有版本、拓扑授权、审计和乐观更新链路；
- [ ] 空产线时仍能从脑图直接创建第一条产线；
- [ ] 无权限时保留按钮但置灰，并说明缺少的权限；
- [ ] 增加删除产线的影响提示，明确会级联删除层级结构。

### 阶段 B：共享职责整理

- [ ] `use-line-mgmt-lines.ts` 重命名为 `use-production-lines.ts`；
- [ ] `LineMutationPayload` 移到共享 hooks/contracts；
- [ ] `line-topology-helpers.ts` 移到 `production-shared/topology`；
- [ ] 安全授权弹窗移到生产架构共享组件；
- [ ] 脑图不再从 `tabs/line-mgmt` 深层导入共享能力；
- [ ] 检查所有跨域深层 import，保持 `index.ts` 或显式公共模块边界。

### 阶段 C：删除旧入口

- [ ] 从 `getProductionArchitectureTabs` 删除 `line`；
- [ ] 删除 `src/routes/_authenticated/production-architecture/line.tsx`；
- [ ] 根路径进入 `mindmap`，不保留旧兼容跳转；
- [ ] 从 `search-data.ts` 删除产线管理搜索项；
- [ ] 从 `authenticated-route-catalog.ts` 删除旧路由；
- [ ] 从 `route-permission-labels.ts` 删除旧页签标签；
- [ ] 删除旧 UI 组件和只服务于旧页面的 hook；
- [ ] 更新中文、英文 locale 中的旧“产线管理 TAB”文案；
- [ ] 重新生成路由树、权限清单和搜索索引。

### 阶段 D：验收

- [ ] 新增、编辑、启停、删除产线均可在脑图完成；
- [ ] 拓扑和工序能力编辑不丢失；
- [ ] 版本冲突能阻止覆盖；
- [ ] 二次授权仍生效；
- [ ] 删除产线后相关层级级联清理；
...
- [ ] 脑图补齐产线基础资料 CRUD；
- [ ] 共享 hook、类型、拓扑 helper 重命名和归位；
- [ ] 删除旧 `line` TAB、路由、搜索项和权限目录引用；
- [ ] 更新根路径默认入口；
- [ ] 完成构建、权限、路由和回归测试。
...
> 结论：本轮只完成了结构分析和长期文档。下一步应先实施“脑图补齐产线基础管理 + 删除产线管理 TAB”的迁移，验收通过后再开始委外实体和状态链路设计。
