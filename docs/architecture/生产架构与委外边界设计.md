...
> 文档状态：产线管理 TAB 迁移已完成；生产路线、条码状态、工序执行、统一扫码命令、路线推进、转移事件基础和委外单位档案已落地；委外可执行工序范围、委外任务与发出/回收/检验尚未实施<br>
> 确认日期：2026-07-25<br>
...
当前进度：步骤 1 到 6 已完成并提交，生产路线、条码状态、工序执行基础骨架和委外单位档案已落地；委外专项设计已迁入 `docs/architecture/生产委外执行链路设计.md`，下一步只补委外可执行工序范围。
...
- [x] 委外页面和菜单归属；
- [x] 一个三级域 `委外管理`，当前内部承载 `委外单位` TAB；
- [ ] 继续在同一域内补可执行工序范围、委外任务、收发检验 TAB；
- [x] 权限不足按钮显示但置灰；
- [ ] 移动端布局验收；
- [ ] 不保留临时旧路由和兼容跳转；
...
> 结论：产线唯一入口迁移已完成。系统结构固定为 L1 → L2 → L3；统一扫码命令入口、账号动作权限落点、路线推进和转移事件基础已落地。下一步可以进入委外基础资料，但委外任务和发出/回收状态机必须继续复用这条生产执行主链。
