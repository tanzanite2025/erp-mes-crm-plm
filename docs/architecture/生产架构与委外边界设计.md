...
当前已落地 `production_operation_executions`，用于记录某个产品码在某个工序上的开始、完成、暂停或返工动作。该服务会校验传入的 `operatorPositionId` 是否允许执行对应工序，并同步写入一维码状态事件；但账号到当前职位的自动解析、USB/扫码入口、路线自动推进和转移事实仍在后续阶段。
...
> 结论：产线唯一入口迁移已完成。系统结构固定为 L1 → L2 → L3；下一步不应直接做委外 UI，应先实施账号职位自动解析、扫码入口、路线推进和转移事件，再进入委外状态机。
