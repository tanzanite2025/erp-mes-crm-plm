...
> 文档状态：产线管理 TAB 迁移已完成；生产路线、条码状态、工序执行、统一扫码命令、路线推进和转移事件基础已落地；委外单位、委外任务与发出/回收/检验尚未实施<br>
> 确认日期：2026-07-24<br>
...
当前进度：步骤 1 到 6 已完成并提交，生产路线、条码状态与工序执行基础骨架已落地；委外专项设计已迁入 `docs/architecture/生产委外执行链路设计.md`。
...
- [ ] 委外页面和菜单归属；
- [ ] 一个三级域 `委外管理`，内部 TAB 承载委外单位、能力、任务、收发检验；
- [ ] 权限不足按钮显示但置灰；
- [ ] 移动端布局验收；
- [ ] 不保留临时旧路由和兼容跳转；
