...
当前已经落地 `OutsourceOrder` / `OutsourceOrderLine` 首期骨架。它支持从销售订单、生产计划或手工来源创建委外任务，销售订单和生产计划只作为来源快照；委外任务自己保存委外单位、数量、产品快照、计划发出/返回日期、状态和可选工序锚点。因此“销售订单 500 件，本厂做不过来，直接开始委外”目前已有基础登记入口。
