...
> 文档状态：产线管理 TAB 迁移已完成；生产路线、条码状态与工序执行基础骨架已落地；扫码推进、转移事实与委外执行尚未实施<br>
> 确认日期：2026-07-24<br>
> 适用范围：生产架构、产线脑图、生产路线、在制品执行、委外/外协<br>
> 当前原则：产线领域已收敛到唯一入口；后续委外先补统一扫码入口、路线推进和转移事实，项目尚未正式接入生产，不保留旧路由兼容跳转
...
当前已落地 `production_operation_executions`，用于记录某个产品码在某个工序上的开始、完成、暂停或返工动作。该服务只校验工序、路线、批次等生产事实；是否允许调用扫码、按钮或接口，由账号/人员权限体系在入口侧处理。写入成功后会同步写入一维码状态事件；USB/扫码入口、路线自动推进和转移事实仍在后续阶段。
...
> 结论：产线唯一入口迁移已完成。系统结构固定为 L1 → L2 → L3；下一步不应直接做委外 UI，应先实施统一扫码命令入口、账号权限校验、路线推进和转移事件，再进入委外状态机。
