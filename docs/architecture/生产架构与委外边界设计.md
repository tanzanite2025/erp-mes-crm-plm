...
> 文档状态：产线管理 TAB 迁移已完成；生产路线、条码状态、工序执行、统一扫码命令、路线推进、转移事件基础、委外单位档案和委外任务首期已落地；委外可执行工序范围、发出/回收/检验尚未实施<br>
> 确认日期：2026-07-25<br>
...
当前进度：步骤 1 到 6 已完成并提交，生产路线、条码状态、工序执行基础骨架、委外单位档案和委外任务首期已落地；委外专项设计已迁入 `docs/architecture/生产委外执行链路设计.md`。下一步不再补一个孤立委外表单，而是继续补委外可执行工序范围、扫码发出、扫码回收和回收检验。
...
该入口是 Web、PDA、USB 扫码的统一后端命令层，复用 `action_production_issuance_execute` 动作权限，不从工序、职位或人员档案推导授权。它会从一维码当前状态、请求参数和路线步骤解析当前执行上下文，完成后按路线 `sequence` 推进下一步骤。
...
当前已经落地 `OutsourceOrder` / `OutsourceOrderLine` 首期骨架。它支持从销售订单、生产计划或手工来源创建委外任务，销售订单和生产计划只作为来源快照；委外任务自己保存委外单位、数量、产品快照、计划发出/返回日期、状态和从已有 L3 下拉选择的工序锚点。因此“销售订单 500 件，本厂做不过来，直接开始委外”目前已有基础登记入口。

委外任务行里的 L3 不是重新指定一个新工序，也不是手填文本；它必须选择系统已有且启用的 L3，并保存 `processStepId + processCode + processName` 快照。后续扫码发出/回收会围绕这个 L3 写入转移事件，而不是在委外页反向修改产线脑图或工序库。

但该首期骨架还没有形成完整流转：委外单位可执行工序范围、扫码发出、扫码回收、回收检验、质量结果驱动路线跳转仍在后续阶段。
...
不允许把职位、员工、账号或人员授权写进产线层级，也不允许通过产线节点推断操作人。职位名称、组织归属和启停状态仍由组织人事域负责；生产执行链路只记录操作账号，不记录职位。
...
- [x] 执行批次可选关联落地；
- [x] 工序执行记录落地；
- [x] 账号/命令权限边界确认：生产工序不绑定职位，不通过职位限制扫码或执行。
- [x] 统一扫码命令入口落地；
- [x] 当前路线步骤解析与推进落地；
- [x] 路线推进和持有方转移事件基础表落地；
...
> 结论：产线唯一入口迁移已完成。系统结构固定为 L1 → L2 → L3；统一扫码命令入口、账号动作权限落点、路线推进、转移事件基础、委外单位和委外任务首期已落地。下一步继续补委外可执行工序范围、发出/回收/检验状态机，并且必须复用这条生产执行主链。
