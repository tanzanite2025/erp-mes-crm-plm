...
func registerProductionRoutes(authorized *gin.RouterGroup) {
	...
	outsourceTransferExecute := middleware.RequireAnyPermission(authz.ActionOutsourceTransferExecute)
	outsourceInspectionSubmit := middleware.RequireAnyPermission(authz.ActionOutsourceInspectionSubmit)
	...
	productionGroup.Use(productionAccess)
	{
		...
		productionGroup.GET("/outsourcing/inventory-category-options", handlers.GetProductionOutsourceInventoryCategoryOptionsHandler)
		productionGroup.GET("/outsourcing/transfers", handlers.GetOutsourceTransfersHandler)
		productionGroup.GET("/outsourcing/inspections", handlers.GetOutsourceInspectionsHandler)
		productionGroup.GET("/outsourcing/diagnostics", handlers.GetOutsourceDiagnosticsHandler)
		...
		productionGroup.POST("/outsourcing/orders/:id/cancel", outsourceOrderManage, handlers.CancelOutsourceOrderHandler)
		productionGroup.POST("/outsourcing/order-lines/:lineId/send", outsourceTransferExecute, handlers.SendOutsourceOrderLineHandler)
		productionGroup.POST("/outsourcing/order-lines/:lineId/return", outsourceTransferExecute, handlers.ReturnOutsourceOrderLineHandler)
		productionGroup.POST("/outsourcing/order-lines/:lineId/inspection-task", outsourceInspectionSubmit, handlers.PrepareOutsourceInspectionTaskHandler)
		productionGroup.POST("/outsourcing/order-lines/:lineId/inspect", outsourceInspectionSubmit, handlers.InspectOutsourceOrderLineHandler)
