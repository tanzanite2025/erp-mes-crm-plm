...
func registerProductionRoutes(authorized *gin.RouterGroup) {
	...
	productionGroup.Use(productionAccess)
	{
		...
		productionGroup.POST("/mappings/assign", adminOnly, handlers.AssignProcessToStationHandler)
		productionGroup.POST("/mappings/remove", adminOnly, handlers.RemoveProcessFromStationHandler)
		productionGroup.GET("/mappings", handlers.GetStationMappingsHandler)
