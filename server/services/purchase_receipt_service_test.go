...
func TestRecalculatePurchaseOrderStatusTxUpdatesPersistedOrder(t *testing.T) {
	...
	require.NoError(t, testDB.Exec(`
		INSERT INTO purchase_order_lines (purchase_order_id, line_no, material_id, qty, uom, price, amount, received_qty, status)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, "po-rx-1", 1, "mat-1", 10.0, "PCS", 8.8, 88.0, 3.0, "Open").Error)
