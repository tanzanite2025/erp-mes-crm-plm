...
func TestPreviewMyLeaveRequestHandlerReturnsUnauthorizedWhenUserMissing(t *testing.T) {
	...
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/leaves/preview", strings.NewReader(`{"leaveType":"annual","startTime":"2026-04-09T08:00:00Z","endTime":"2026-04-10T08:00:00Z"}`))
	ctx.Request.Header.Set("Content-Type", "application/json")

	PreviewMyLeaveRequestHandler(ctx)

	require.Equal(t, http.StatusUnauthorized, recorder.Code)
}

func TestCreateMyLeaveRequestHandlerCreatesLeaveForCurrentEmployee(t *testing.T) {
	gin.SetMode(gin.TestMode)
	setupLeaveHandlerSQLiteTestDB(t)
	...
	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/leaves", strings.NewReader(`{"leaveType":"personal","startTime":"2026-04-09T08:00:00Z","endTime":"2026-04-10T08:00:00Z","reason":"家中有事"}`))
	ctx.Request.Header.Set("Content-Type", "application/json")
	ctx.Set("userId", "u-handler")

	CreateMyLeaveRequestHandler(ctx)
