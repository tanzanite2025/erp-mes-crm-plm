...
export class ErrorRecoveryHandler {
  ...
  async withRetry<T>(
    ...
    for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        console.warn(
          `[ErrorRecoveryHandler] ${operationName} failed (attempt ${attempt + 1}/${this.config.maxRetries + 1}):`,
          lastError.message
        );
  ...
  async withGracefulDegradation<T>(
    ...
    try {
      ...
      console.warn(
        `[ErrorRecoveryHandler] ${operationName} failed, falling back to safe mode:`,
    ...
    try {
      ...
      if (stateKey) {
        ...
        if (restoredState !== undefined) {
          console.info(
            `[ErrorRecoveryHandler] Restored state for '${stateKey}' after error`
          );
        }
      }
 ...
 */
export function isProxyTrackerError(error: unknown): error is ProxyTrackerError {
 ...
 */
export function isVirtualScrollerError(error: unknown): error is VirtualScrollerError {
