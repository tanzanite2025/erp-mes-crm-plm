...
export function Search({ className = '', placement = 'header' }: SearchProps) {
    ...
    >
      <SearchIcon
        ...
        className={cn(
          ...
          isDock
            ? 'size-4'
            : 'md:absolute md:start-3 md:top-1/2 md:-translate-y-1/2'
        )}
        size={14}
      />
      {!isDock && (
        <span className='ms-6 hidden truncate tracking-widest uppercase md:inline-block'>
