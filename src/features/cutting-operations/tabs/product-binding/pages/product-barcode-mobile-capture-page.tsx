...
export function ProductBarcodeMobileCapturePage({
  ...
  const submitCapturedCode = async (nextCode: string) => {
    if (!token) {
      setError(
        t(
          'cuttingOperations.productBinding.mobileCapture.page.errors.missingToken'
        )
      )
      return
    }
    if (!nextCode.trim()) {
      setError(
        t(
          'cuttingOperations.productBinding.mobileCapture.page.errors.missingBarcode'
        )
      )
      return
    }
    ...
    try {
      ...
      setError(
        submitError instanceof Error
          ? submitError.message
          : t(
              'cuttingOperations.productBinding.mobileCapture.page.errors.submitFailed'
            )
      )
    } finally {
      setSubmitting(false)
      ...
      <main className='min-h-dvh bg-background px-5 py-8 text-foreground'>
        <section className='mx-auto flex min-h-[70dvh] max-w-md flex-col items-center justify-center text-center'>
          ...
          <h1 className='mt-6 text-3xl font-black tracking-tighter italic'>
            {t(
              'cuttingOperations.productBinding.mobileCapture.page.submitted.title'
            )}
          </h1>
          <p className='mt-3 text-sm leading-6 font-semibold text-muted-foreground'>
            {t(
              'cuttingOperations.productBinding.mobileCapture.page.submitted.description'
            )}
          </p>
        </section>
      </main>
      ...
      <section className='mx-auto max-w-md space-y-4'>
          ...
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='mt-1 text-xs leading-5 font-bold text-muted-foreground'>
                {t(
                  'cuttingOperations.productBinding.mobileCapture.page.description'
                )}
              </p>
            </div>
          </div>
        ...
        <div className='rounded-[28px] border border-dashed border-muted-foreground/20 bg-background p-4 shadow-sm'>
          <TrackingNumberInput
            ...
            placeholder={t(
              'cuttingOperations.productBinding.mobileCapture.page.placeholder'
            )}
        ...
        >
          ...
          {t(
            'cuttingOperations.productBinding.mobileCapture.page.actions.submit'
          )}
        </Button>
      </section>
    </main>
