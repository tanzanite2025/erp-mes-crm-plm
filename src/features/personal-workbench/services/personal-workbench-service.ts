...
export async function getPersonalRecords(): Promise<PersonalRecordListResponse> {
  ...
  const total =
    typeof objectResponse.total === 'number'
...
export async function patchPersonalRecord(
  id: string,
  payload: PersonalRecordUpsertPayload
): Promise<PersonalRecord> {
  const response = await apiFetch<unknown>(
    `/personal-workbench/records/${id}`,
    {
      method: 'PATCH',
      body: JSON.stringify(payload),
    }
...
export async function reorderPersonalRecords(
  payload: PersonalRecordReorderPayloadItem[]
): Promise<void> {
