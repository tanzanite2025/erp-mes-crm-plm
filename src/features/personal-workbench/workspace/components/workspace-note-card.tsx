...
export function WorkspaceNoteCard({ item, onDelete, onEdit }: WorkspaceNoteCardProps) {
  return (
    <Card className='min-h-64 rounded-[28px] border border-amber-200/70 bg-amber-50/70 py-0 shadow-sm'>
      <CardHeader className='px-5 py-5'>
        <div className='flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-amber-700/70'>
          <FileText className='size-4' />
          便签
        </div>
        <CardTitle className='text-base font-black tracking-tight text-foreground'>{item.title || '未命名便签'}</CardTitle>
      </CardHeader>
      <CardContent className='px-5 pb-0'>
        <p className='whitespace-pre-wrap text-sm leading-6 text-foreground/80'>{item.content || '这张便签还没有内容。'}</p>
      </CardContent>
