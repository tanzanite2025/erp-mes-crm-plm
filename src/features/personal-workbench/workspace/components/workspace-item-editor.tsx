...
export function WorkspaceItemEditor({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-w-xl overflow-hidden rounded-[32px] border-none p-0 shadow-2xl'>
        <div className='space-y-6 p-6'>
          <DialogHeader>
            ...
            <DialogDescription className='text-[11px] font-medium text-muted-foreground'>
              {type === 'note'
                ? '把随手复制的文本、备注和个人上下文先贴在这里。'
                : '保存你常用的网站、后台入口和查询页，并补充备注说明。'}
            </DialogDescription>
          </DialogHeader>
            ...
            {type === 'note' ? (
              <div className='space-y-2'>
                ...
                <Textarea
                  ...
                  placeholder='把文本、备注或临时说明贴在这里'
                  className='min-h-56 rounded-2xl bg-amber-50/50 p-4 text-sm leading-6'
                />
              </div>
            ) : (
              <>
                ...
                <div className='space-y-2'>
                  ...
                  <Textarea
                    ...
                    placeholder='写一下这是哪个后台、什么时候用、要注意什么'
                    className='min-h-36 rounded-2xl p-4 text-sm leading-6'
                  />
                </div>
              </>
            )}
