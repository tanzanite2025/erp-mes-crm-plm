...
export function PersonalWorkbenchCardEditor({ open, onOpenChange, record, onSubmit }: PersonalWorkbenchCardEditorProps) {
  ...
  const payload = useMemo<PersonalRecordUpsertPayload>(() => ({
    ...
    assets: coverImageUrl
      ? [{ storagePath: coverImageUrl, mimeType: 'image/webp', width: 0, height: 0, sizeBytes: 0 }]
      : [],
          ...
          <div className='grid gap-4 md:grid-cols-[1.2fr_0.8fr]'>
            <div className='space-y-4'>
              <div className='space-y-2'>
                ...
                <Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder='这条记录想记什么？' />
              </div>
              <div className='space-y-2'>
                <span className='text-[10px] font-black uppercase tracking-widest text-muted-foreground/60'>备注</span>
                <Textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder='写一点背景、想法或后续提醒' className='min-h-36' />
              </div>
              <div className='space-y-2'>
                <span className='text-[10px] font-black uppercase tracking-widest text-muted-foreground/60'>所在分栏</span>
                <Select value={columnKey} onValueChange={(value) => setColumnKey(value as typeof columnKey)}>
                  <SelectTrigger>
