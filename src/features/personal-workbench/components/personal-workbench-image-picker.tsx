...
export function PersonalWorkbenchImagePicker({
  ...
  const activeDraft = useMemo(
    () => getDraftById(activeDraftId),
    [activeDraftId, getDraftById]
  )
  ...
  const discardActiveDraft = useCallback(async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : '丢弃本地草稿失败'
      toast.error(message)
    }
  ...
  const saveLocalDraft = useCallback(
    async ({
      ...
      if (draft) {
        toast.success(
          kind === 'video' ? '视频已保存到本地草稿' : '图片已保存到本地草稿'
        )
  ...
  useEffect(() => {
    ...
    void (async () => {
      const draft = await saveLocalDraft({
        durationSeconds: 10,
  ...
  const handleCapturePhoto = async () => {
    ...
    const file = new File([blob], `personal-record-${Date.now()}.jpg`, {
      type: 'image/jpeg',
    })
    const draft = await saveLocalDraft({ file, kind: 'image' })
    if (draft) {
  ...
  const handleStartRecording = useCallback(async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : '无法开始录制视频'
      toast.error(message)
    }
  ...
  useEffect(() => {
    ...
    void startCamera(
      initialCaptureMode === 'video' ? 'environment' : facingMode
    )
  }, [
    autoStartCamera,
    cameraMode,
  ...
  return (
    <div className='space-y-2'>
      <div className='flex items-center justify-between gap-2'>
        ...
        {supportsCamera ? (
          <Badge variant='outline'>
            {supportsVideoRecording ? '媒体增强' : '相机增强'}
          </Badge>
        ) : (
          <Badge variant='outline'>普通上传</Badge>
        )}
      </div>
      ...
      {activeDraft ? (
        <div className='space-y-3 rounded-2xl border border-dashed border-primary/20 bg-background/80 p-3'>
          <div className='flex items-center justify-between gap-3'>
            <div className='flex items-center gap-2 text-[11px] font-black tracking-widest text-primary uppercase'>
          ...
          <div className='flex flex-wrap items-center justify-between gap-2'>
            ...
            <div className='flex flex-wrap gap-2'>
              ...
              <Button
                type='button'
                className='rounded-full'
                onClick={() => void uploadDraftToServer()}
                disabled={
                  isUploading ||
                  activeDraft.status === 'uploaded' ||
                  activeDraft.status === 'linked_to_record' ||
      ...
      {shouldShowUploadedPreview ? (
        <div className='group relative aspect-video overflow-hidden rounded-2xl border border-dashed border-primary/20 bg-muted/5'>
          <img
          ...
          {cameraMode ? (
            <div
              className={cn(
                ...
                isRecording
                  ? 'border-destructive/70 shadow-[0_0_0_1px_rgba(220,38,38,0.18)]'
                  : 'border-primary/20'
              )}
            >
              <div className='relative aspect-video bg-black'>
                ...
                <div className='absolute top-3 left-3 flex items-center gap-2'>
                  ...
                  {captureMode === 'video' ? (
                    ...
                    <Badge variant='outline'>拍照</Badge>
                  )}
                  {captureMode === 'video' && !isRecording ? (
                    <Badge variant='secondary'>准备录制</Badge>
                  ) : null}
                </div>
              </div>
              <div
                className={cn(
                  'flex flex-wrap items-center justify-between gap-2 border-t bg-background/90 p-3',
                  compactMode && 'gap-3'
                )}
              >
                <div className='flex flex-wrap gap-2'>
                ...
                <div className='flex flex-wrap gap-2'>
                  ...
                  {captureMode === 'photo' && !compactMode ? (
                    <Button
                      ...
                      disabled={isUploading || isStartingCamera || isRecording}
                    >
                      {isUploading ? (
                        <Loader2 className='size-4 animate-spin' />
                      ) : (
              ...
              <div className='border-t bg-background/80 px-3 pb-3'>
                <Tabs
                  value={captureMode}
                  onValueChange={(value) =>
                    setCaptureMode(value as 'photo' | 'video')
                  }
                ...
                >
                  ...
                  >
                    ...
                    <TabsTrigger
                      value='video'
                      className='rounded-full'
                      disabled={!supportsVideoRecording}
                    >
