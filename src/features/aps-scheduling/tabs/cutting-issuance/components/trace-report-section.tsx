...
export function TraceReportSection(props: TraceReportSectionProps) {
  ...
  return (
    <Collapsible
      ...
      className={`${APS_TERTIARY_SECTION_CLASS} p-4`}
    >
      <div
      ...
      >
        <div className='flex items-start gap-3'>
          ...
          <div>
            <span
              className={`${APS_SECTION_MARKER_CLASS} ${APS_TERTIARY_SECTION_MARKER_CLASS}`}
            >
            ...
            {!isOpen ? (
              <p className='mt-2 text-xs font-semibold text-slate-600/90'>
                {t(
                  'apsScheduling.cuttingIssuance.trace.summary.executionCount'
                )}{' '}
                {formatNumber(traceReport?.summary.executionCount, locale)}
                {' / '}
                {t(
                  'apsScheduling.cuttingIssuance.trace.summary.orderCount'
                )}{' '}
                {formatNumber(traceReport?.summary.orderCount, locale)}
                {' / '}
                {t(
                  'apsScheduling.cuttingIssuance.trace.summary.totalLineQuantity'
                )}{' '}
        ...
        <div className='mt-3 grid gap-3 md:grid-cols-3 xl:grid-cols-5'>
          <div className={`${APS_PANEL_CLASS} bg-white/75 p-2.5`}>
            <p className={APS_KICKER_CLASS}>
              {t('apsScheduling.cuttingIssuance.trace.summary.executionCount')}
            </p>
          ...
          <div className={`${APS_PANEL_CLASS} bg-white/75 p-2.5`}>
            <p className={APS_KICKER_CLASS}>
              {t('apsScheduling.cuttingIssuance.trace.summary.orderCount')}
            </p>
            <p className='mt-2 text-2xl font-black tracking-tight text-slate-800'>
              {formatNumber(traceReport?.summary.orderCount, locale)}
            </p>
          ...
          <div className={`${APS_PANEL_CLASS} bg-white/75 p-2.5`}>
            <p className={APS_KICKER_CLASS}>
              {t('apsScheduling.cuttingIssuance.trace.summary.batchCount')}
            </p>
            <p className='mt-2 text-2xl font-black tracking-tight text-slate-800'>
          ...
          <div className={`${APS_PANEL_CLASS} bg-white/75 p-2.5`}>
            <p className={APS_KICKER_CLASS}>
              {t('apsScheduling.cuttingIssuance.trace.summary.timeRange')}
            </p>
            ...
            <table className='w-full min-w-[360px] text-sm'>
              <thead className='bg-muted/30 text-left'>
                <tr>
                  ...
                  <th className='px-4 py-2 text-[10px] font-black tracking-[0.22em] text-muted-foreground/70 uppercase'>
                    {t(
                      'apsScheduling.cuttingIssuance.trace.columns.executionCount'
                    )}
                  </th>
                  <th className='px-4 py-2 text-[10px] font-black tracking-[0.22em] text-muted-foreground/70 uppercase'>
                    {t(
                      'apsScheduling.cuttingIssuance.trace.columns.totalLineQuantity'
                    )}
                  </th>
                </tr>
              </thead>
              <tbody>
                {traceReport?.byStatus.length ? (
                  traceReport.byStatus.map((item) => (
                    <tr
                      key={item.status || UNKNOWN_STATUS_KEY}
                      className='border-t border-border/50'
                    >
                      <td className='px-4 py-2'>
                        {getProductionPlanStatusLabel(item.status, t)}
                      </td>
                      <td className='px-4 py-2'>
                        {formatNumber(item.executionCount, locale)}
                      </td>
                      <td className='px-4 py-2'>
                        {formatNumber(item.totalLineQuantity, locale)}
                      </td>
                    </tr>
                  ))
                ) : (
            ...
            <table className='w-full min-w-[440px] text-sm'>
              <thead className='bg-muted/30 text-left'>
                <tr>
                  <th className='px-4 py-2 text-[10px] font-black tracking-[0.22em] text-muted-foreground/70 uppercase'>
                    {t(
                      'apsScheduling.cuttingIssuance.trace.columns.productModel'
                    )}
                  </th>
                  <th className='px-4 py-2 text-[10px] font-black tracking-[0.22em] text-muted-foreground/70 uppercase'>
