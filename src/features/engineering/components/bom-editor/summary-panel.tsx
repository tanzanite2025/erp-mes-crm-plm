    ...
    <div className='space-y-4 bg-muted/5 p-3 sm:p-4'>
      ...
      <div className='space-y-2 rounded-[24px] border border-dashed border-muted/50 bg-white/50 p-3 sm:p-4'>
        <h5 className='flex items-center gap-3 text-[9px] font-black uppercase tracking-widest text-slate-500 sm:text-[10px]'>
          ...
          <span className='h-[1px] flex-1 bg-muted-foreground/10' />
        </h5>
        <div className='grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-3'>
          {sections.map((section) => {
            const sectionItems = fields.filter((_, i) => form.getValues(`items.${i}.section`) === section)
            const sectionCost = sectionItems.reduce((acc, _, i) => {
              const item = fields.find((_, ri) => form.getValues(`items.${ri}.id`) === sectionItems[i].id)
              if (!item) return acc
            ...
            return (
              ...
              >
                <div className='flex items-center gap-3'>
                  <div className={`size-3 rounded-full border-2 border-white ${sectionItems.length > 0 ? 'bg-blue-500 group-hover/row:bg-white' : 'bg-slate-300'}`} />
                  <span className='text-xs font-black uppercase tracking-tight'>{section}</span>
                </div>
