...
import { Hash, ImageIcon, Palette, Pencil, Plus, Rows3, Trash2, Upload, X } from 'lucide-react'
...
export function ProductAppearanceMgmt() {
  ...
  const dialogShellClasses = buildActionDialogShellClasses({
    ...
    header: 'relative shrink-0 border-b border-dashed border-muted/50 bg-muted/5 px-5 pb-4 pt-5 text-start sm:px-6',
    body: 'custom-scrollbar flex-1 overflow-y-auto px-5 py-4 sm:px-6',
    footer: 'shrink-0 flex-row gap-3 border-t border-dashed border-muted/50 bg-muted/5 p-4 sm:justify-end sm:px-6',
    title: 'flex flex-col gap-0.5 text-lg font-black uppercase tracking-tighter italic',
    description: 'mt-2 text-[10px] font-black uppercase tracking-widest opacity-60',
  })
  ...
  const handleSave = async () => {
    const nextRow: ProductAppearanceDraft = {
      ...
      imageThumbnailUrl: currentRow.imageThumbnailUrl?.trim() || currentRow.imageUrl?.trim() || '',
    ...
    const duplicatedCode = appearances.some(
      (item) => item.barcodeCode === nextRow.barcodeCode && item.id !== nextRow.id
    )

    if (duplicatedCode) {
      toast.error(t('engineering.productAppearance.toasts.duplicateBarcodeCode'))
      return
    }

    try {
      await productAppearanceService.saveProductAppearance(nextRow)
      await queryClient.invalidateQueries({ queryKey: PRODUCT_APPEARANCES_QUERY_KEY })
      setDialogOpen(false)
  ...
  const handleDelete = async (row: ProductAppearance) => {
    if (!window.confirm(t('engineering.productAppearance.actions.deleteConfirm', { name: row.name }))) {
      return
    }
  ...
  const handleActiveChange = async (row: ProductAppearance, active: boolean) => {
    try {
  ...
  return (
    <div className='flex flex-col gap-5 animate-in fade-in duration-700 lg:gap-6'>
        ...
        <div className='flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between'>
          <div className='space-y-1'>
            <div className='flex items-center gap-2.5 text-primary'>
              ...
              <h3 className='text-base font-black uppercase tracking-tighter italic md:text-lg'>
        ...
        <Card className='gap-3 border-none bg-muted/30 py-4 shadow-sm'>
          <CardContent className='flex items-center gap-3 px-4'>
            ...
            <div>
              ...
              <div className='text-xl leading-none font-black'>{appearances.length}</div>
            </div>
          </CardContent>
        </Card>
        <Card className='gap-3 border-none bg-muted/30 py-4 shadow-sm'>
        ...
        <Card className='gap-3 border-none bg-muted/30 py-4 shadow-sm'>
          <CardContent className='flex items-center gap-3 px-4'>
            ...
            <div>
              ...
              <div className='text-xl leading-none font-black'>{activeCount}</div>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {appearances.length === 0 ? (
        <Card className='border-dashed'>
          <CardContent className='flex flex-col items-center justify-center gap-3 py-12 text-center'>
            ...
            <div className='space-y-1'>
              <h4 className='text-base font-black'>{t('engineering.productAppearance.empty.title')}</h4>
              ...
              <CardContent className='space-y-2 px-3.5'>
                <div className='flex items-start justify-between gap-2'>
                  <div className='flex min-w-0 flex-1 gap-2.5'>
                    <div className='flex size-12 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-dashed bg-muted/20'>
                      {row.imageThumbnailUrl ? (
                        <img src={row.imageThumbnailUrl} alt={row.name} className='size-full object-cover' />
                      ) : (
                  ...
                  <div className='min-w-0 flex-1 space-y-1'>
                    <div className='flex items-center gap-1.5'>
                      <CardTitle className='truncate text-sm font-black leading-none'>{row.name}</CardTitle>
                    ...
                    <div className='flex flex-wrap items-center gap-1.5'>
                      <Badge
                        className={row.active ? 'h-5 bg-emerald-500/10 px-1.5 text-[10px] text-emerald-600' : 'h-5 bg-slate-500/10 px-1.5 text-[10px] text-slate-500'}
                      >
                        {row.active
                      ...
                      <Badge variant='secondary' className='h-5 px-1.5 text-[10px]'>
                        {t('engineering.productAppearance.fields.sortOrder')}: {row.sortOrder}
                      </Badge>
                    </div>
                  </div>
                  </div>
                  <div className='flex items-center gap-2 rounded-lg border bg-muted/15 px-2 py-1'>
                    ...
                    <Switch
                      checked={row.active}
                      onCheckedChange={(checked) => void handleActiveChange(row, checked)}
                    />
                  </div>
                </div>
                <div className='grid grid-cols-[auto_1fr] items-start gap-x-2 text-[10px] leading-4 text-muted-foreground'>
                  ...
                  <div className='truncate'>
                    {row.description || t('engineering.productAppearance.empty.descriptionFallback')}
                  </div>
                </div>
                <div className='flex items-center justify-end gap-1.5 border-t pt-2'>
                  <Button variant='outline' size='sm' className='h-7 px-2.5 text-[10px]' onClick={() => openEditDialog(row)}>
                    <Pencil className='mr-1.5 size-3' />
                    {t('engineering.productAppearance.actions.edit')}
                  </Button>
                  <Button variant='outline' size='sm' className='h-7 px-2.5 text-[10px]' onClick={() => void handleDelete(row)}>
      ...
      <ActionDialogShell
        ...
        title={(
          <>
            ...
            <span className='text-[9px] font-mono tracking-widest opacity-40'>PRODUCT_APPEARANCE_MASTER</span>
          </>
        )}
        ...
        footer={(
          <>
            ...
            <Button
              ...
              className='h-11 flex-1 rounded-full px-10 text-[10px] font-black uppercase tracking-widest shadow-xl shadow-primary/20 sm:flex-none'
            >
          ...
          <div className='rounded-[22px] border border-dashed border-primary/20 bg-primary/5 p-4'>
            <div className='mb-4 flex items-center gap-2.5 text-primary'>
              ...
              <div className='space-y-0.5'>
                ...
                <div className='text-[10px] font-mono uppercase tracking-[0.24em] opacity-50'>MASTER_EDIT_FORM</div>
              </div>
            </div>
                  ...
                  {currentRow.imageUrl ? (
                    <div className='space-y-2.5'>
                      <div className='relative aspect-square overflow-hidden rounded-[16px] border bg-muted/10'>
                        <img src={currentRow.imageUrl} alt={currentRow.name || 'appearance-image'} className='size-full object-cover' />
                      </div>
                      <div className='space-y-2'>
                        <div className='truncate text-[11px] font-black text-foreground'>
                          {currentRow.imageName || t('engineering.productAppearance.fields.image')}
                        </div>
                        <div className='grid grid-cols-2 gap-2'>
                          ...
                          >
                            ...
                            {t('engineering.productAppearance.actions.replaceImage')}
                          </Button>
                          <Button
                            type='button'
                            variant='ghost'
                          ...
                          >
                            ...
                            {t('engineering.productAppearance.actions.removeImage')}
                          </Button>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div className='space-y-1'>
                        <div className='text-[11px] font-black uppercase tracking-widest text-foreground'>
                          {t('engineering.productAppearance.actions.uploadImage')}
                        </div>
              ...
              <div className='grid content-start gap-4'>
                <div className='space-y-2'>
                  ...
                  <Input
                    ...
                    onChange={(event) => setCurrentRow((prev) => ({ ...prev, name: event.target.value }))}
                    placeholder={t('engineering.productAppearance.placeholders.name')}
                  />
                </div>
                <div className='space-y-2'>
                  <Label className='text-[10px] font-black uppercase tracking-widest text-primary/80'>
                  ...
                  <Input
                    ...
                    placeholder={t('engineering.productAppearance.placeholders.barcodeCode')}
                  />
                </div>
              </div>
            </div>
            ...
            <div className='grid gap-4 md:grid-cols-[minmax(0,1fr)_220px]'>
              <div className='space-y-2'>
                ...
                <Textarea
                  ...
                  onChange={(event) =>
                    setCurrentRow((prev) => ({ ...prev, description: event.target.value }))
                  }
                  placeholder={t('engineering.productAppearance.placeholders.description')}
                />
              </div>
              <div className='flex h-full flex-col justify-end'>
                <div className='flex min-h-20 items-center justify-between rounded-[18px] border border-dashed border-primary/20 bg-background/80 px-4 py-3 shadow-sm'>
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => setCurrentRow((prev) => ({ ...prev, active: checked }))}
                  />
                </div>
              </div>
            </div>
