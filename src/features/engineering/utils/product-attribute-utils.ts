...
export function getAttributeValue(
  product: Pick<Product, 'attributeValues'> | undefined,
  categoryKey: string
): string {
  if (!product?.attributeValues?.length) return ''
  return (
    product.attributeValues.find((item) =>
      areSameProductAttributeCategoryKey(item.categoryKey, categoryKey)
    )?.optionValue || ''
  )
}
...
export function upsertAttributeValue(
  ...
  const index = nextValues.findIndex((item) =>
    areSameProductAttributeCategoryKey(item.categoryKey, categoryKey)
  )
...
export function getCategoryName(
  locale: string,
  category:
    | Pick<ProductAttributeCategory, 'nameZh' | 'nameEn' | 'key'>
    | undefined
): string {
  if (!category) return ''
  if (locale === 'en-US')
...
export function getOptionLabel(
  locale: string,
  option:
    | Pick<ProductAttributeOption, 'labelZh' | 'labelEn' | 'value'>
    | undefined
): string {
  if (!option) return ''
  if (locale === 'en-US')
