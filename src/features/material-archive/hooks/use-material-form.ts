    ...
    useEffect(() => {
        if (!material && materialName) {
            ...
            for (const [key, val] of Object.entries(categoryShortNames)) {
                if (selectedCategory && (selectedCategory.includes(key) || (typeof key === 'string' && key.includes(selectedCategory)))) {
                    prefix = val
                    break
                }
            }
            ...
            const suffix = alphanumericMatch ? alphanumericMatch.join('-').toUpperCase() : term
