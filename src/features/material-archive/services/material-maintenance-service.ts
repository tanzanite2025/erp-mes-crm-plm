...
export const MaterialMaintenanceService = {
  async saveMaterial(material: Partial<Material>): Promise<Material> {
    const payload = saveMaterialApiDTOSchema.parse(toSaveMaterialApiDTO(material))
    const updated = await apiFetch<MaterialApiDTO>('/materials', {
      method: 'POST',
      body: JSON.stringify(payload),
    })

    return toMaterialContract(parseMaterial(updated, 'MaterialMaintenanceService.saveMaterial'))
  },
  ...
  async saveMaterials(
    ...
    const payload = bulkSyncMaterialsApiDTOSchema.parse(toBulkSyncMaterialsApiDTO(materials, options))
  ...
  async patchMaterial(id: string, delta: DeltaSet, version: number): Promise<Material> {
    ...
    const payload: DeltaPayload = {
      op: 'PATCH',
      delta,
      metadata: buildVersionedPatchMetadata(id, expectedVersion, 'MaterialMaintenanceService.patchMaterial'),
    }
    ...
    return toMaterialContract(parseMaterial(res, 'MaterialMaintenanceService.patchMaterial'))
  },
}
