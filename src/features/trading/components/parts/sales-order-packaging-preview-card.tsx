  ...
  return (
    <section className='space-y-1.5'>
      ...
      <div className='space-y-1.5'>
        {data.lines.map((item) => (
          <div key={item.key} className='rounded-xl border bg-background/80 p-2'>
            ...
            {item.plan.lines.length > 0 ? (
              <div className='mt-1.5 grid gap-1.5 md:grid-cols-2'>
                {item.plan.lines.map((line) => (
                  <div key={`${item.key}-${line.profileId}`} className='rounded-lg border border-dashed bg-muted/20 p-1.5'>
                    <div className='flex items-center justify-between gap-2'>
                      <span className='text-[11px] font-black'>{line.profileName}</span>
