...
export function ShippingPlaceholderCard({
  title,
  description,
  actionLabel,
}: {
  ...
  return (
    <Card className='rounded-[28px] border-dashed border-border/60 bg-card p-6 shadow-none'>
      ...
      <div className='mt-6 flex flex-wrap gap-3'>
        <Button
          type='button'
          className='h-10 rounded-full bg-primary px-5 text-[10px] font-black tracking-widest text-primary-foreground uppercase shadow-lg shadow-primary/20'
        >
          {actionLabel}
        </Button>
        <Button
...
export function VirtualShipmentRow({
  item,
  onMatchVehicle,
}: {
  item: ShippingVehicleMatchItem
  onMatchVehicle?: (item: ShippingVehicleMatchItem) => void
}) {
  return (
      ...
      <div className='flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between'>
        <div className='space-y-3'>
          ...
          <div>
            ...
            <div className='mt-1 text-[11px] font-black tracking-widest text-muted-foreground/60 uppercase'>
              {item.orderNo || item.materialCode || item.shipmentId}
            </div>
            <div className='mt-2 text-xs text-muted-foreground'>
              {[item.materialName, item.materialCode, item.packageProfileName]
                .filter(Boolean)
                .join(' / ')}
            </div>
          </div>
        ...
        <div className='grid grid-cols-2 gap-3 md:grid-cols-4 lg:min-w-[520px]'>
          ...
          <div className='rounded-2xl border border-dashed border-border/60 bg-muted/[0.03] px-3 py-2'>
            ...
            <div className='mt-1 text-sm font-black'>
              {formatMetric(item.volumeM3, 1, 'm³')}
            </div>
          </div>
          <div className='rounded-2xl border border-dashed border-border/60 bg-muted/[0.03] px-3 py-2'>
            <div className='text-[9px] font-black tracking-widest text-muted-foreground/50 uppercase'>
            ...
            <div className='mt-1 text-sm font-black'>
              {formatMetric(item.weightKg, 0, 'kg')}
            </div>
          </div>
          <div className='rounded-2xl border border-dashed border-border/60 bg-primary/5 px-3 py-2'>
            ...
            <div className='mt-1 text-sm font-black text-primary'>
              {item.logisticsStatus || item.shipmentStatus || item.status}
            </div>
          </div>
        </div>
      </div>
