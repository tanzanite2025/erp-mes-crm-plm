...
function ShippingVehicleMatchRecommendationResolvedContent({
  item,
  summary,
}: {
  ...
  return (
    <div className='space-y-5'>
      <div className='grid grid-cols-2 gap-3 md:grid-cols-4'>
        ...
        <SummaryMetricCard
          title='体积'
          value={`${summary.totalVolumeM3.toFixed(1)} m³`}
        />
        <SummaryMetricCard
          title='重量'
      ...
      {packageInputNotice ? (
        <Card className='rounded-[22px] border-dashed border-primary/30 bg-primary/5 shadow-none'>
          <CardContent className='px-5 py-4 text-[11px] leading-relaxed text-primary/80'>
            {packageInputNotice}
          </CardContent>
        </Card>
      ) : null}
      ...
      {readResource.status === 'error' ? (
        <ConfigErrorPanel
          title={errorTitle || '车型推荐失败'}
          error={readResource.error}
      ...
      {readResource.status === 'loading' ? (
        <Card className='rounded-[22px] border-dashed border-primary/20 bg-primary/5 shadow-none'>
          <CardContent className='px-5 py-6 text-[10px] font-black tracking-widest text-primary/70 uppercase'>
            车型推荐计算中...
          </CardContent>
        </Card>
      ) : null}
      ...
      {readResource.status === 'ready' ? (
        <VehicleRecommendationPanel
          recommendations={readResource.data.recommendations}
        />
      ) : null}
    </div>
  )
...
export function ShippingVehicleMatchRecommendationDialog({
  item,
  open,
  onOpenChange,
}: Props) {
  const summaryResolution =
    resolveShippingVehicleMatchRecommendationSummary(item)
  const resolvedSummary = summaryResolution.summary

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='w-[95vw] overflow-hidden rounded-2xl border-none p-0 shadow-2xl sm:max-w-[1100px] md:rounded-[32px]'>
        <div className='relative space-y-6 p-5 md:p-8'>
          <DialogHeader className='text-left'>
            <div className='flex flex-wrap items-center gap-2'>
              ...
              {item?.packageProfileName ? (
                <Badge variant='outline'>{item.packageProfileName}</Badge>
              ) : null}
            </div>
            <DialogTitle>
              {item
                ? `${item.customerName || item.materialName || item.orderNo} · 车型匹配`
                : '车型匹配'}
            </DialogTitle>
            <DialogDescription>
              {item
                ? `${item.orderNo || item.shipmentId} / ${item.materialName || item.materialCode || '未关联货物'}`
                : '根据当前行数据计算可复用的车型推荐结果。'}
            </DialogDescription>
          </DialogHeader>

          {item && resolvedSummary ? (
            <ShippingVehicleMatchRecommendationResolvedContent
              item={item}
            ...
            <ConfigErrorPanel
              title='无法计算车型推荐'
              error={
                summaryResolution.error ?? new Error('未选择待匹配发货记录')
              }
              retryLabel='关闭'
