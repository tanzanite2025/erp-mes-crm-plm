...
export function ShippingVehicleMatchPage() {
  const { t } = useLanguage()
  const { items, isLoading, error, refetch } = useShippingVehicleMatch()
      ...
      {isLoading ? (
        ...
        <Card className='rounded-[24px] border-dashed border-destructive/40 bg-destructive/5 p-5 shadow-none'>
          <div className='space-y-3'>
            <div className='text-sm font-black text-destructive'>真实待匹配发货数据加载失败</div>
            <div className='text-xs leading-relaxed text-muted-foreground'>{error.message}</div>
            <div>
              <Button type='button' variant='outline' className='h-10 rounded-full border-dashed px-5 text-[10px] font-black uppercase tracking-widest' onClick={() => void refetch()}>
                重新加载
              </Button>
            </div>
          </div>
