...
const LEDGER_SORT_BY_OPTIONS = [
  ...
  { label: '未收金额', value: 'outstanding_amount' },
...
export function SalesReceivableDetailDialog({
  ...
  const detailQuery = useReceivableLedgerDetail(open ? ledgerId : null)
  ...
  const [selectedReceiptRecordId, setSelectedReceiptRecordId] = useState<string | null>(null)
  ...
  const receivableSearchQuery = useSearchReceivableLedgers({
  ...
  const records = useMemo(() => detail?.receiptRecords ?? [], [detail])
  const allocationHistory = useMemo(() => detail?.allocations ?? [], [detail])
  const ledgerOptions = useMemo(() => receivablesQuery.data?.items ?? [], [receivablesQuery.data])
  const remoteLedgerOptions = useMemo(() => receivableSearchQuery.data ?? [], [receivableSearchQuery.data])
  const ledgerDisplayMap = useMemo(() => {
    const entries: [string, string][] = ledgerOptions.map((option) => [
      option.id,
      `${option.documentNo} / ${option.customerName} / 未收 ${option.outstandingAmount}`,
    ] as [string, string])
    remoteLedgerOptions.forEach((option) => {
      entries.push([
        option.id,
        `${option.documentNo} / ${option.partnerName} / 未收 ${option.outstandingAmount}`,
      ] as [string, string])
    })
    if (detail) {
      entries.push([
        detail.id,
        `${detail.documentNo} / ${detail.customerName} / 未收 ${detail.outstandingAmount}`,
      ] as [string, string])
    }
  ...
  const allocationHistoryGroups = useMemo(() => {
    return records.map((record) => ({
      record,
      allocations: allocationHistory.filter((allocation) => allocation.receiptRecordId === record.id),
    }))
  ...
  const fallbackFilteredLedgerOptions = useMemo(() => {
    ...
    return ledgerOptions.filter((option) => {
      const haystack = `${option.documentNo} ${option.customerName} ${option.outstandingAmount}`.toLowerCase()
  ...
  const displayLedgerOptions = useMemo(() => {
    if (debouncedLedgerSearchTerm.trim().length >= 2) {
      return remoteLedgerOptions.map((option) => ({
        ...
        displayName: `${option.documentNo} / ${option.partnerName} / 未收 ${option.outstandingAmount}`,
      }))
    }
    return fallbackFilteredLedgerOptions.map((option) => ({
      id: option.id,
      documentNo: option.documentNo,
      displayName: `${option.documentNo} / ${option.customerName} / 未收 ${option.outstandingAmount}`,
    }))
    ...
    <>
      ...
      <DialogContent className='sm:max-w-3xl'>
        <DialogHeader>
          ...
          <DialogDescription>查看台账明细并登记一笔最小收款记录。</DialogDescription>
        </DialogHeader>
        ...
        <div className='grid gap-6'>
          <div className='grid gap-2 md:grid-cols-2'>
            ...
            <div className='rounded-lg border p-4'>
              ...
              <div className='mt-1 font-medium'>{detail?.customerName ?? '-'}</div>
            </div>
            <div className='rounded-lg border p-4'>
            ...
            <div className='rounded-lg border p-4'>
              <div className='text-sm text-muted-foreground'>未收金额</div>
              <div className='mt-1 font-medium'>{detail?.outstandingAmount ?? 0}</div>
            </div>
          </div>

          <div className='grid gap-4 rounded-lg border p-4'>
            <div className='text-sm font-medium'>登记收款</div>
            <div className='grid gap-4 md:grid-cols-2'>
              <div className='grid gap-2'>
                <Label htmlFor='receivable-date'>收款日期</Label>
                <Input id='receivable-date' type='date' value={recordDate} onChange={(event) => setRecordDate(event.target.value)} />
              </div>
              <div className='grid gap-2'>
                <Label htmlFor='receivable-ref'>参考号</Label>
                <Input id='receivable-ref' value={referenceNo} onChange={(event) => setReferenceNo(event.target.value)} />
              </div>
            </div>
            ...
            <div className='grid gap-2'>
              <Label htmlFor='receivable-ledger-search'>搜索台账</Label>
              <Input
                id='receivable-ledger-search'
                value={ledgerSearchTerm}
                onChange={(event) => setLedgerSearchTerm(event.target.value)}
                placeholder='按单据编号、客户、未收金额过滤台账'
              />
            </div>
            <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
              <div className='grid gap-2'>
                <Label htmlFor='receivable-ledger-status-filter'>状态</Label>
                <Select value={ledgerStatusFilter || '__all__'} onValueChange={(value) => setLedgerStatusFilter(value === '__all__' ? '' : value)}>
                  <SelectTrigger id='receivable-ledger-status-filter'>
              ...
              <div className='grid gap-2'>
                <Label htmlFor='receivable-ledger-currency-filter'>币种</Label>
                <Select
                ...
                >
                  <SelectTrigger id='receivable-ledger-currency-filter'>
              ...
              <div className='grid gap-2'>
                <Label htmlFor='receivable-ledger-outstanding-min'>未收最小值</Label>
                <Input
                  id='receivable-ledger-outstanding-min'
                  type='number'
              ...
              <div className='grid gap-2'>
                <Label htmlFor='receivable-ledger-outstanding-max'>未收最大值</Label>
                <Input
                  id='receivable-ledger-outstanding-max'
                  type='number'
            ...
            <div className='grid gap-4 md:grid-cols-2'>
              <div className='grid gap-2'>
                <Label htmlFor='receivable-ledger-sort-by'>排序字段</Label>
                <Select value={ledgerSortBy} onValueChange={setLedgerSortBy}>
                  <SelectTrigger id='receivable-ledger-sort-by'>
              ...
              <div className='grid gap-2'>
                <Label htmlFor='receivable-ledger-sort-order'>排序方向</Label>
                <Select value={ledgerSortOrder} onValueChange={setLedgerSortOrder}>
                  <SelectTrigger id='receivable-ledger-sort-order'>
            ...
            <div className='grid gap-3'>
              {allocations.map((item) => (
                <div key={item.sequenceNo} className='grid gap-3 rounded-md border p-3 md:grid-cols-[1fr_1fr_1fr_auto]'>
                  <div className='grid gap-2'>
                    <Label htmlFor={`receivable-allocation-ledger-${item.sequenceNo}`}>台账ID</Label>
                    <div id={`receivable-allocation-ledger-${item.sequenceNo}`} className='rounded-md border px-3 py-2 text-sm'>
                  ...
                  <div className='grid gap-2'>
                    <Label htmlFor={`receivable-allocation-amount-${item.sequenceNo}`}>分摊金额</Label>
                    <Input
                      id={`receivable-allocation-amount-${item.sequenceNo}`}
                      type='number'
                  ...
                  <div className='grid gap-2'>
                    <Label htmlFor={`receivable-allocation-remark-${item.sequenceNo}`}>备注</Label>
                    <Input
                      id={`receivable-allocation-remark-${item.sequenceNo}`}
                      value={item.remark}
          ...
          <div className='rounded-lg border p-4'>
            <div className='mb-3 flex items-center justify-between gap-4'>
              <div className='text-sm font-medium'>收款记录</div>
              <Button
                type='button'
                ...
                {filteredRecords.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className='text-center text-muted-foreground'>
                      {showOnlyMissingEvidenceRecords ? '当前没有缺凭证的收款记录' : '暂无收款记录'}
                    </TableCell>
                  </TableRow>
                ) : (
                  filteredRecords.map((record) => (
                    <TableRow
                      ...
                      data-state={selectedReceiptRecordId === record.id ? 'selected' : undefined}
                      onClick={() => setSelectedReceiptRecordId(record.id)}
                    >
          ...
          <SettlementRecordEvidencePanel
            recordId={selectedReceiptRecordId}
            recordType='receipt'
          ...
          <div className='rounded-lg border p-4'>
            ...
            <div className='mb-4 grid gap-2'>
              <Label htmlFor='receivable-history-search'>筛选历史</Label>
              <Input
                id='receivable-history-search'
      ...
      <LedgerSearchDialog
        ...
        title='选择应收台账'
        description='在独立弹窗内搜索、筛选并确认一条应收台账候选。'
        partnerLabel='客户'
        outstandingLabel='未收金额'
        ...
        isSearching={receivableSearchQuery.isFetching}
