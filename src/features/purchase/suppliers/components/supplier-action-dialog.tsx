...
import { buildSupplierSaveSnapshot } from '../utils/supplier-save-snapshot'
...
export function SupplierActionDialog({
  ...
  const { initialFormData, categoryOptions, statusOptions } =
    useSupplierActionViewModel({ supplier, t })
  const { data: formData, tracker } = useDeltaTracker(
    initialFormData as Supplier,
    open
  )
  ...
  const updateField = <K extends keyof Supplier>(
    ...
    Reflect.set(
      formData as unknown as Partial<
        Record<keyof Supplier, Supplier[keyof Supplier]>
      >,
      key,
      value
    )
  ...
  const handleSave = () => {
    ...
    // Convert proxy to plain object before cloning
    ...
    // For new suppliers, code can be empty (will be auto-generated by backend)
  ...
  const removeProduct = (product: string) => {
    updateField(
      'mainProducts',
      formData.mainProducts?.filter((item) => item !== product) ?? []
    )
  }
    ...
    <ActionDialogShell
      ...
      title={
        <div className='flex items-center justify-between gap-3'>
          <span>
            {supplier
              ? t('purchase.suppliers.dialogEditTitle')
              : t('purchase.suppliers.dialogCreateTitle')}
          </span>
          {supplier?.id ? (
      ...
      footer={
        <>
          ...
          <Button
            onClick={handleSave}
            className='h-11 flex-1 rounded-full bg-primary px-10 text-[10px] font-black tracking-widest uppercase shadow-xl shadow-primary/20 transition-all hover:bg-primary/90 active:scale-95 sm:flex-none'
          >
      ...
      <div className='grid gap-3'>
        ...
        <div className='mb-1 flex min-h-[50px] flex-wrap gap-2 rounded-2xl border-2 border-dashed border-muted-foreground/5 bg-muted/30 p-3'>
          ...
          {(!formData.mainProducts || formData.mainProducts.length === 0) && (
            <span className='p-1 text-[9px] font-black tracking-widest text-muted-foreground/20 uppercase italic'>
