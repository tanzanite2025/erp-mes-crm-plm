...
import type { PurchaseReturnRecord } from '../services/purchase-return-service'
import {
...
export const PurchaseReturnPrint = React.forwardRef<
  HTMLDivElement,
  PurchaseReturnPrintProps
>(({ record }, ref) => {
  const user = useAuthStore((state) => state.user)
  const hasMainEvidences = (record.evidences?.length || 0) > 0
  const lineEvidenceLines = record.lines.filter(
    (line) => (line.evidences?.length || 0) > 0
  )
      ...
      >
        <PurchasePrintInfoGrid
          items={[
            ...
            {
              label: '退货日期',
              value: formatPurchasePrintDateTime(record.returnDate),
            },
            ...
            <tbody>
              {record.lines.map((line) => (
                <tr
                  ...
                  className='purchase-print-avoid-break'
                >
          ...
          >
            <div className='space-y-6'>
              {lineEvidenceLines.map((line) => (
                <div
                  key={`evidence-${record.id}-${line.id}`}
                  className='purchase-print-avoid-break space-y-3'
                >
                  <div className='border-l-4 border-black pl-3 text-sm font-bold'>
                    第 {line.lineNo} 行 / {line.materialName} /{' '}
                    {line.materialCode}
                  </div>
                  <div className='purchase-print-photo-grid'>
                    {line.evidences?.map((evidence, index) => (
                      <PurchasePrintPhotoCard
                        ...
                        metaRows={[
                          {
                            label: '异常分类',
                            value: line.issueCategory || record.issueCategory,
                          },
                          {
                            label: '备注说明',
                            value:
                              evidence.note || line.reason || record.reason,
                          },
                          { label: '拍摄位置', value: evidence.location },
                          { label: '缺陷部位', value: evidence.defectPart },
                          {
                            label: '拍摄时间',
                            value: formatPurchasePrintDateTime(
                              evidence.uploadedAt
                            ),
                          },
                        ]}
