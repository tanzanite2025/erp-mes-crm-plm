...
export function DocumentFooterStats({ formData, onCancel, onSave }: DocumentFooterStatsProps) {
  const { t } = useLanguage()
    ...
    <DialogFooter className='sticky bottom-0 z-20 flex flex-col items-stretch justify-between gap-5 border-t bg-background/90 px-4 py-4 backdrop-blur-md sm:flex-row sm:items-center sm:gap-6 sm:px-6 sm:py-3'>
      ...
      <div className='order-2 flex shrink-0 flex-col gap-2.5 sm:order-1 sm:flex-row sm:gap-3'>
        ...
        <Button
          variant='ghost'
          onClick={onCancel}
          className='order-2 h-11 rounded-full border border-muted-foreground/10 px-6 text-[11px] font-bold uppercase transition-all hover:bg-muted/50 sm:order-1'
        >
