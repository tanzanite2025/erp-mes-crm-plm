    ...
    return (
        <div className="flex flex-col gap-5 animate-in fade-in duration-700">
            ...
            <div className='grid grid-cols-1 md:grid-cols-3 gap-3'>
                <Card className='rounded-[20px] border-none bg-rose-500/5 shadow-none overflow-hidden relative group'>
                    <CardContent className='p-3.5 flex items-center justify-between gap-3'>
                        <span className='text-[10px] font-black text-rose-600/60 uppercase tracking-widest whitespace-nowrap'>{t('quality.abnormalities.page.activeCriticals')}</span>
                        <div className='text-xl font-black italic tracking-tighter text-rose-600 tabular-nums font-mono shrink-0'>
                            {abnormalities?.filter((item) => item.severity === 'CRITICAL').length || 0}
                        </div>
                    </CardContent>
                </Card>
                <Card className='rounded-[20px] border-none bg-amber-500/5 shadow-none overflow-hidden relative group'>
                    <CardContent className='p-3.5 flex items-center justify-between gap-3'>
                        <span className='text-[10px] font-black text-amber-600/60 uppercase tracking-widest whitespace-nowrap'>{t('quality.abnormalities.page.openReports')}</span>
                        <div className='text-xl font-black italic tracking-tighter text-amber-600 tabular-nums font-mono shrink-0'>
                            {abnormalities?.filter((item) => item.status === 'OPEN').length || 0}
                        </div>
                    </CardContent>
                </Card>
                <Card className='rounded-[20px] border-none bg-emerald-500/5 shadow-none overflow-hidden relative group'>
                    <CardContent className='p-3.5 flex items-center justify-between gap-3'>
                        <span className='text-[10px] font-black text-emerald-600/60 uppercase tracking-widest whitespace-nowrap'>{t('quality.abnormalities.page.closedLooped')}</span>
                        <div className='text-xl font-black italic tracking-tighter text-emerald-600 tabular-nums font-mono shrink-0'>
                ...
                {abnormalities?.length === 0 ? (
                    ...
                    abnormalities?.map((ab: QualityAbnormality) => (
                        <Card key={ab.id} className='group relative rounded-[24px] border border-dashed border-muted/50 bg-background hover:bg-muted/5 transition-all overflow-hidden cursor-pointer active:scale-[0.99]'>
                            ...
                            />
                            <CardContent className='py-3.5 px-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4'>
                                <div className='flex items-center gap-3 lg:gap-4 flex-1 min-w-0'>
                                    <div className='size-9 lg:size-10 rounded-xl bg-muted/10 flex items-center justify-center shrink-0 group-hover:bg-white transition-colors border border-transparent group-hover:border-muted-foreground/10'>
                                ...
                                <div className='flex items-center justify-between lg:justify-end gap-0 lg:gap-8 border-t lg:border-t-0 border-dashed border-muted/20 pt-3 lg:pt-0'>
                                    <div className='flex items-center gap-6 lg:gap-8'>
                                        ...
                                        <div className='flex flex-col gap-0.5 w-[110px] lg:w-[130px]'>
