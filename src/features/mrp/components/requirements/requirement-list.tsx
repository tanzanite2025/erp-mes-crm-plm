    ...
    <div className='space-y-4 animate-in fade-in duration-500 pb-20'>
      ...
      {sections.length === 0 ? (
        <div className='rounded-2xl border-2 border-dashed h-[50vh] flex flex-col items-center justify-center text-muted-foreground/30 bg-muted/5 mx-1'>
          ...
          <p className='text-[10px] font-black uppercase tracking-[0.2em]'>{t('trading.requirements.list.empty')}</p>
        </div>
      ) : (
                    ...
                    <div className='flex flex-col gap-0.5'>
                      ...
                      <div className='flex items-center gap-2 uppercase'>
                        <Badge variant='secondary' className='text-[10px] font-black px-2 py-0.5 h-4.5 rounded-full bg-primary/10 text-primary border-primary/20'>
                          {t('trading.requirements.list.items', { count: groupedData[section].length })}
                        </Badge>
                  ...
                  <Table className='w-full border-collapse text-left'>
                    <TableHeader className='bg-muted/10'>
                      <TableRow className='hover:bg-transparent border-b border-dashed'>
                        <TableHead className='w-[100px] min-w-[100px] text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 pl-6 py-4 italic'>{t('trading.requirements.list.headers.code')}</TableHead>
                        <TableHead className='w-auto min-w-[200px] text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 py-4 italic'>{t('trading.requirements.list.headers.name')}</TableHead>
                        <TableHead className='w-[90px] min-w-[90px] text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-right py-4 italic'>{t('trading.requirements.list.headers.total')}</TableHead>
                        <TableHead className='w-[90px] min-w-[90px] text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-right py-4 italic'>{t('trading.requirements.list.headers.inventory')}</TableHead>
                        <TableHead className='w-[200px] min-w-[200px] text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 px-4 py-4 italic'>{t('trading.requirements.list.headers.supplyAnalysis')}</TableHead>
                        <TableHead className='w-[110px] min-w-[110px] text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 text-right py-4 italic'>{t('trading.requirements.list.headers.effectiveGap')}</TableHead>
                        <TableHead className='w-[150px] min-w-[150px] text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 py-4 text-center italic'>{t('trading.requirements.list.headers.packaging')}</TableHead>
                        <TableHead className='w-[140px] min-w-[140px] text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 pr-6 py-4 text-right italic'>{t('trading.requirements.list.headers.status')}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
