...
export function useRequirements() {
  ...
  const [calculateError, setCalculateError] =
    useState<RequirementsError | null>(null)
  ...
  const calculateMutation = useMutation({
    ...
    mutationFn: async (selectedKeys?: string[]) => {
      if (selectedKeys) {
        logger.info(
          `Calculating requirements for ${selectedKeys.length} keys...`
        )
      }
      const result = await requirementService.getMrpRequirements(
        selectedKeys ?? []
      )
      if (!result.requirements) {
        throw new Error(
          '[CRITICAL] MRP Calculation returned invalid null requirements'
        )
      }
      return result
    },
