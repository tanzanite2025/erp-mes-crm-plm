...
export function useAssets() {
  ...
  const reloadAll = () =>
    Promise.all([reloadMolds(), reloadFurnaces(), reloadLoans()])
  ...
  const loans = useMemo(() => {
    ...
    if (!loansQuery.data) {
      const error = new Error(
        '[CRITICAL] Asset loans data is missing after load'
      )
  ...
  const moldMutation = useMutation({
    mutationFn: async ({
      ...
      if (isPatch && delta && mold.id) {
        return MoldMaintenanceService.patchMold(
          mold.id,
          delta,
          mold.version || 1
        )
    ...
    onMutate: async ({ mold }) => {
      ...
      const previousMolds =
        queryClient.getQueryData<Mold[]>(MOLDS_QUERY_KEY) || []
  ...
  return {
    ...
    loans,
    isLoading:
      moldsQuery.isLoading || furnacesQuery.isLoading || loansQuery.isLoading,
