...
export function useDashboardStats(): DashboardStats {
    ...
    const moldStats = {
        ...
        maintenance: molds.filter(m => m.status === 'CHECKING' || m.status === 'MAINTENANCE').length,
        fault: molds.filter(m => m.status === 'RETIRED').length,
        overdue: (loans as any[] || []).filter(l => l.status === 'OVERDUE').length,
    }
    ...
    const furnaceStats = {
        ...
        running: furnaces.filter(f => f.status === 'HEATING' || f.status === 'COOLING').length,
