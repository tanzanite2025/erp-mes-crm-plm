...
export class MoldLoanService {
  ...
  static async createLoan(loanData: Omit<MoldLoan, 'id' | 'createdAt'>) {
    ...
    if (!newLoan) {
      throw new Error(
        '[CRITICAL_DATA_PATH] Create mold loan failed, returned no data.'
      )
    }
  ...
  static async createBorrowRecord(
    ...
    return ensureObjectResponse<
      MoldBorrowRecordResponse & Record<string, unknown>
    >(res, 'MoldLoanService.createBorrowRecord') as MoldBorrowRecordResponse
  }
  ...
  static async patchLoan(
    ...
    if (!res) {
      throw new Error(
        `[CRITICAL_DATA_PATH] Patch mold loan ${loanId} failed, returned no data.`
      )
    }
