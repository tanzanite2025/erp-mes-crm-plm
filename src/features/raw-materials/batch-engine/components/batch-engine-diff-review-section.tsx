...
export function BatchEngineDiffReviewSection(
  props: BatchEngineDiffReviewSectionProps
) {
  const {
    plans,
    selectedPlanRank,
      ...
      {selectedPlan ? (
        <div className='rounded-[24px] border border-dashed border-slate-300 bg-white/90 p-4'>
          ...
          <div className='mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4'>
            ...
            <DiffMetricCard
              label='候选预算'
              value={`${selectedPlan.candidateBudgetSummary.mergedCandidateCount}/${selectedPlan.candidateBudgetSummary.globalBudget}`}
              tone='slate'
            />
            <DiffMetricCard
              label='相邻破坏'
              value={`${selectedPlan.reportSummary.adjacencyBreakCount}`}
              tone='amber'
            />
            <DiffMetricCard
              label='切卷次数'
              value={`${selectedPlan.reportSummary.rollSwitchCount}`}
              tone='amber'
            />
            <DiffMetricCard
              label='残料复用命中'
              value={`${selectedPlan.reportSummary.geometryReuseHitCount}`}
              tone='emerald'
            />
          </div>
                  ...
                  .map((item) => ({
                    ...
                    content: (
                      <div className='flex flex-wrap items-center gap-1.5'>
                        ...
                        />
                        <BatchEnginePhase7ExplainabilityMetaBadge
                          label={buildPhase7SeverityBadgeLabel(
                            item.severityScore
                          )}
                          tone='rose'
                          compact
                  ...
                  .map((item) => ({
                    ...
                    content: (
                      <div className='flex flex-wrap items-center gap-1.5'>
                        ...
                        />
                        <BatchEnginePhase7ExplainabilityMetaBadge
                          label={buildPhase7DensityBadgeLabel(
                            item.densityScore
                          )}
                          tone='slate'
                          compact
              ...
              rows={[
                {
                  key: 'search-preset',
                  content: (
                    <p>搜索预设: {selectedPlan.searchConfig.presetKey}</p>
                  ),
                },
                {
                  key: 'beam-width',
                  content: (
                    <p>Beam Width: {selectedPlan.searchConfig.beamWidth}</p>
                  ),
                },
                {
          ...
          <div className='mt-4 rounded-[20px] border border-dashed border-slate-200 bg-slate-50/70 p-3 text-xs font-semibold text-slate-700'>
            热区归因:{' '}
            {selectedPlan.explainabilitySummary.heatZoneAttributions
              .slice(0, 3)
              .map((item) => `${item.zoneId}:${item.reason}`)
              .join(' / ') || '--'}
          </div>
        </div>
      ) : null}
