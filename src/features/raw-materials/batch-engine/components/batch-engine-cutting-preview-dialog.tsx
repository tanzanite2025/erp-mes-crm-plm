...
export function BatchEngineCuttingPreviewDialog(
  props: BatchEngineCuttingPreviewDialogProps
) {
  const { t } = useLanguage()
  const {
    open,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        size='full'
        showCloseButton
        className='flex max-h-[calc(100dvh-1rem)] min-h-0 w-[calc(100vw-1rem)] max-w-[1500px] flex-col gap-0 overflow-hidden rounded-[24px] p-0 sm:w-[calc(100vw-2rem)] sm:max-w-[1500px]'
      >
        ...
        <DialogFooter className='border-t border-dashed border-slate-200 bg-slate-50/70 px-4 py-2.5 sm:items-center sm:justify-between'>
          <div className='flex min-w-0 flex-wrap items-center gap-1.5 text-[10px] font-semibold text-slate-600'>
            <LegendDot
              color='bg-slate-900'
              label={t('rawMaterials.batchEngine.canvasPreview.legend.roll')}
            />
            <LegendDot
              color='bg-cyan-500'
              label={t('rawMaterials.batchEngine.canvasPreview.legend.strip')}
            />
            <LegendDot
              color='bg-emerald-500'
              label={t('rawMaterials.batchEngine.canvasPreview.legend.piece')}
            />
            <LegendDot
              color='bg-amber-400'
              label={t('rawMaterials.batchEngine.canvasPreview.legend.loss')}
            />
            <LegendDot
              color='bg-slate-400'
              label={t(
                'rawMaterials.batchEngine.canvasPreview.legend.aggregate'
              )}
            />
          </div>
          <Button
            ...
            className='shrink-0 rounded-full px-5 font-black'
          >
