...
export function BatchEnginePreviewDemandDetailCard(props: BatchEnginePreviewDemandDetailCardProps) {
  const {
  ...
  const showingFallbackFocus = !hasExplicitSelection && Boolean(effectiveSelectedDemand)
      ...
      {effectiveSelectedDemand ? (
        <div className='mt-3 grid gap-2 text-xs font-semibold text-slate-700'>
          {showingFallbackFocus ? (
            <div className='rounded-2xl border border-amber-200 bg-amber-500/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-amber-700'>
              当前未显式选择需求行，下面展示的是筛选结果中的默认焦点。
            </div>
          ) : null}
          {showingDifferentEffectiveFocus ? (
            <div className='rounded-2xl border border-cyan-200 bg-cyan-500/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-cyan-700'>
              已显式选择 {explicitSelectedDemandLineId}，但当前筛选仅命中 {effectiveSelectedDemandLineId}，以下为当前焦点详情。
            </div>
          ) : null}
          {hasExplicitSelection && explicitSelectedDemand && !showingDifferentEffectiveFocus ? (
            <div className='rounded-2xl border border-emerald-200 bg-emerald-500/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-emerald-700'>
              当前展示的是你显式选择的需求行。
            </div>
          ) : null}
          ...
          <p>跨卷分配: {effectiveSelectedDemand.isSplitAcrossRolls ? '是' : '否'}</p>
        </div>
      ) : (
        <div className='mt-3 grid gap-2 text-xs font-semibold text-slate-700'>
          <p>当前未选中需求行。</p>
