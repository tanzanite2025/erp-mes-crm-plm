...
export function BatchEnginePhase7HomeSummarySection(
  props: BatchEnginePhase7HomeSummarySectionProps
) {
  const {
    selectedPlan,
    activeDiffSummary,
  ...
  const activeSourceLabel = resolveExplainabilitySourceLabel(
    selectedExplainabilityTargetSource
  )
  const isPrimaryBreakSliceActive =
    selectedExplainabilityTargetKind === 'break-slice' &&
    selectedExplainabilityTargetId === primaryBreakSlice?.id
  const isPrimaryZoneClusterActive =
    selectedExplainabilityTargetKind === 'zone-cluster' &&
    selectedExplainabilityTargetId === primaryZoneCluster?.clusterId
  const breakSliceQuickActions = resolveSecondaryBreakSlices(
    selectedPlan,
    3
  ).map((item) => ({
    key: item.id,
  ...
  const zoneClusterQuickActions = resolveSecondaryZoneClusters(
    selectedPlan,
    3
  ).map((item) => ({
    key: item.clusterId,
    ...
    <div className='rounded-[24px] border border-dashed border-border/60 bg-card p-4'>
      <div className='flex flex-wrap items-start justify-between gap-3'>
        <div>
          ...
          <p className='mt-1 text-[9px] font-black tracking-[0.18em] text-muted-foreground/40 uppercase'>
            break slice / zone cluster / dynamic budget
          </p>
        </div>
        <div className='rounded-full border border-dashed border-primary/40 bg-primary/10 px-3 py-1 text-[10px] font-black tracking-[0.18em] text-primary uppercase'>
      ...
      <div className='mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5'>
        <Phase7HeroMetricCard
          ...
          detail={
            primaryBreakSlice
              ? `${primaryBreakSlice.segmentKind}:${primaryBreakSlice.segmentKey} / severity ${primaryBreakSlice.severityScore.toFixed(2)}`
              : '当前没有可联动 break slice'
          }
          actionLabel={primaryBreakSlice ? '点击联动预览' : undefined}
          active={isPrimaryBreakSliceActive}
          sourceLabel={
            isPrimaryBreakSliceActive ? activeSourceLabel : undefined
          }
        ...
        <Phase7HeroMetricCard
          ...
          detail={
            primaryZoneCluster
              ? `${primaryZoneCluster.clusterId} / density ${primaryZoneCluster.densityScore.toFixed(2)}`
              : '当前没有可联动 zone cluster'
          }
          actionLabel={primaryZoneCluster ? '点击联动预览' : undefined}
          active={isPrimaryZoneClusterActive}
          sourceLabel={
            isPrimaryZoneClusterActive ? activeSourceLabel : undefined
          }
        ...
        <Phase7HeroMetricCard
          label='Cluster 密度峰值'
          value={`${(selectedPlan.explainabilitySummary.zoneClusters[0]?.densityScore ?? 0).toFixed(2)}`}
          tone='amber'
        />
        <Phase7HeroMetricCard
          label='差异热区'
          value={`${(activeDiffSummary ?? selectedPlan.diffSummary).highlightZoneIds.length}`}
          tone='rose'
        />
      </div>
      ...
      <div className='mt-4 grid gap-3 xl:grid-cols-3'>
        <Phase7SummaryBlock
          title='Break Summary'
          content={
            selectedPlan.explainabilitySummary.primaryBreakReasons.join(
              ' / '
            ) || '连续段稳定'
          }
          tone='violet'
        ...
        <Phase7SummaryBlock
          ...
          content={
            selectedPlan.explainabilitySummary.zoneClusters
              ...
              .map(
                (item) =>
                  `${item.clusterId} / density ${item.densityScore.toFixed(2)}`
              )
              .join(' / ') || '热区聚类稳定'
          }
          tone='amber'
        ...
        <Phase7SummaryBlock
          ...
          content={
            selectedPlan.candidateBudgetSummary.dynamicStrategyStats
              ...
              .map(
                (item) =>
                  `${item.strategyKey} / quota ${item.targetQuota} / priority ${item.priorityScore.toFixed(2)}`
              )
              .join(' / ') || '动态预算稳定'
          }
          tone='slate'
...
function Phase7HeroMetricCard({
  ...
  const content = (
    <>
      ...
      {detail ? (
        <p className='mt-2 text-[10px] leading-4 font-semibold opacity-80'>
          {detail}
        </p>
      ) : null}
      {actionLabel ? (
        <p className='mt-3 text-[9px] font-black tracking-[0.18em] uppercase opacity-75'>
          {actionLabel}
        </p>
      ) : null}
      {active && sourceLabel ? (
        <div className='mt-2 flex items-center gap-1.5 font-mono text-[8px] tracking-[0.14em] uppercase opacity-80'>
  ...
  return (
    <div
      className={`rounded-[20px] border border-dashed px-3 py-3 ${className} ${activeClassName}`.trim()}
    >
      {onClick ? (
        <button
          type='button'
...
function Phase7SummaryBlock({
  ...
  return (
    ...
    >
      ...
      <p className='mt-2 text-xs leading-5 font-semibold'>{content}</p>
    </div>
  )
}
