...
export function BatchEnginePreviewDiffSummaryCard(
  props: BatchEnginePreviewDiffSummaryCardProps
) {
      ...
      solvedContent={
        selectedPlan && diffSummary ? (
          <div className='mt-3 grid gap-2 text-xs font-semibold text-slate-700'>
            ...
            <p>
              变化需求: {diffSummary.changedDemandLineIds.join(', ') || '--'}
            </p>
            <p>变化卷材: {diffSummary.changedRollIds.join(', ') || '--'}</p>
            <p>热区数: {diffSummary.highlightZoneIds.length}</p>
            <p>
              Break Slice:{' '}
              {selectedPlan.explainabilitySummary.breakSlices.length}
            </p>
            <p>
              Zone Cluster:{' '}
              {selectedPlan.explainabilitySummary.zoneClusters.length}
            </p>
