...
export function resolveBatchEngineDemandLineRules(
  ...
  return {
    ...
    orderSequence: Math.floor(toPositiveNumber(line.constraintProfile?.orderSequence)),
    yarnDirectionMode: line.constraintProfile?.yarnDirectionMode?.trim() || line.yarnDirection?.trim() || '',
    processTags: normalizeRuleStringArray(line.constraintProfile?.processTags),
    noteKeywords: normalizeRuleStringArray(line.constraintProfile?.noteKeywords),
  }
}
