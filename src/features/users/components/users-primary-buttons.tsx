...
export function UsersPrimaryButtons() {
  ...
  return (
    <div className='flex flex-col sm:flex-row gap-3 w-full sm:w-auto'>
      <NonBlockingPermissionBoundary permission='perm_manage'>
        <Button
          variant='ghost'
          className='flex-1 sm:flex-initial rounded-full h-11 px-6 border border-dashed border-primary/20 hover:bg-primary/5 text-primary font-black text-[10px] uppercase tracking-widest transition-all'
          onClick={() => setOpen('addAdmin')}
        >
          <ShieldCheck className='mr-2 size-4 shrink-0' /> {t('users.actions.addAdmin')}
        </Button>
