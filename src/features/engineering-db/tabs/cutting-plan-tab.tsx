...
export function CuttingPlanTab() {
  ...
  const { downloadTemplate, parseExcel, exportPrint, previewPrint } =
    useCuttingPlanImportExport()
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success(
        editingPlan
          ? t('engineering.cuttingPlan.toasts.saveUpdated')
          : t('engineering.cuttingPlan.toasts.saveCreated')
      )
      closeDialog()
    },
    onError: (error) => {
      ...
      const message =
        error instanceof Error
          ? error.message
          : t('engineering.cuttingPlan.toasts.saveFailed')
  ...
  const handlePreparationIssues = (error: CuttingPlanPreparationError) => {
    ...
    switch (firstIssue.kind) {
      ...
      case 'missing_cut_size_binding':
        toast.error(
          `第 ${firstIssue.sequenceNo} 行未绑定尺寸库条目，裁纱单所有行必须引用尺寸库。`
        )
        return
      case 'missing_cut_size_unit':
        toast.error(
          `第 ${firstIssue.sequenceNo} 行引用的尺寸库条目不存在或未启用，请先修复尺寸库。`
        )
        return
    }
  }

  const preparePlanForOutput = (
    plan: CuttingPlanInput | CuttingPlan
  ): CuttingPlanInput | null => {
    if (cutSizeQuery.isLoading) {
  ...
  return (
    <div className='flex animate-in flex-col gap-5 duration-700 fade-in'>
      <IndustrialHeader
        ...
        statusBadge={
          <div className='grid grid-cols-3 gap-2 text-center md:min-w-[360px]'>
            <Metric
              label={t('engineering.cuttingPlan.metrics.plans')}
              value={plans.length}
            />
            <Metric
              label={t('engineering.cuttingPlan.metrics.active')}
              value={activeCount}
            />
            <Metric
              label={t('engineering.cuttingPlan.metrics.lines')}
              value={lineCount}
            />
          </div>
        }
      ...
      {invalidItems.length > 0 ? (
        <InvalidCuttingPlanAlert invalidItems={invalidItems} t={t} />
      ) : null}
        ...
        {isLoading ? (
          ...
          <div className='rounded-[24px] border border-dashed border-muted-foreground/15 p-12 text-center'>
            <p className='text-sm font-black'>
              {t('engineering.cuttingPlan.empty.title')}
            </p>
                ...
                >
                  ...
                  <div className='mt-2 grid gap-2 text-xs font-semibold text-muted-foreground md:grid-cols-4'>
                    ...
                    <Info
                      icon={CalendarDays}
                      label={t(
                        'engineering.cuttingPlan.fields.revisionEffective'
                      )}
                      value={`${plan.revisionNo || '--'} / ${plan.effectiveDate || '--'}`}
                    />
                    <Info
                      icon={Layers3}
                      label={t('engineering.cuttingPlan.fields.material')}
                      value={plan.carbonFiberModel || '--'}
                    />
                    <Info
                      icon={Scissors}
                      label={t('engineering.cuttingPlan.fields.lineCount')}
                      value={t('engineering.cuttingPlan.values.lineCount', {
        ...
        <DialogContent className='flex h-[85vh] w-[min(96vw,1500px)] flex-col overflow-hidden rounded-[32px] p-0 sm:max-w-[1500px]'>
          <DialogHeader className='border-b border-dashed border-muted-foreground/15 px-6 py-5'>
            <DialogTitle className='flex items-center gap-2 text-xl font-black tracking-tighter italic'>
              ...
              {editingPlan
                ? t('engineering.cuttingPlan.dialog.editTitle')
                : t('engineering.cuttingPlan.dialog.createTitle')}
            </DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter className='border-t border-dashed border-muted-foreground/15 px-6 py-4 sm:flex-row sm:flex-wrap sm:justify-end'>
            ...
            >
              {saveMutation.isPending
                ? `${t('engineering.cuttingPlan.actions.save')}...`
                : t('engineering.cuttingPlan.actions.save')}
            </Button>
          </DialogFooter>
        </DialogContent>
...
function getInvalidCuttingPlanFailureFilterLabel(
  ...
  switch (failureType) {
    case 'missing_required_fields':
      return t(
        'engineering.cuttingPlan.alerts.failureTypes.missing_required_fields',
        { count }
      )
    case 'invalid_lines':
      return t('engineering.cuttingPlan.alerts.failureTypes.invalid_lines', {
        count,
      })
    case 'schema_mismatch':
      return t('engineering.cuttingPlan.alerts.failureTypes.schema_mismatch', {
        count,
      })
    case 'unknown_invalid_payload':
      return t(
        'engineering.cuttingPlan.alerts.failureTypes.unknown_invalid_payload',
        { count }
      )
  }
}
...
function getInvalidCuttingPlanFailureGroupLabel(
  ...
  switch (failureType) {
    case 'missing_required_fields':
      return t(
        'engineering.cuttingPlan.alerts.failureGroups.missing_required_fields'
      )
    case 'invalid_lines':
    ...
    case 'unknown_invalid_payload':
      return t(
        'engineering.cuttingPlan.alerts.failureGroups.unknown_invalid_payload'
      )
  }
}
...
function InvalidCuttingPlanAlert({
  ...
  const [selectedFailureType, setSelectedFailureType] = useState<
    'all' | InvalidCuttingPlanFailureType
  >('all')

  const groupedInvalidItems = useMemo(() => {
    const result = new Map<
      InvalidCuttingPlanFailureType,
  ...
  const effectiveFailureType =
    ...
    availableFailureTypes.includes(selectedFailureType)
      ? selectedFailureType
      : 'all'
  ...
  const filteredInvalidItems = useMemo(() => {
    ...
    return invalidItems.filter(
      (item) => item.failureType === effectiveFailureType
    )
    ...
    <div className='relative overflow-hidden rounded-[24px] border border-dashed border-amber-500/40 bg-amber-500/5'>
      ...
      <div className='relative flex flex-col gap-4 p-5'>
          ...
          <div className='min-w-0'>
            <div className='flex items-center gap-2'>
              ...
              <div className='text-sm font-black tracking-tighter text-amber-700 italic'>
                {t('engineering.cuttingPlan.alerts.invalidRecordsTitle', {
                  count: invalidItems.length,
                })}
              </div>
            </div>
            <p className='mt-1 text-[9px] font-black tracking-widest text-amber-700/70 uppercase'>
              {t('engineering.cuttingPlan.alerts.invalidRecordsDescription')}
            </p>
          </div>
          <Badge className='h-5 rounded-full bg-amber-500/10 px-2.5 font-mono text-[8px] text-amber-700'>
            {t('engineering.cuttingPlan.alerts.invalidRecordsBadge', {
              count: invalidItems.length,
            })}
          </Badge>
        </div>
        ...
        <div className='flex flex-wrap gap-2'>
          ...
          >
            {t('engineering.cuttingPlan.alerts.filters.all', {
              count: invalidItems.length,
            })}
          </Button>
          {availableFailureTypes.map((failureType) => {
            ...
            return (
              <Button
                ...
                type='button'
                variant={
                  effectiveFailureType === failureType ? 'default' : 'outline'
                }
                size='sm'
                onClick={() => setSelectedFailureType(failureType)}
                className='h-8 rounded-full px-3 text-[10px] font-black tracking-widest uppercase'
              >
                {getInvalidCuttingPlanFailureFilterLabel(
                  t,
                  failureType,
                  groupItems.length
                )}
        ...
        {filteredInvalidItems.length === 0 ? (
          <div className='rounded-[20px] border border-dashed border-amber-500/30 bg-background/70 p-6 text-center text-[10px] font-black tracking-widest text-amber-700 uppercase'>
            ...
            {filteredGroupedEntries.map(({ failureType, items }) => (
              <div key={failureType} className='grid gap-3'>
                <div className='flex items-center justify-between gap-3 rounded-[20px] border border-dashed border-amber-500/25 bg-background/60 px-4 py-3'>
                  ...
                  <Badge className='h-5 rounded-full bg-amber-500/10 px-2.5 font-mono text-[8px] text-amber-700'>
                    {t('engineering.cuttingPlan.alerts.failureGroupCount', {
                      count: items.length,
                    })}
                  </Badge>
                </div>
                    ...
                    <div className='flex flex-col gap-2 md:flex-row md:items-start md:justify-between'>
                      <div className='min-w-0'>
                        ...
                        <div className='mt-1 flex flex-wrap gap-2'>
                          <Badge
                            variant='outline'
                            className='rounded-full border-dashed font-mono text-[8px]'
                          >
                            {t(
                              'engineering.cuttingPlan.alerts.invalidRecordCode',
                              { code: item.specCode }
                            )}
                          </Badge>
                          <Badge
                          ...
                          >
                            {t(
                              'engineering.cuttingPlan.alerts.invalidRecordId',
                              { id: item.specId }
                            )}
                          </Badge>
                        </div>
        ...
        <div className='text-[9px] font-black tracking-widest text-amber-700/70 uppercase'>
