        ...
        <div className='divide-y divide-dashed divide-muted/40'>
          {data.map((item) => (
            <div key={item.id} className='grid gap-3 px-4 py-3 md:grid-cols-[1.2fr_0.7fr_0.7fr_0.7fr_auto] md:items-center'>
              ...
              <div>
                <span className='inline-flex rounded-full border border-dashed border-muted/40 bg-muted/10 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.16em] text-muted-foreground/80'>
                  {item.isSystemPreset
                    ? t('engineering.masterData.weavingMode.badges.systemPreset')
              ...
              <div className='flex items-center gap-2 md:justify-end'>
                <Button variant='outline' size='sm' className='h-8 rounded-full px-3 text-[10px] font-black uppercase tracking-widest' onClick={() => onEdit(item)}>
                  <Pencil className='mr-1.5 size-3.5' />
                  {t('engineering.masterData.weavingMode.actions.edit')}
                </Button>
                <Button variant='ghost' size='sm' className='h-8 rounded-full px-3 text-[10px] font-black uppercase tracking-widest text-destructive' onClick={() => void onDelete(item)}>
