...
import type { SpokeLengthInput } from '../data/schema'
...
vi.mock('../services/nipple-service', () => ({
  nippleService: {
    getNipples: getNipplesMock,
  },
}))
...
import { SpokeLengthActionDialog } from './spoke-length-action-dialog'

describe('SpokeLengthActionDialog', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    getHubsMock.mockResolvedValue([])
    getNipplesMock.mockResolvedValue([])
    ...
    useDeltaTrackerMock.mockImplementation((initialData: SpokeLengthInput & { id?: string; createdAt?: string }) => ({
  ...
  it('uses engineering-db authority product options for parent product dropdown', () => {
    render(
