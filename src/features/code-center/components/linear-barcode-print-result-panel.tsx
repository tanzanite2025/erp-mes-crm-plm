    ...
    <div className='space-y-3 rounded-xl border border-dashed border-primary/20 bg-primary/5 px-3 py-3'>
      <div className='flex items-start justify-between gap-3'>
        <div>
          <div className='flex items-center gap-2'>
            <div className='text-[10px] font-black tracking-[0.2em] text-primary/60 uppercase'>
      ...
      <div className='flex flex-wrap items-center gap-2'>
        ...
        >
          {isRetryingFailedOnly
            ? t(
                'codeCenter.linearBarcode.print.sections.result.actions.retryingFailedOnly'
              )
            : t(
                'codeCenter.linearBarcode.print.sections.result.actions.retryFailedOnly'
              )}
        </Button>
        <Button
          type='button'
      ...
      <div className='grid gap-2 sm:grid-cols-2 xl:grid-cols-5'>
        <div className='rounded-lg border border-dashed bg-background/80 px-3 py-2'>
          <div className='text-[9px] font-black tracking-[0.18em] text-muted-foreground/50 uppercase'>
            {t(
              'codeCenter.linearBarcode.print.sections.result.summary.totalLines'
            )}
          </div>
          <div className='text-sm font-black text-foreground'>
            {batchPrintResult.totalLines}
          </div>
        </div>
        <div className='rounded-lg border border-dashed bg-background/80 px-3 py-2'>
          <div className='text-[9px] font-black tracking-[0.18em] text-muted-foreground/50 uppercase'>
            {t(
              'codeCenter.linearBarcode.print.sections.result.summary.printableLines'
            )}
          </div>
          <div className='text-sm font-black text-foreground'>
            {batchPrintResult.printableLines}
          </div>
        </div>
        <div className='rounded-lg border border-dashed bg-emerald-50/70 px-3 py-2'>
          <div className='text-[9px] font-black tracking-[0.18em] text-emerald-700/60 uppercase'>
            {t(
              'codeCenter.linearBarcode.print.sections.result.summary.successCount'
            )}
          </div>
          <div className='text-sm font-black text-emerald-700'>
            {batchPrintResult.successCount}
          </div>
        </div>
        <div className='rounded-lg border border-dashed bg-rose-50/70 px-3 py-2'>
          <div className='text-[9px] font-black tracking-[0.18em] text-rose-700/60 uppercase'>
            {t(
              'codeCenter.linearBarcode.print.sections.result.summary.failureCount'
            )}
          </div>
        ...
        <div className='rounded-lg border border-dashed bg-amber-50/70 px-3 py-2'>
          <div className='text-[9px] font-black tracking-[0.18em] text-amber-700/60 uppercase'>
            {t(
              'codeCenter.linearBarcode.print.sections.result.summary.skippedCount'
            )}
          </div>
          <div className='text-sm font-black text-amber-700'>
            {batchPrintResult.skippedCount}
          </div>
        </div>
      </div>
      <div className='space-y-2'>
        {filteredResultItems.length === 0 ? (
          <div className='rounded-lg border border-dashed bg-background/80 px-3 py-3 text-[10px] text-muted-foreground'>
            {t(
              'codeCenter.linearBarcode.print.sections.result.states.emptyFiltered'
            )}
          </div>
        ) : (
          filteredResultItems.map((item) => (
            <div
              key={`${item.key}-result`}
              className='rounded-lg border border-dashed bg-background/80 px-3 py-2'
            >
              <div className='mb-2 flex items-start justify-between gap-3'>
                <div>
                  <div className='text-[9px] font-black tracking-[0.18em] text-muted-foreground/50 uppercase'>
                    {t(
                      'codeCenter.linearBarcode.print.sections.result.fields.lineNo'
                    )}{' '}
                    #{item.lineNo}
                  </div>
                ...
                <div className='flex flex-col items-end gap-2'>
                  ...
                  >
                    {item.status === 'success'
                      ? t(
                          'codeCenter.linearBarcode.print.sections.result.status.success'
                        )
                      : item.status === 'failed'
                        ? t(
                            'codeCenter.linearBarcode.print.sections.result.status.failed'
                          )
                        : t(
                            'codeCenter.linearBarcode.print.sections.result.status.skipped'
                          )}
                  </Badge>
                  {item.status === 'failed' && (
                    <Button
                      ...
                      className='h-7 rounded-full px-3 text-[9px] font-black tracking-[0.18em] uppercase'
                    >
                      {retryingKeys[item.key]
                        ? t(
                            'codeCenter.linearBarcode.print.sections.result.actions.retryingItem'
                          )
                        : t(
                            'codeCenter.linearBarcode.print.sections.result.actions.retryItem'
                          )}
                    </Button>
                  )}
                </div>
              ...
              <div className='grid gap-1 text-[10px] text-muted-foreground'>
                <div>
                  {t(
                    'codeCenter.linearBarcode.print.sections.result.fields.message'
                  )}
                  : {item.message}
                </div>
                <div>
                  {t(
                    'codeCenter.linearBarcode.print.sections.result.fields.serial'
                  )}
                  : {item.serial}
                </div>
                <div>
                  {t(
                    'codeCenter.linearBarcode.print.sections.result.fields.barcodeSerial'
                  )}
                  : {item.barcodeSerial}
                </div>
              </div>
