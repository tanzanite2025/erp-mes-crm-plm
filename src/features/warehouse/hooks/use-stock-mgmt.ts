...
import {
  ...
  type StockMgmtReadyData,
} from '../utils/stock-mgmt-read-model'
...
export function useStockMgmt(feedback?: Pick<WarehouseUiFeedback, 'success'>) {
  ...
  const [hideZeroStockMap, setHideZeroStockMap] = useState<
  ...
  const {
    ...
    getReconcileSuccessMessage: () =>
      t('warehouse.stock.toast.reconcileSuccess'),
  ...
  const readResource = useMemo<StockMgmtReadResource>(() => {
      ...
      return {
        ...
        error:
          valuationQuery.error instanceof Error
            ...
            : new Error('[CRITICAL] Inventory valuation query failed'),
    ...
    const alertSummary = alertSummaryQuery.data as
  ...
  useEffect(() => {
    ...
    logger.error(
      `Failed to load stock management resources: ${readResource.scope}`,
      readResource.error
    )
  ...
  const groupedInventory =
    readResource.status === 'ready' ? readResource.groupedInventory : {}
  const materialTotalStock =
    readResource.status === 'ready' ? readResource.materialTotalStock : {}
  const totalAssetsValue =
    readResource.status === 'ready' ? readResource.totalAssetsValue : 0
  const materialThresholdMap =
    readResource.status === 'ready' ? readResource.materialThresholdMap : {}
  const categories =
    readResource.status === 'ready' ? readResource.categories : []
  const alertCount =
    readResource.status === 'ready' ? readResource.alertCount : 0
  const materialAlertCount =
    readResource.status === 'ready' ? readResource.materialAlertCount : 0
  const bomAlertCount =
    readResource.status === 'ready' ? readResource.bomAlertCount : 0
