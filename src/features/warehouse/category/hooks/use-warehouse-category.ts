...
export function useWarehouseCategory(
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      WarehouseCategoryMaintenanceService.deleteCategory(id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: warehouseQueryKeys.categoryList(),
      })
  ...
  const readResource = useMemo<WarehouseCategoryListResource>(() => {
    ...
    if (!categories) {
      return {
        status: 'error',
        error: new Error(
          '[CRITICAL] Warehouse category list missing after load'
        ),
        scope: 'useWarehouseCategory.categories',
      }
    }
  ...
  useEffect(() => {
    ...
    logger.error(
      `Failed to load warehouse category list: ${readResource.scope}`,
      readResource.error
    )
...
export function useWarehouseCategoryOptions() {
  return useQuery({
    queryKey: warehouseQueryKeys.categoryOptions(),
    queryFn: (): Promise<WarehouseCategoryOption[]> =>
