...
export function ApprovalHistory() {
  ...
  const getModuleKey = useCallback(
    (value: string): Parameters<typeof t>[0] => `approval.modules.${value.toLowerCase()}` as Parameters<typeof t>[0],
    []
  )
    ...
    <div className='flex flex-col gap-8 animate-in fade-in duration-700'>
      <div className='flex flex-col gap-1 rounded-[32px] border border-dashed border-muted/50 bg-muted/5 p-6'>
        ...
        <p className='text-[9px] font-black uppercase tracking-widest text-muted-foreground opacity-60'>
          {t('approval.history.heroSubtitle', { count: filteredHistory.length })}
        </p>
      </div>
        ...
        <Table>
          <TableHeader className='h-14 bg-muted/30'>
            <TableRow className='border-b border-dashed hover:bg-transparent'>
              <TableHead className='w-[150px] pl-6 text-[10px] font-black uppercase tracking-widest text-muted-foreground/50'>
                {t('approval.labels.module')}
              </TableHead>
              <TableHead className='w-[150px] text-[10px] font-black uppercase tracking-widest text-muted-foreground/50'>
                {t('approval.labels.action')}
              </TableHead>
              <TableHead className='w-[120px] text-[10px] font-black uppercase tracking-widest text-muted-foreground/50'>
              ...
              <TableHead className='pr-6 text-right text-[10px] font-black uppercase tracking-widest text-muted-foreground/50'>
            ...
            {loading ? (
              Array(5).fill(0).map((_, index) => (
                <TableRow key={index} className='h-16'>
                  ...
                  <TableCell className='pr-6'><Skeleton className='ml-auto h-4 w-24' /></TableCell>
                </TableRow>
              ))
              ...
              filteredHistory.map((req) => (
                <TableRow key={req.id} className='group h-16 border-b transition-colors last:border-0 hover:bg-muted/5'>
