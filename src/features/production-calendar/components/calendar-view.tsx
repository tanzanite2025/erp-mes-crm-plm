...
export function CalendarView({ onDateClick }: CalendarViewProps) {
        ...
        <div className='flex flex-col justify-between gap-2 sm:flex-row sm:items-center'>
          <div className='flex items-center gap-2 md:gap-2.5'>
            ...
            <div className='flex flex-col gap-0.5 overflow-hidden'>
              <h1 className='truncate text-sm font-black tracking-tighter text-primary uppercase italic md:text-base'>
                {format(currentMonth, 'MMMM yyyy', { locale: dateLocale })}
              </h1>
              <p className='truncate text-[8px] font-black tracking-widest text-muted-foreground uppercase opacity-60 md:text-[9px]'>
          ...
          <div className='flex items-center gap-1.5 md:gap-2'>
            <Button
              ...
              className='h-8 flex-1 rounded-full border-dashed border-primary/30 px-3 text-[8px] font-black tracking-widest text-primary uppercase transition-all hover:bg-primary/5 sm:flex-none md:px-4 md:text-[9px]'
            >
              {t('dashboard.page.calendar.view.today')}
            </Button>
            <div className='flex items-center rounded-full border border-dashed border-muted/50 bg-background p-0.5 shadow-sm'>
              <Button
                variant='ghost'
                size='icon'
    ...
    return (
      <div className='mb-1.5 grid grid-cols-7 px-1'>
        {days.map((day, i) => (
          <div
            key={i}
            className='text-center text-[8px] font-black tracking-[0.26em] text-muted-foreground/30 uppercase italic'
          >
            {day}
          </div>
        ))}
  ...
  const renderCells = () => {
    ...
    return (
      <div className='grid grid-cols-7 gap-1 rounded-xl border border-dashed border-muted/50 bg-muted/10 p-0.5 shadow-inner md:rounded-[20px]'>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                !isInMonth && 'pointer-events-none bg-muted/5 opacity-30',
                isInMonth &&
                  'hover:border-primary/20 hover:bg-white hover:shadow-lg',
                isToday && 'bg-primary/2 ring-2 ring-primary/20'
              )}
            ...
            >
              <div className='flex items-start justify-between'>
                <span
                  className={cn(
                    'flex size-6 items-center justify-center rounded-md text-[11px] font-black tracking-tighter italic tabular-nums transition-colors',
                    isToday
                ...
                {hasOutput && (
                  <Badge className='h-4 rounded-md border-none bg-emerald-500/10 px-1.5 py-0 text-[7px] font-black text-emerald-600 italic shadow-none'>
              ...
              <div className='mt-2 space-y-1'>
                {dayRecord?.items.slice(0, 2).map((item, idx) => (
                  <div
                    key={idx}
                    className='group/item flex flex-col gap-0.5 transition-all'
                  >
                    <div className='flex items-center gap-1'>
                      <div className='size-1 shrink-0 rounded-full bg-blue-500' />
                      <span className='truncate text-[9px] font-bold tracking-tight text-secondary uppercase'>
                        {item.name}
                      </span>
                    </div>
                    <div className='flex items-center gap-1.5 pl-2 font-mono text-[7px] opacity-40'>
                      <span>#{item.orderNo}</span>
                    </div>
                  </div>
                ))}
                {dayRecord && dayRecord.items.length > 2 && (
                  <p className='pt-0.5 pl-2 text-[7px] font-black tracking-widest text-muted-foreground/30 uppercase italic'>
                    + {dayRecord.items.length - 2}{' '}
                    {t('dashboard.page.calendar.view.moreNodes')}
                  </p>
                )}
              </div>
              ...
              {/* Task Hint */}
              {hasOutput && (
                <div className='absolute right-2 bottom-2 translate-y-1 transform opacity-0 transition-all group-hover:translate-y-0 group-hover:opacity-100'>
        ...
        <div className='flex-1'>
          {loadError ? (
            <div className='rounded-[16px] border border-dashed border-rose-200 bg-rose-50/70 p-4 text-center text-rose-700'>
              ...
              <p className='mt-2 text-xs font-bold wrap-break-word'>
                {loadError}
              </p>
            </div>
          ) : (
