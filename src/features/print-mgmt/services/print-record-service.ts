...
export const PrintRecordService = {
  ...
  async getNextSequence(key: string): Promise<{ value: number; sn: string }> {
    return apiFetch<{ value: number; sn: string }>(
      `/print-batches/next-sequence?key=${encodeURIComponent(key)}`
    )
  },
  ...
  async getPrintStats(): Promise<{
    ...
    return ensureObjectResponse<Record<string, unknown>>(
      res,
      'PrintRecordService.getPrintStats'
    ) as { totalActivated: number; totalScrapped: number }
  },
