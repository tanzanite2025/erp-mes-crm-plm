  ...
  return (
    <div className='grid gap-3 rounded-2xl border border-dashed border-border/60 bg-background/80 p-3 md:grid-cols-[160px_minmax(0,1fr)_auto] md:items-center'>
      <Select
        value={channel.type}
        onValueChange={(value) =>
          onTypeChange?.(index, value as ContactChannelType)
        }
      ...
      <div className='flex flex-wrap items-center gap-2'>
        {primaryControlMode === 'radio' ? (
          <RadioGroup
            value={isPrimarySelected ? 'primary' : ''}
            className='flex items-center justify-start'
          >
            <label className='flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/5 px-3 py-2 text-[11px] font-black tracking-wide text-foreground uppercase'>
              <RadioGroupItem
                value='primary'
                onClick={() => onPrimaryChange(index, true)}
              />
              {primaryLabel}
            </label>
          </RadioGroup>
        ) : (
          <label className='flex items-center gap-2 rounded-xl border border-border/60 bg-background px-3 py-2 text-[11px] font-black tracking-wide text-foreground uppercase'>
            <Checkbox
              checked={isPrimarySelected}
              onCheckedChange={(checked) =>
                onPrimaryChange(index, checked === true)
              }
            />
            {primaryLabel}
          </label>
        ...
        {showRemoveAction && onRemove ? (
          <Button
            type='button'
            variant='outline'
            className='h-10 rounded-xl px-4 text-[10px] font-black tracking-wider uppercase'
