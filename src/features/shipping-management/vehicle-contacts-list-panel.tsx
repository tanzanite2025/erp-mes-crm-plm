        ...
        {vehicleSpecsStatus === 'failed' && vehicleSpecsError ? (
          <div className='mb-4 rounded-2xl border border-destructive/30 bg-destructive/5 px-5 py-4 text-destructive'>
            ...
            <div className='mt-1.5 text-[11px] leading-5 text-muted-foreground'>
              {vehicleSpecsError.message}
            </div>
          </div>
        ) : null}
        <div className='grid gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:items-center'>
          <div className='space-y-4'>
            <div className='inline-flex rounded-full border border-border/60 bg-background px-3 py-1 text-[10px] font-black tracking-widest text-muted-foreground uppercase'>
              空状态引导
            </div>
            <div>
              <div className='text-[18px] font-black tracking-tighter text-foreground italic'>
                {emptyStateTitle}
              </div>
              <div className='mt-2 max-w-2xl text-[10px] leading-5 text-muted-foreground'>
                {emptyStateDescription}
              </div>
            </div>
            <div className='grid gap-3 sm:grid-cols-2'>
              <div className='rounded-2xl border border-border/60 bg-background/70 px-4 py-3 text-sm'>
                ...
                <div className='mt-1 text-[11px] leading-5 font-medium text-foreground'>
                  先确认车型库接口能正常返回车型
                </div>
              </div>
              <div className='rounded-2xl border border-border/60 bg-background/70 px-4 py-3 text-sm'>
            ...
            <div className='flex flex-wrap gap-2'>
              <Button
                type='button'
                size='sm'
                className='h-9 rounded-lg px-4 text-[10px] font-black tracking-widest uppercase shadow-none'
                onClick={onCreate}
                disabled={
                  vehicleSpecsLoading ||
              ...
              <Button
                ...
                variant='outline'
                size='sm'
                className='h-9 rounded-lg border-dashed px-4 text-[10px] font-black tracking-widest uppercase shadow-none'
                onClick={onGoToVehicleCatalog}
              >
              ...
              <div className='rounded-lg border border-border/50 bg-background px-4 py-3'>
                ...
                <span className='font-semibold text-foreground'>
                  {vehicleSpecsStatus === 'forbidden'
                    ...
                    : vehicleSpecsStatus === 'failed'
                      ? '接口失败'
                      : vehicleSpecsStatus === 'empty'
                        ? '空目录'
                        : vehicleSpecsStatus === 'loading'
                          ? '加载中'
                          : '正常'}
                </span>
              </div>
  ...
  return (
    <div className='space-y-3'>
      ...
      {bindings.map((item) => (
        <Card
          key={item.id}
          className='rounded-2xl border border-border/60 bg-background/90 p-4 shadow-[0_1px_0_rgba(15,23,42,0.04)]'
        >
          <div className='flex flex-wrap items-start justify-between gap-3'>
            <div className='flex min-w-0 flex-1 flex-col gap-1.5'>
              ...
              <div className='flex flex-wrap gap-2'>
                <Badge className='border border-primary/20 bg-primary/8 px-2 py-0 text-[10px] leading-none font-black tracking-widest text-primary uppercase'>
                  {CATEGORY_LABELS[item.category]}
                </Badge>
                <Badge
                  variant='outline'
                  className='border-dashed px-2 py-0 text-[10px] leading-none font-black tracking-widest uppercase'
                >
                  {item.enabled ? '启用' : '停用'}
                </Badge>
                {item.region ? (
            ...
            <div className='flex items-center gap-2'>
              <Button
                ...
                variant='outline'
                size='sm'
                className='h-9 rounded-lg border-dashed px-3 text-[10px] font-black tracking-widest uppercase shadow-none'
                onClick={() => onEdit(item)}
              >
                编辑
              </Button>
              ...
              <Button
                ...
                variant='outline'
                size='sm'
                className='h-9 rounded-lg border-dashed px-3 text-[10px] font-black tracking-widest text-destructive uppercase shadow-none'
              ...
              <div className='mt-2 space-y-1.5 text-sm leading-5 text-foreground'>
                {item.channels.map((channel) => (
                  ...
                  >
                    ...
                    {channel.primary ? (
                      <span className='uds-chip text-[10px] leading-none'>
                        主
                      </span>
                    ) : null}
                  </div>
