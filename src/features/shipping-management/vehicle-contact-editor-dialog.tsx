...
): VehicleContactBindingForm {
  const defaultChannels: ContactChannel[] = [
    { type: 'phone', value: '', primary: true },
  ]
  const channels = binding?.channels?.length
    ? binding.channels
    : defaultChannels
  const primaryPhone =
    channels.find((c) => c.type === 'phone' && c.primary)?.value ?? ''
...
const CHANNEL_TYPE_OPTIONS: ContactChannelType[] = [
  'phone',
  'wechat',
  'email',
  'whatsapp',
...
export function VehicleContactEditorDialog({
  open,
  binding,
  vehicleOptions,
  onOpenChange,
  ...
  const selectedVehicle = useMemo(
    () => vehicleOptions.find((item) => item.value === form.vehicleId),
    [form.vehicleId, vehicleOptions]
  )

  const primaryPhoneCount = form.channels.filter(
  ...
  const updateFormField = <K extends keyof VehicleContactBindingForm>(
    field: K,
    value: VehicleContactBindingForm[K]
  ) => {
    setForm((prev) => {
      const next = { ...prev }
      next[field] = value
  ...
  const getChannelRowUiState = (channel: ContactChannel) => {
    ...
    return {
      typeOptions: isPhoneChannel
        ? PHONE_CHANNEL_TYPE_OPTIONS
  ...
  const addChannel = () => {
    updateChannels((currentChannels) => [
      ...currentChannels,
      { type: 'other', value: '', primary: false },
    ])
  }
  ...
  const shellClasses = buildActionDialogShellClasses({
    content:
      'w-full max-w-3xl rounded-[28px] border-dashed p-0 shadow-2xl bg-background',
    header:
      'flex items-start justify-between gap-4 p-6 pb-4 border-b border-dashed border-border/60 bg-muted/10',
    title:
      'flex items-center gap-2 text-lg font-black normal-case not-italic tracking-normal',
    description:
      'mt-1 text-sm text-muted-foreground font-medium uppercase tracking-normal opacity-100',
    body: 'p-6',
    footer:
      'flex justify-end gap-2 p-6 pt-4 border-t border-dashed border-border/60 bg-muted/5',
  })
    ...
    >
      <div className='grid gap-4 md:grid-cols-2'>
        <div className='space-y-2'>
          <Label className='ml-1 text-[10px] font-black uppercase'>车型</Label>
          <Select
        ...
        <div className='space-y-2'>
          <Label className='ml-1 text-[10px] font-black uppercase'>区域</Label>
          <Input
            value={form.region}
        ...
        <div className='space-y-2'>
          ...
          <Select
            ...
            onValueChange={(value) =>
              updateFormField('enabled', value === 'true')
            }
          >
            <SelectTrigger className='h-10 w-full rounded-xl border border-input bg-background/50 text-xs font-black shadow-sm'>
      ...
      <div className='mt-5 rounded-2xl border border-dashed border-border/60 bg-muted/5 p-4'>
        ...
        <div className='mt-2 ml-1 text-[10px] font-bold tracking-wider uppercase opacity-60'>
          电话项必须且只能有一个主项；主电话会自动提取并用于列表显示。
        </div>
        {validationError ? (
          <div className='mt-3 rounded-xl border border-dashed border-destructive/40 bg-destructive/5 px-3 py-2 text-[11px] font-bold text-destructive'>
            {validationError}
          </div>
        ) : null}
      ...
      <div className='mt-5 grid gap-4 md:grid-cols-2'>
        <div className='space-y-2'>
          ...
          <textarea
            className='min-h-[96px] w-full rounded-xl border border-input bg-background/50 px-3 py-2 text-xs font-medium shadow-sm outline-none focus:ring-1 focus:ring-primary'
            value={form.dispatchAdvice}
            onChange={(e) => updateFormField('dispatchAdvice', e.target.value)}
          />
        </div>
        <div className='space-y-2'>
          <Label className='ml-1 text-[10px] font-black uppercase'>备注</Label>
          <textarea
            className='min-h-[96px] w-full rounded-xl border border-input bg-background/50 px-3 py-2 text-xs font-medium shadow-sm outline-none focus:ring-1 focus:ring-primary'
            value={form.note}
