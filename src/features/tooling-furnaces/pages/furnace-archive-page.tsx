...
import type { Furnace, FurnaceStatus } from '../data/furnace-schema'
import { useFurnaces } from '../hooks/use-furnaces'

export function FurnaceArchivePage() {
  const { t } = useLanguage()
  const { runConfirmedAction } = useConfirmedActionFlow()
  const { furnaceStats, furnaces, updateFurnace } = useFurnaces()
  ...
  const saveFurnace = (
    ...
    return updateFurnace(furnace, isPatch, delta)
  }
  ...
  const handleConfirm = (
    ...
    runConfirmedAction({
      ...
      onAction: async () => {
        ...
        toast.success(
          editingFurnace
            ? t('toolingFurnaces.toast.updated')
            : t('toolingFurnaces.toast.created')
        )
      },
    })
  }
  ...
  const getStatusInfo = (status: FurnaceStatus) => {
    switch (status) {
      case 'IDLE':
        return {
          label: t('toolingFurnaces.status.idle'),
          color: 'bg-slate-500/10 text-slate-600 border-slate-200',
        }
      case 'HEATING':
        return {
          label: t('toolingFurnaces.status.heating'),
          color: 'bg-slate-500/10 text-slate-500 border-slate-200',
        }
      case 'COOLING':
        return {
          label: t('toolingFurnaces.status.cooling'),
          color: 'bg-slate-500/10 text-slate-500 border-slate-200',
        }
      case 'MAINTENANCE':
        return {
          label: t('toolingFurnaces.status.maintenance'),
          color: 'bg-amber-500/10 text-amber-600 border-amber-200',
        }
      case 'FAULT':
        return {
          label: t('toolingFurnaces.status.fault'),
          color: 'bg-rose-500/10 text-rose-600 border-rose-200 animate-pulse',
        }
      default:
        return {
          label: t('toolingFurnaces.status.unknown'),
  ...
  return (
    <div className='flex animate-in flex-col gap-6 duration-700 fade-in md:gap-8'>
      <IndustrialHeader
        icon={Thermometer}
        title={t('toolingFurnaces.archive.title')}
        description={t('toolingFurnaces.archive.description')}
      ...
      <div className='flex flex-col items-stretch justify-between gap-4 rounded-[24px] border border-dashed bg-muted/5 p-4 sm:p-5 md:flex-row md:items-center'>
        <div className='relative w-full md:max-w-sm'>
          ...
          <Input
            placeholder={t('toolingFurnaces.archive.searchPlaceholder')}
                ...
                <div className='space-y-5'>
                  <div className='grid grid-cols-2 gap-y-3 border-t border-dashed border-muted-foreground/5 pt-2'>
                    ...
                    <div className='flex items-center gap-2 text-[9px] font-black tracking-widest text-muted-foreground/40 uppercase'>
                      ...
                      {t('toolingFurnaces.card.location')}
                    </div>
                    <div className='truncate text-right text-[10px] font-black text-slate-600'>
                      {furnace.location || t('toolingFurnaces.card.none')}
                    </div>
                  </div>
                  ...
                  <div className='space-y-2 border-t border-dashed border-muted-foreground/10 pt-4'>
                    ...
                    <p className='mt-1 text-center text-[7px] font-black tracking-[0.2em] text-muted-foreground/20 uppercase italic'>
                      {t('toolingFurnaces.card.sensorOffline')}
                    </p>
                  </div>
                  ...
                  <Button
                    asChild
                    variant='outline'
                    className='h-10 w-full rounded-full border-dashed text-[10px] font-black tracking-widest uppercase'
                  >
                    <Link
                      to='/tooling-furnaces/maintenance'
                      search={{ assetId: furnace.id }}
                    >
                      <Wrench className='mr-2 size-3.5' />
                      {t('toolingFurnaces.actions.viewMaintenance')}
                    </Link>
                  </Button>
                </div>
              </CardContent>
