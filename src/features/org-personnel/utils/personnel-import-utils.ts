...
type LocalizedPersonnelArchiveColumn = Omit<
  (typeof PERSONNEL_TEMPLATE_COLUMNS)[number],
  'header'
> & {
  header: string
}
...
function getLocalizedPersonnelColumns(
  locale: AppLocale
): LocalizedPersonnelArchiveColumn[] {
...
function getPossibleHeadersForKey(key: PersonnelArchiveColumnKey): string[] {
  return Array.from(
    new Set(
      [
        PERSONNEL_TEMPLATE_COLUMNS.find((column) => column.key === key)
          ?.header || '',
...
function normalizeStatus(value: unknown): Employee['status'] {
  const normalized = String(value || '')
    .trim()
    .toLowerCase()
  const zhActive = translate(
    'zh-CN',
    'orgPersonnel.excel.statuses.active'
  ).toLowerCase()
  const zhResigned = translate(
    'zh-CN',
    'orgPersonnel.excel.statuses.resigned'
  ).toLowerCase()
  const zhLeave = translate(
    'zh-CN',
    'orgPersonnel.excel.statuses.onLeave'
  ).toLowerCase()
  const enActive = translate(
    'en-US',
    'orgPersonnel.excel.statuses.active'
  ).toLowerCase()
  const enResigned = translate(
    'en-US',
    'orgPersonnel.excel.statuses.resigned'
  ).toLowerCase()
  const enLeave = translate(
    'en-US',
    'orgPersonnel.excel.statuses.onLeave'
  ).toLowerCase()

  if ([zhResigned, enResigned, 'resigned'].includes(normalized))
    return 'resigned'
  if ([zhLeave, enLeave, 'on leave', 'on-leave'].includes(normalized))
    return 'on-leave'
...
function normalizeGender(value: unknown): string {
  const normalized = String(value || '')
    .trim()
    .toLowerCase()
  const zhMale = translate(
    'zh-CN',
    'orgPersonnel.excel.gender.male'
  ).toLowerCase()
  const zhFemale = translate(
    'zh-CN',
    'orgPersonnel.excel.gender.female'
  ).toLowerCase()
  const enMale = translate(
    'en-US',
    'orgPersonnel.excel.gender.male'
  ).toLowerCase()
  const enFemale = translate(
    'en-US',
    'orgPersonnel.excel.gender.female'
  ).toLowerCase()

  if ([zhFemale, enFemale, 'female', 'f'].includes(normalized))
    return translate('zh-CN', 'orgPersonnel.excel.gender.female')
  if ([zhMale, enMale, 'male', 'm'].includes(normalized))
    return translate('zh-CN', 'orgPersonnel.excel.gender.male')
...
function normalizeEducation(value: unknown): string {
  ...
  const matchIndex = [...zh, ...en].findIndex(
...
function validatePersonnelSheetName(sheetName: string) {
  ...
  throw new Error(
    translate('zh-CN', 'orgPersonnel.excel.invalidSheetName', {
      sheetName,
...
export function validatePersonnelWorkbookStructure(
  ...
  const actualHeaderMap = new Map(
    actualHeaders.map((header) => [normalizePersonnelHeader(header), header])
  )

  const missingHeaders = PERSONNEL_TEMPLATE_COLUMNS.map((column) =>
    getPossibleHeadersForKey(column.key)
  )
    .filter(
      (headers) =>
        !headers.some((header) =>
          actualHeaderMap.has(normalizePersonnelHeader(header))
        )
    )
  ...
  if (missingHeaders.length > 0 || unexpectedHeaders.length > 0) {
    ...
    if (missingHeaders.length > 0) {
      problems.push(
        translate('zh-CN', 'orgPersonnel.excel.missingColumns', {
    ...
    if (unexpectedHeaders.length > 0) {
      problems.push(
        translate('zh-CN', 'orgPersonnel.excel.unexpectedColumns', {
          columns: unexpectedHeaders.join('、'),
        })
      )
    }

    throw new Error(
      translate('zh-CN', 'orgPersonnel.excel.structureFailed', {
...
export function mapExcelToEmployees(
      ...
      switch (column.key) {
        case 'staffId': {
          ...
          if (staffId && seenIds.has(staffId)) {
            errors.push(
              translate('zh-CN', 'orgPersonnel.excel.duplicateStaffId', {
                line: lineNumber,
                staffId,
              })
            )
  ...
  if (errors.length > 0) {
    throw new Error(
      translate('zh-CN', 'orgPersonnel.excel.validationFailed', {
...
function getLocalizedArchiveValue(
  ...
  if (columnKey === 'gender') {
    const genderLabels = getPersonnelGenderLabels(locale)
    if (
      String(value) === translate('zh-CN', 'orgPersonnel.excel.gender.female')
    )
      return genderLabels.female
    if (String(value) === translate('zh-CN', 'orgPersonnel.excel.gender.male'))
      return genderLabels.male
  }
...
export async function downloadPersonnelTemplate(locale: AppLocale = 'zh-CN') {
  ...
  const sheet = workbook.addWorksheet(
    translate(locale, 'orgPersonnel.excel.templateSheetName'),
    {
    ...
    localizedColumns.forEach((column, columnIndex) => {
      if (column.type === 'list' && column.options?.length) {
        const cell = sheet.getCell(
          `${getExcelColumnLetter(columnIndex)}${rowIndex}`
        )
        cell.dataValidation = {
          type: 'list',
          allowBlank: true,
...
export async function exportPersonnelData(
  ...
  const workbook = new ExcelJS.Workbook()
  const sheet = workbook.addWorksheet(
    translate(locale, 'orgPersonnel.excel.exportSheetName'),
    {
      views: [{ state: 'frozen', ySplit: 1 }],
    }
  )
  ...
  configurePersonnelWorksheet(
    sheet,
    Math.max(data.length + 1, PERSONNEL_EDITABLE_ROW_COUNT)
  )
