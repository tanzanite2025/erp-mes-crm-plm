...
export function useProcessLibraryProcesses() {
  ...
  const saveProcess = async (step: ProductionProcessStep) => {
    ...
    try {
      ...
      toast.success(isUpdate ? 'Process updated' : 'Process created')
      return saved
    } catch (error) {
      toast.error(isUpdate ? 'Failed to update process' : 'Failed to create process')
  ...
  const deleteProcess = async (process: ProductionProcessStep) => {
    try {
      ...
      toast.success(`Deleted process ${process.name}`)
    } catch (error) {
