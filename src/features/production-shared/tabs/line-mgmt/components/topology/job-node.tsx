    ...
    return (
      <div className='group/job-category relative flex h-auto min-h-0 w-full flex-col items-start rounded-[24px] border border-dashed border-muted/30 bg-background/85 shadow-none transition-all hover:border-cyan-500/20 hover:bg-cyan-500/5'>
        <div className='flex w-full flex-col justify-start p-4'>
          <div className='mb-2 flex w-full items-center gap-2 text-sm font-black tracking-tighter text-foreground uppercase'>
            ...
            <span className='shrink-0 pl-1 text-[9px] font-black tracking-[0.24em] text-cyan-700/40'>
              [{level2Name}]
            </span>

            {isEditing ? (
              <div className='flex flex-1 animate-in items-center gap-1 duration-200 zoom-in-95 fade-in'>
                <Input
                  value={editValue}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  type='button'
                  className='flex size-7 shrink-0 items-center justify-center rounded-full text-muted-foreground opacity-0 transition-opacity group-hover/job-category:opacity-100 hover:bg-muted/40 hover:text-foreground'
                >
              ...
              <DropdownMenuContent
                align='end'
                className='rounded-2xl border border-dashed border-muted/40 bg-background/95 p-1 shadow-xl backdrop-blur-md'
              >
                <DropdownMenuItem
                  onClick={() => {
                  ...
                  className='cursor-pointer gap-2 rounded-xl px-3 py-2 text-[10px] font-bold tracking-[0.2em] uppercase'
                >
                  <ShieldCheck className='size-3.5 text-cyan-600' />
                  {t('orgPersonnel.lineMgmt.topology.renameLevel', {
                ...
                <DropdownMenuItem
                  ...
                  className='cursor-pointer gap-2 rounded-xl px-3 py-2 text-[10px] font-bold tracking-[0.2em] text-rose-500 uppercase focus:text-rose-600'
                >
                  <X className='size-3.5' />
                  {t('orgPersonnel.lineMgmt.topology.removeLevel', {
                    levelName: level2Name,
                  })}
                </DropdownMenuItem>
              </DropdownMenuContent>
        ...
        <SecurityAuthDialog
          ...
          title={
            pendingAction === 'rename'
              ? t('orgPersonnel.lineMgmt.topology.levelRenameTitle', {
                  levelName: level2Name,
                })
              : t('orgPersonnel.lineMgmt.topology.levelRemoveTitle', {
                  levelName: level2Name,
                })
          }
          description={
            pendingAction === 'rename'
              ? t('orgPersonnel.lineMgmt.topology.levelRenameDesc', {
                  levelName: level2Name,
                })
              : t('orgPersonnel.lineMgmt.topology.levelRemoveDesc', {
                  levelName: level2Name,
                })
          }
        />
