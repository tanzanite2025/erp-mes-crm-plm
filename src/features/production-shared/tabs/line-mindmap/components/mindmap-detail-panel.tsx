  ...
  return (
    <Card className='rounded-[24px] border border-dashed border-muted/40 bg-background/90 shadow-none'>
      ...
      <CardContent className='space-y-5 p-5'>
        {selectedNode ? (
          <>
            <MindmapDetailSummary selectedNode={selectedNode} levelNames={levelNames} />
          ...
          <div className='space-y-4'>
            <div className='rounded-[20px] border border-dashed border-muted/40 bg-muted/5 px-4 py-4'>
              ...
              <p className='mt-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground/55'>
                {readonlyMode ? '请选择一条产线并点击左侧节点查看真实详情。' : '请先使用脑图上方工具条新增节点，或点击左侧已有节点进入编辑态。'}
              </p>
            </div>

            {readonlyMode ? null : (
              <div className='rounded-[20px] border border-dashed border-muted/40 bg-muted/5 px-4 py-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground/55'>
