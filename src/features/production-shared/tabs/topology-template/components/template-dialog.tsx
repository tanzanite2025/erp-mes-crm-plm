...
export function TemplateDialog({
  ...
  const formSchema = z.object({
    name: z
      .string()
      .min(2, t('orgPersonnel.topologyTemplateMgmt.dialog.nameRequired')),
        ...
        <DialogHeader className='space-y-4'>
          <div className='flex items-center gap-2 text-primary'>
            <DialogTitle className='text-lg font-black tracking-tighter uppercase italic'>
              {template
                ? t('orgPersonnel.topologyTemplateMgmt.dialog.editTitle')
                : t('orgPersonnel.topologyTemplateMgmt.dialog.createTitle')}
            </DialogTitle>
          </div>
          <DialogDescription className='text-[9px] font-black tracking-widest uppercase opacity-60'>
            {t('orgPersonnel.topologyTemplateMgmt.dialog.descriptionDynamic', {
              level1Name,
              level2Name,
            })}
          </DialogDescription>
        ...
        <Form {...form}>
          ...
          >
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder={t(
                        'orgPersonnel.topologyTemplateMgmt.dialog.namePlaceholder'
                      )}
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      placeholder={t(
                        'orgPersonnel.topologyTemplateMgmt.dialog.descPlaceholder'
                      )}
                      className='min-h-[100px] resize-none rounded-2xl border-none bg-muted/50 font-medium transition-all focus-visible:ring-1 focus-visible:ring-primary/20'
                      {...field}
                    />
                  </FormControl>
                  <FormMessage className='text-[10px] font-bold' />
            ...
            <DialogFooter className='pt-4'>
              <Button
                type='submit'
                className='h-11 rounded-full px-10 text-[10px] font-black tracking-widest uppercase shadow-xl shadow-blue-500/20 transition-all active:scale-95'
              >
                {template
                  ? t('orgPersonnel.topologyTemplateMgmt.dialog.saveChanges')
                  : t('orgPersonnel.topologyTemplateMgmt.dialog.initTemplate')}
              </Button>
            </DialogFooter>
          </form>
        </Form>
