...
export function useVehicleLoadingSourcePackageInput(args: {
  ...
  const selectedProfile = useMemo(() => {
    ...
    return activeProfiles.find((profile) => profile.id === selectedPackagingProfileId) ?? activeProfiles[0] ?? null
    ...
    try {
      if (source === 'packing-rule') {
        ...
        if (profilesQuery.error instanceof Error) {
          return { packageInput: null, error: profilesQuery.error, enabled: false }
        }
      ...
      if (source === 'api') {
        return {
          packageInput: buildVehicleLoadingPackageInputFromApiDraft(apiPackageDraft, sourceLabel),
          error: null,
          enabled: true,
        }
      }
      ...
      return {
        packageInput: buildManualVehicleLoadingPackageInput(summary, sourceLabel),
        error: null,
        enabled: true,
      }
    } catch (error) {
      return {
        packageInput: null,
        error: error instanceof Error ? error : new Error('Failed to build vehicle loading package input'),
        enabled: false,
      }
    }
  ...
  return {
    ...
    isLoadingPackageInput: source === 'packing-rule' ? (profilesQuery.isLoading || isLoadingUnits) : false,
