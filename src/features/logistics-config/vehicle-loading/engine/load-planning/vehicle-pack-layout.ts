...
export function calculateLoadPlanForOrientation(
  ...
  const boxesAlongLength = Math.floor(
    vehicle.usableInnerSize.lengthMm / orientation.lengthMm
  )
  const boxesAlongWidth = Math.floor(
    vehicle.usableInnerSize.widthMm / orientation.widthMm
  )
  const layers = Math.floor(
    vehicle.usableInnerSize.heightMm / orientation.heightMm
  )

  const boxesPerLayer = boxesAlongLength * boxesAlongWidth
  const maxBoxesByGeometry = boxesPerLayer * layers
  const maxBoxesByWeight =
    unitWeightKg > 0 ? Math.floor(vehicle.payloadKg / unitWeightKg) : 0
  const maxBoxesPerVehicle = Math.min(
    packageCount,
    maxBoxesByGeometry,
    maxBoxesByWeight
  )
  ...
  const boxVolumeM3 =
    (orientation.lengthMm * orientation.widthMm * orientation.heightMm) /
    1_000_000_000
  ...
  return {
    ...
    maxBoxesPerVehicle,
    volumeUtilization:
      vehicle.volumeM3 > 0 ? Math.min(loadedVolumeM3 / vehicle.volumeM3, 1) : 0,
    weightUtilization:
      vehicle.payloadKg > 0
        ? Math.min((maxBoxesPerVehicle * unitWeightKg) / vehicle.payloadKg, 1)
        : 0,
    loadingReason: [
