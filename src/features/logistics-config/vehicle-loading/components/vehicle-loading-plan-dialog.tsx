...
export function VehicleLoadingPlanDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='flex w-[calc(100vw-1rem)] max-w-[1200px] sm:max-w-[1200px] flex-col gap-0 rounded-[28px] border-dashed bg-background/95 p-0 shadow-xl max-h-[calc(100vh-1rem)]'>
        <DialogHeader className='shrink-0 border-b border-dashed border-border/60 px-3 py-2 sm:px-4 sm:py-3'>
          <DialogTitle className='text-sm font-black tracking-tight'>装载示意</DialogTitle>
        </DialogHeader>
            ...
            <div className='space-y-3 rounded-[22px] border border-dashed border-border/60 bg-muted/3 p-3 sm:p-4'>
              <div>
                ...
                <div className='mt-1 text-[11px] leading-relaxed text-muted-foreground'>
                  当前方案按 <span className='font-black text-foreground'>{orientationLabel}</span> 朝向摆放。
                </div>
              </div>
              ...
              <div className='space-y-2'>
                {explanation.map((item) => (
                  <div key={item} className='rounded-2xl border border-dashed border-primary/20 bg-primary/5 px-3 py-2 text-[11px] leading-relaxed text-primary/80'>
                    {item}
                  </div>
                ))}
              </div>
