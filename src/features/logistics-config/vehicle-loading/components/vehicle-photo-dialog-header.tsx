...
export function VehiclePhotoDialogHeader({ vehicle, photoEntry }: Props) {
  ...
  return (
    <DialogHeader className='shrink-0 border-b border-dashed border-border/60 px-4 py-3 sm:px-5'>
      <DialogTitle className='text-[15px] font-black tracking-tight leading-none'>
        {photoEntry?.displayTitle ?? vehicle?.name ?? t('logisticsConfig.vehiclePhotos.dialogTitle')}
      </DialogTitle>
      <DialogDescription className='mt-1 text-[12px] leading-5 text-muted-foreground'>
        {photoEntry?.description ?? t('logisticsConfig.vehiclePhotos.dialogDescription')}
      </DialogDescription>
    </DialogHeader>
  )
}
