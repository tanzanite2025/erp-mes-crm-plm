        ...
        <div className='grid grid-cols-1 gap-3 md:grid-cols-3'>
          <div className='space-y-2'>
            <Label className={labelClass}>
              {t('logisticsConfig.vehicleLoading.filters.category')}
            </Label>
            <Select
              value={category}
              onValueChange={(value) =>
            ...
            >
              <SelectTrigger className={fieldClass}>
                <SelectValue
                  placeholder={t(
                    'logisticsConfig.vehicleLoading.filters.categoryPlaceholder'
                  )}
                />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value='all'>
                  {t('logisticsConfig.vehicleLoading.filters.all')}
                </SelectItem>
                <SelectItem value='van'>
                  {t('logisticsConfig.vehicleLoading.filters.van')}
                </SelectItem>
                <SelectItem value='boxTruck'>
                  {t('logisticsConfig.vehicleLoading.filters.boxTruck')}
                </SelectItem>
                <SelectItem value='lightTruck'>
                  {t('logisticsConfig.vehicleLoading.filters.lightTruck')}
                </SelectItem>
                <SelectItem value='mediumTruck'>
                  {t('logisticsConfig.vehicleLoading.filters.mediumTruck')}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className='space-y-2'>
            <Label className={labelClass}>
              {t('logisticsConfig.vehicleLoading.filters.minVolume')}
            </Label>
            <Input
              className={fieldClass}
              inputMode='decimal'
              value={minVolumeM3}
              onChange={(event) => onMinVolumeM3Change(event.target.value)}
            />
          </div>
          <div className='space-y-2'>
            <Label className={labelClass}>
              {t('logisticsConfig.vehicleLoading.filters.minPayload')}
            </Label>
            <Input
              className={fieldClass}
              inputMode='decimal'
              value={minPayloadKg}
              onChange={(event) => onMinPayloadKgChange(event.target.value)}
            />
          </div>
        </div>
      </div>
