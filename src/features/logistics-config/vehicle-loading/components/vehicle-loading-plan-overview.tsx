...
export function VehicleLoadingPlanOverview({
  ...
  const highestLoadRate = recommendations.reduce(
    (max, item) => Math.max(max, item.loadRateWeight, item.loadRateVolume),
    0
  )
    ...
    <Card className='rounded-[28px] border-dashed bg-background/80 shadow-none'>
      <CardHeader className='space-y-2'>
        <div className='flex items-center justify-between gap-3'>
          ...
          <Badge className='border-none bg-primary/10 text-primary'>
            {recommendations.length > 0 ? 'READY' : 'NO DATA'}
          </Badge>
        </div>
      </CardHeader>
      <CardContent className='grid grid-cols-2 gap-3 md:grid-cols-4'>
        <PlanOverviewRecommendationCard
          ...
          detail={
            best
              ? `${best.vehiclesNeeded} 辆 · 体积 ${formatPercent(best.loadRateVolume)} · 重量 ${formatPercent(best.loadRateWeight)}`
              : '请调整货量或筛选条件后再试算'
          }
          emptyText='暂无推荐'
        />
        ...
        <PlanOverviewRiskCard
          riskText={
            riskTexts.length > 0 ? riskTexts.join(' · ') : '当前未发现明显风险'
          }
          detail={
            recommendations.length > 0
              ? `最高利用率 ${formatPercent(highestLoadRate)}，请结合实际线路和装载规则复核。`
              : '暂无可执行推荐时请调整货量或车型筛选条件。'
          }
        />

        <PlanOverviewSectionCard className='col-span-2 border-primary/20 bg-primary/5'>
          ...
          <div className='mt-2 text-[11px] leading-relaxed text-primary/80'>
            箱数 {summary.boxes} · 体积 {summary.totalVolumeM3.toFixed(2)} m³ ·
            重量 {summary.totalWeightKg.toFixed(0)} kg
          </div>
        ...
        <PlanOverviewMetricCard
          label='最高利用率'
          value={formatPercent(highestLoadRate)}
        />
