...
export function useLabExperimentalEquipment(categoryId?: string) {
  return useQuery<Equipment[]>({
    ...
    queryFn: () =>
      apiFetch(`/labs/experimental/equipment?categoryId=${categoryId || ''}`),
  })
}

// --- 实验项目与报告 ---

export function useLabExperimentalTasks(
  page: number,
  pageSize: number,
  type?: string
) {
  return useQuery<LabExperimentalTaskQueryResult>({
    ...
    queryFn: () =>
      apiFetch(
        `/labs/experimental/tasks?page=${page}&pageSize=${pageSize}&type=${type || 'ALL'}`
      ),
  })
}
...
export function useLabExperimentalMutations() {
  ...
  const saveCategoryMutation = useMutation({
    mutationFn: (data: Partial<EquipmentCategory>) =>
      apiFetch('/labs/experimental/categories', {
        method: 'POST',
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: (id: string) =>
      apiFetch(`/labs/experimental/categories/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['exp_categories'] })
  ...
  const saveEquipmentMutation = useMutation({
    mutationFn: (data: Partial<Equipment>) =>
      apiFetch('/labs/experimental/equipment', {
        method: 'POST',
        body: JSON.stringify(data),
      }),
    onSuccess: () => {
  ...
  const deleteEquipmentMutation = useMutation({
    mutationFn: (id: string) =>
      apiFetch(`/labs/experimental/equipment/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['exp_equipment'] })
      toast.success(
        t('labExperimental.toasts.equipmentDeleted') || '设备已删除'
      )
    },
  })

  const saveTaskMutation = useMutation({
    mutationFn: (data: LabExperimentalTaskMutationInput) =>
      apiFetch('/labs/experimental/tasks', {
        method: 'POST',
        body: JSON.stringify(data),
      }),
    onSuccess: () => {
  ...
  const saveReportMutation = useMutation({
    mutationFn: (data: LabExperimentalReportMutationInput) =>
      apiFetch('/labs/experimental/reports', {
        method: 'POST',
        body: JSON.stringify(data),
      }),
    onSuccess: () => {
...
export function buildLabExperimentalCategoryTree(
