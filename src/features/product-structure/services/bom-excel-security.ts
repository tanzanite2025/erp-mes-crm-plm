...
export const unescapeFormula = (val: string): string => {
  if (!val) return val
  if (val.length > 1 && val[0] === "'" && ['=', '+', '-', '@'].includes(val[1])) {
    return val.slice(1)
  }
  return val
}
...
export const validateBOMFileSize = (file: File) => {
  if (file.size > BOM_EXCEL_LIMITS.maxFileSize) {
    throw new Error(`文件体积过大 (${(file.size / 1024 / 1024).toFixed(2)}MB)，上限为 10MB。请检查是否上传了错误的超大附件。`)
  }
}

export const validateBOMWorkbookSheetCount = (workbook: Workbook) => {
  if (workbook.worksheets.length > BOM_EXCEL_LIMITS.maxSheets) {
    throw new Error(`工作簿包含过多的 Sheet (${workbook.worksheets.length} 个)，存在解析风险。请使用标准模板。`)
  }
}
...
export const safelyGetCellValue = (cell: Cell): string => {
  ...
  if (typeof val === 'object') {
    if ('result' in val) return unescapeFormula(val.result?.toString().trim() || '')
    if ('richText' in val && Array.isArray(val.richText)) {
      return unescapeFormula(val.richText.map((t: { text: string }) => t.text).join('').trim())
    }
    if ('text' in val) return unescapeFormula((val as { text?: string }).text?.toString().trim() || '')
