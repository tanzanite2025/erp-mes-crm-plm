...
import { type BOMEditDetailSourceResource } from '../hooks/use-bom-edit-detail-source'
...
interface BOMDialogResourceBoundaryProps {
  ...
  detailResource?: BOMEditDetailSourceResource
  children: ReactNode
}
...
export function BOMDialogResourceBoundary({ resource, detailResource, children }: BOMDialogResourceBoundaryProps) {
  ...
  if (resource.status === 'loading' || detailResource?.status === 'loading') {
    return (
