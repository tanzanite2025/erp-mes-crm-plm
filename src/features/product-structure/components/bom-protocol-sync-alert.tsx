...
export function BOMProtocolSyncAlert({ validation, needsSync }: BOMProtocolSyncAlertProps) {
  ...
  if (errors.length > 0) {
    return (
      <Alert variant="destructive" className="mb-2 py-2">
        ...
        <AlertTitle className="text-xs font-semibold mb-1">树结构数据不一致</AlertTitle>
  ...
  if (warnings.length > 0) {
    return (
      <Alert variant="default" className="mb-2 py-2 border-yellow-500/50 bg-yellow-50/50 dark:bg-yellow-950/10">
        <AlertTriangle className="h-3 w-3 text-yellow-600 dark:text-yellow-500" />
        <AlertTitle className="text-xs font-semibold mb-1 text-yellow-900 dark:text-yellow-100">
          树结构已同步
        </AlertTitle>
  ...
  if (needsSync) {
    return (
      <Alert variant="default" className="mb-2 py-2 bg-blue-50/50 dark:bg-blue-950/10 border-blue-500/30">
        <Info className="h-3 w-3 text-blue-600 dark:text-blue-400" />
        <AlertTitle className="text-xs font-semibold mb-0 text-blue-900 dark:text-blue-100">
          树结构已自动同步
        </AlertTitle>
      </Alert>
    )
