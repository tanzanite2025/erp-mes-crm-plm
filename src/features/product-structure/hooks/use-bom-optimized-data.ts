 ...
 *     console.log('Committed changes:', delta);
 *   },
 * });
 * ```
 */
export function useBOMOptimizedData<T extends BOMRowData>({
  ...
  const proxyManager = useRef(new BOMProxyManager<T>(
    dirtyMarker.current,
    rowIdExtractor
  ));
    ...
    try {
      ...
      for (const rowId of dirtyRowIds) {
        ...
        if (!currentRow || !baselineRow) {
          console.warn(`[useBOMData] Row not found for ID: ${rowId}`);
          continue;
        }
  ...
  // Get Proxy for a row
  const getRowProxy = useCallback((rowId: string): T | undefined => {
  ...
  const releaseRowProxy = useCallback((rowId: string): void => {
