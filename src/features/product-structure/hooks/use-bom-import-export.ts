...
export function useBOMImportExport({
  products,
  productDisplayLabelMap,
  sections,
}: UseBOMImportExportParams) {
  const { t } = useLanguage()
  const queryClient = useQueryClient()

  const downloadTemplate = async () => {
    const loadingId = toast.loading(
      t('engineering.bomArchive.toasts.downloadLoading')
    )
    ...
    try {
      ...
      if (!loadedMaterials) {
        const error = new Error(
          '[CRITICAL] Material options missing during BOM template export'
        )
        failLoudly(error, 'useBOMImportExport.downloadTemplate')
        throw error
      }

      await ExcelService.generateBOMTemplate(
        loadedMaterials,
        products,
        sections,
        productDisplayLabelMap
      )
      toast.success(t('engineering.bomArchive.toasts.downloadSuccess'), {
        id: loadingId,
      })
    } catch (error) {
      logger.error('Template generation error', error)
      toast.error(t('engineering.bomArchive.toasts.downloadFailed'), {
        id: loadingId,
      })
    }
  }
  ...
  const parseExcel = async (file: File) => {
    const loadingId = toast.loading(
      t('engineering.bomArchive.toasts.parseLoading')
    )

    try {
      const { items: parsedItems, productId: parsedProductId } =
        await ExcelService.parseBOMExcel(file, sections)

      if (parsedItems.length === 0) {
        toast.error(t('engineering.bomArchive.toasts.parseNoValid'), {
          id: loadingId,
        })
        return null
      }
      ...
      if (!latestMaterials) {
        const error = new Error(
          '[CRITICAL] Material options missing during BOM import'
        )
        failLoudly(error, 'useBOMImportExport.parseExcel')
        throw error
      }

      const { items: processedItems, errors } =
        ExcelService.normalizeParsedBOMItems({
          parsedItems,
      ...
      toast.error(t('engineering.bomArchive.toasts.parseFailed'), {
        id: loadingId,
      })
      return null
    }
