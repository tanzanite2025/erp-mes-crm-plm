...
export type ProtocolRecoveryStrategy = 'rebuild' | 'filter' | 'ignore' | 'manual'
...
interface UseBOMProtocolRecoveryParams {
  ...
  onRecoverySuccess?: (recoveredProtocol: BOMWorkspaceParentChildrenProtocolDraft) => void
...
function filterInvalidReferences(
  ...
  const filteredBranchNodes = protocolDraft.branchNodes.filter((branchNode) => {
    if (!validSectionCodes.has(branchNode.sectionCode)) {
      console.warn(`[Protocol Recovery] Filtering branch node with invalid section: ${branchNode.id}`)
      return false
    }
    return true
  })
  ...
  const filteredItemNodes = protocolDraft.itemNodes.filter((itemNode) => {
    ...
    if (!validSectionCodes.has(itemNode.sectionCode)) {
      console.warn(`[Protocol Recovery] Filtering item node with invalid section: ${itemNode.id}`)
      return false
    }
    
    // Check parent exists
    if (itemNode.parentId && !validBranchIds.has(itemNode.parentId) && itemNode.parentId !== 'root') {
      console.warn(`[Protocol Recovery] Filtering item node with invalid parent: ${itemNode.id}`)
      return false
    }
    ...
    if (isItemIdBased && itemId && !validItemIds.has(itemId)) {
      console.warn(`[Protocol Recovery] Filtering item node with deleted item: ${itemNode.id}`)
      return false
    }
    
    if (isFieldIdBased && parsed && 'fieldId' in parsed) {
      if (!validFieldIds.has(parsed.fieldId)) {
        console.warn(`[Protocol Recovery] Filtering item node with invalid field: ${itemNode.id}`)
        return false
      }
    }
  ...
  const updatedBranchNodes = filteredBranchNodes.map((branchNode) => ({
    ...branchNode,
    children: branchNode.children.filter((childId) => 
      validBranchIds.has(childId) || validItemNodeIds.has(childId)
    ),
  }))
...
export function useBOMProtocolRecovery({
  ...
  const [recoveredProtocol, setRecoveredProtocol] = useState<BOMWorkspaceParentChildrenProtocolDraft>()
  ...
  const attemptRecovery = useCallback(async (strategy: ProtocolRecoveryStrategy): Promise<boolean> => {
    ...
    try {
      ...
      switch (strategy) {
        case 'rebuild':
          ...
          recovered = buildBOMWorkspaceParentChildrenProtocolDraftFromBOMDetailSource({
            sourceBOM,
  ...
  }, [sourceBOM, protocolDraft, sections, fields, watchedItems, onRecoverySuccess, onRecoveryFailed])
