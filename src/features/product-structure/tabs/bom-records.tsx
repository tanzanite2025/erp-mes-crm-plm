...
export function BOMRecordsTab() {
  ...
  const productOptions = useMemo(
    () =>
      [...(readyResource?.products ?? [])]
        .map((product) => ({
          id: product.id,
          label: resolveProductLabel(
            product,
            readyResource?.productDisplayLabelMap ?? new Map<string, string>()
          ),
        }))
  ...
  const selectedBomLabel = useMemo(
    () =>
      bomOptions.find((option) => option.id === selectedBomId)?.bomNo ||
      readyResource?.data.find((bom) => bom.id === selectedBomId)?.bomNo ||
      '',
    [bomOptions, readyResource, selectedBomId]
  )

  const hasActiveFilters = Boolean(
    selectedProductId || selectedBomId || createdFrom || createdTo
  )
  ...
  const handleProductChange = (value: string) => {
    ...
    const matchingBoms = filterBOMsByProductId(
      readyResource?.data ?? [],
      nextProductId
    )
        ...
        <div className='flex flex-col gap-4'>
          ...
          <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1.2fr)_220px_220px]'>
            <div className='rounded-[24px] border border-dashed bg-background/80 p-4'>
              ...
              <Select
                value={selectedProductId || ALL_FILTER_VALUE}
                onValueChange={handleProductChange}
              >
                <SelectTrigger
                  aria-label='按产品筛选'
                  className='mt-3 h-12 w-full rounded-2xl border-none bg-muted/50 px-4'
                >
            ...
            <div className='rounded-[24px] border border-dashed bg-background/80 p-4'>
              ...
              <Select
                value={selectedBomId || ALL_FILTER_VALUE}
                onValueChange={(value) =>
                  setSelectedBomId(value === ALL_FILTER_VALUE ? '' : value)
                }
              >
                <SelectTrigger
                  aria-label='按BOM筛选'
          ...
          {readResource.status === 'error' ? (
            <div className='rounded-[24px] border border-dashed border-amber-400/40 bg-amber-500/10 px-4 py-3 text-[10px] font-black tracking-widest text-amber-700 uppercase'>
              筛选基准读取失败，当前仍可继续使用追溯工作区，但产品与 BOM
              下拉可能不完整。
            </div>
          ) : null}
        </div>
