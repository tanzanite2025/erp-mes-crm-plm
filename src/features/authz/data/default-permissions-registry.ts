...
export function collectDefaultPermissions(): Permission[] {
  ...
  ;[...getRouteDerivedPermissions(), ...ACTION_PERMISSIONS].forEach(
    (permission) => {
  ...
  return Array.from(byId.values()).sort((a, b) => {
    const categoryOrder =
      permissionCategoryWeight[a.category] -
      permissionCategoryWeight[b.category]
    if (categoryOrder !== 0) return categoryOrder
    return a.id.localeCompare(b.id)
  })
}

export function validateDefaultPermissionsContract(
  permissions: Permission[]
): void {
  assertBackendPermissionContract(
