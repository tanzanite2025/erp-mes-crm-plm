...
export const XDFC_EFFECTIVE_PERMISSIONS_EVENT = 'xdfc_effective_permissions_updated'
...
export async function processAndNotifyPermissions(permissionIds: string[]): Promise<string[]> {
...
export async function resolveOrSyncPermissionIds(_roleIds: string[]): Promise<string[]> {
  const state = useAuthStore.getState()
  const hasAccessToken = !!state.accessToken
  const isIdentitySynced = state.isIdentitySynced
  const currentPermissions = toUniquePermissionIds(state.user?.permissions || [])
...
export async function resolveStoredPermissionSet(roleIds: string[]): Promise<Set<string>> {
