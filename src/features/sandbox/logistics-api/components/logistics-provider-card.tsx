...
export function LogisticsProviderCard({
  ...
  const hasCredentials =
    Boolean(provider.appKey?.trim()) && Boolean(provider.appSecret?.trim())
  ...
  return (
    <Card className='group relative overflow-hidden rounded-[32px] border-none bg-white shadow-sm transition-all hover:shadow-md'>
      <div
        className={`absolute top-0 left-0 h-full w-1.5 ${hasCredentials ? 'bg-emerald-500' : 'animate-pulse bg-amber-400'}`}
      />
      <CardHeader className='space-y-3 pb-1'>
        <div className='flex items-start justify-between gap-3'>
          <div className='flex min-w-0 items-start gap-3'>
            <div
              className={`flex size-10 shrink-0 items-center justify-center rounded-2xl shadow-inner ${hasCredentials ? 'bg-slate-50' : 'bg-amber-50'}`}
            >
              {hasCredentials ? (
            ...
            <div className='min-w-0 space-y-2'>
              <CardTitle className='text-base font-black tracking-tighter uppercase italic'>
                {provider.name}
              </CardTitle>
              <CardDescription className='flex flex-wrap items-center gap-1.5'>
                <Badge
                  variant='outline'
                  className='h-4 border-slate-200 px-1.5 text-[8px] font-black tracking-tighter uppercase italic'
                >
                  {provider.code}
                </Badge>
                <span
                  className={getProviderLifecycleBadgeClass(provider.status)}
                >
                  {t(getProviderLifecycleLabelKey(provider.status))}
                </span>
                <span
                  className={`${headerStatusBadgeClass} ${getProviderVerificationBadgeClass(getProviderVerificationStatus(provider))}`}
                >
                  {t(
                    getProviderVerificationLabelKey(
                      getProviderVerificationStatus(provider)
                    )
                  )}
                </span>
                <span
                  className={`${headerStatusBadgeClass} ${getProviderApiConnectionBadgeClass(isProviderApiConnected(provider))}`}
                >
                  {t(
                    getProviderApiConnectionLabelKey(
                      isProviderApiConnected(provider)
                    )
                  )}
                </span>
                {(provider.referenceCount || 0) > 0 ? (
                  <span
                    className={`${headerStatusBadgeClass} ${getProviderReferenceBadgeClass(true)}`}
                  >
                ...
                {!hasCredentials ? (
                  <span
                    className={`${headerStatusBadgeClass} bg-amber-100/50 text-amber-600`}
                  >
      ...
      <CardContent className='space-y-3.5 pt-1'>
        <div className='space-y-2.5 rounded-[24px] border border-dashed border-slate-200 p-3'>
            ...
            <div className='space-y-0.5'>
              ...
              <span>
                {provider.website?.trim() ||
                  t('logisticsConfig.providerShared.states.unset')}
              </span>
            </div>
            <div className='space-y-0.5'>
              <span className='font-black text-slate-500'>
          ...
          <div className='rounded-2xl border border-dashed border-slate-100 bg-slate-50/60 px-3 py-2.5 text-[10px] leading-relaxed text-slate-500'>
            {provider.note?.trim() ||
              t('logisticsConfig.platforms.states.noteEmpty')}
          </div>
        </div>
        ...
        <div className='space-y-2.5 rounded-[24px] border border-dashed border-primary/20 bg-primary/5 p-3'>
          ...
          <div className='grid grid-cols-2 gap-3 rounded-2xl border border-slate-100 bg-white/80 p-3'>
            <div className='space-y-1'>
              ...
              {provider.appKey?.trim() ? (
                <div className='flex items-center gap-2'>
                  <span className='max-w-[140px] truncate font-mono text-[10px] font-bold text-slate-600'>
            ...
            <div className='space-y-1'>
              ...
              <span className='block truncate font-mono text-[9px] font-bold text-slate-500'>
                {provider.endpoint ||
                  t('logisticsConfig.providerShared.states.notConfigured')}
              </span>
            </div>
        ...
        <div className='flex flex-wrap gap-2 pt-0.5'>
          <Button
            ...
            className='h-9 rounded-full px-4 text-[10px] font-black tracking-widest uppercase'
            onClick={onVerify}
            disabled={!provider.id || verifyPending}
          >
            {verifyPending ? (
          ...
          <Button
            ...
            className='h-9 rounded-full px-4 text-[10px] font-black tracking-widest text-rose-500 uppercase hover:bg-rose-50 hover:text-rose-600'
            onClick={onDelete}
          >
