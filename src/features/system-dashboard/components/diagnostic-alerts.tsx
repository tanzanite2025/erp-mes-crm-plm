...
function getLogDurationSeconds(log: AlertDiagnosticLog) {
  ...
  const endsAt = log.endsAt
    ? new Date(log.endsAt).getTime()
        ...
        <CardTitle className='flex items-center justify-between text-sm font-black tracking-tighter uppercase italic'>
          <div className='flex items-center gap-2'>
            <ShieldAlert
              className={cn(
                'size-4',
                hasAlerts ? 'animate-pulse text-rose-500' : 'text-emerald-600'
              )}
            />
          ...
          {hasAlerts && (
            <Badge
              variant='destructive'
              className='rounded-full px-2 text-[8px] font-black uppercase'
            >
              {t('systemManagement.diagnostic.activeBadge', {
          ...
          <>
            {activeAlerts.length > 0 && (
              <div className='space-y-1'>
                <div className='text-[10px] font-black tracking-widest text-rose-600 uppercase dark:text-rose-300'>
                  {t('systemManagement.diagnostic.activeAlerts')}
                </div>
                {activeAlerts.map((alert) => (
                  <div
                    key={alert.fingerprint}
                    className='flex animate-in items-start gap-1.5 rounded-2xl border border-dashed border-rose-200 bg-background p-1.5 shadow-sm duration-300 slide-in-from-right-2 sm:p-2 dark:border-rose-500/25 dark:bg-white/4'
                  >
                    <div className='flex size-5.5 shrink-0 items-center justify-center rounded-xl bg-rose-100 sm:size-6 dark:bg-rose-500/15'>
                      <AlertCircle className='size-2.5 text-rose-600 sm:size-3' />
                    </div>
                    <div className='flex-1 space-y-0.5'>
                      <div className='flex items-center justify-between gap-1.5'>
                        <span className='text-xs font-black tracking-tight text-slate-800 uppercase italic dark:text-slate-100'>
                          {alert.name}
                        </span>
                        <div className='flex items-center gap-1.5 font-mono text-[9px] text-rose-500 dark:text-rose-300'>
                          <Clock className='size-3' />
                          {t(
                            'systemManagement.diagnostic.durationPrefix'
                          )}: {formatDuration(getActiveDurationSeconds(alert))}
                        </div>
                      </div>
                      <p className='text-[11px] leading-tight font-medium text-slate-600 dark:text-slate-300'>
                        {alert.description}
                      </p>
                    </div>
                  </div>
            ...
            <div className='space-y-1'>
              <div className='flex items-center gap-2 text-[10px] font-black tracking-widest text-rose-600 uppercase dark:text-rose-300'>
                <History className='size-3.5' />
                {t('systemManagement.diagnostic.last24Hours')}
              </div>

              {logs.length === 0 ? (
                <div className='rounded-2xl border border-dashed border-rose-200/60 bg-background p-1.5 text-[11px] text-slate-500 sm:p-2 dark:border-rose-500/20 dark:bg-white/4 dark:text-slate-400'>
                ...
                logs.map((log) => (
                  <div
                    key={log.id}
                    className='rounded-2xl border border-dashed border-rose-200 bg-rose-50/70 p-1.5 sm:p-2 dark:border-rose-500/25 dark:bg-rose-500/10'
                  >
                    <div className='mb-0.5 flex items-center justify-between gap-2'>
                      <span className='text-[11px] font-black tracking-tight text-rose-700 uppercase italic dark:text-rose-300'>
                        {log.name}
                      </span>
