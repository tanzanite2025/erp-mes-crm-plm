...
export function normalizeQualityStandardType(
  type?: string | null
): QualityStandardNormalizedType {
...
export function normalizeQualityStandardStatus(
  status?: string | null
): QualityStandardNormalizedStatus {
...
export function getQualityStandardStatusLabel(
  ...
  if (normalized === 'PENDING_APPROVAL')
    return t('quality.standards.values.statusPendingApproval')
  if (normalized === 'APPROVED')
    return t('quality.standards.values.statusApproved')
  if (normalized === 'REJECTED')
    return t('quality.standards.values.statusRejected')
...
export function getQualityAuditMeta(
  locale: string,
  status?: string | null,
  auditor?: string | null
): QualityAuditMeta {
  ...
  if (normalizedStatus === 'ARCHIVED') {
    return {
      ...
      note:
        locale === 'zh-CN'
          ? '该标准已归档，审签链路以归档版本为准。'
          : 'This standard is archived and follows the archived review trail.',
      className: 'bg-slate-500/5 border border-slate-500/10 text-slate-500',
      dotClassName: 'bg-slate-500',
    }
  }
  ...
  if (normalizedStatus === 'PUBLISHED' && hasAuditor) {
    return {
      ...
      note:
        locale === 'zh-CN'
          ? '已发布且审核信息完整，可按正式受控标准追溯。'
          : 'Published with complete reviewer information for controlled traceability.',
      className:
        'bg-emerald-500/5 border border-emerald-500/10 text-emerald-600',
  ...
  if (normalizedStatus === 'PUBLISHED' && !hasAuditor) {
    return {
      ...
      note:
        locale === 'zh-CN'
          ? '当前标准已发布，但审核人信息未补齐，建议尽快补录。'
          : 'The standard is published, but reviewer information is still missing.',
      className: 'bg-rose-500/5 border border-rose-500/10 text-rose-600',
      dotClassName: 'bg-rose-500',
    }
  }
  ...
  if (normalizedStatus === 'APPROVED') {
    return {
      ...
      note:
        locale === 'zh-CN'
          ? '审核信息已存在，但标准尚未发布。'
          : 'Reviewer information exists, but the standard has not been released yet.',
      className: 'bg-blue-500/5 border border-blue-500/10 text-blue-600',
      dotClassName: 'bg-blue-500',
    }
  }
  ...
  if (normalizedStatus === 'REJECTED') {
    return {
      ...
      note:
        locale === 'zh-CN'
          ? '当前标准已被驳回，请修订后重新提交审批。'
          : 'The standard was rejected and must be revised before resubmission.',
      className: 'bg-rose-500/5 border border-rose-500/10 text-rose-600',
      dotClassName: 'bg-rose-500',
    }
  }
  ...
  if (normalizedStatus === 'DRAFT') {
    return {
      ...
      note:
        locale === 'zh-CN'
          ? '当前标准仍为草稿，尚未进入审批链路。'
          : 'The standard is still a draft and has not entered the approval workflow yet.',
      className: 'bg-slate-500/5 border border-slate-500/10 text-slate-600',
      dotClassName: 'bg-slate-500',
    }
  }
  ...
  return {
    ...
    note:
      locale === 'zh-CN'
        ? '当前标准仍处于待审核阶段，尚未形成完整审签链。'
        : 'The standard is still pending review and does not have a complete review trail yet.',
    className: 'bg-amber-500/5 border border-amber-500/10 text-amber-600',
    dotClassName: 'bg-amber-500',
  }
}
