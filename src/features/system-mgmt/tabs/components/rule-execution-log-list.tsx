    ...
    <>
            ...
            <div className='flex flex-col gap-1 lg:flex-row lg:items-start lg:justify-between'>
              ...
              <div className='text-right text-xs text-muted-foreground'>
                ...
                {log.executionStatus === 'failed' && log.eventKey ? (
                  <p className='text-[8px] font-mono leading-tight'>{log.eventKey}</p>
                ) : null}
              </div>
            </div>
      ...
      <div className='flex flex-col gap-0.5 pt-0 lg:flex-row lg:items-center lg:justify-between'>
        ...
        {hasMore ? (
          <button
            type='button'
            className='inline-flex h-9 items-center justify-center rounded-full border border-dashed border-muted-foreground/20 bg-muted/30 px-3.5 text-[10px] font-black uppercase tracking-widest text-foreground transition hover:bg-muted/50 disabled:cursor-not-allowed disabled:opacity-50'
            disabled={isFetching}
            onClick={onLoadMore}
          >
            查看更多
