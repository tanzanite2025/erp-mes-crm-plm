...
import { BusinessEventSourceChangePanel } from './business-event-source-card-change-panel'
import {
...
export function BusinessEventSourceCard({
  ...
  const [savingSections, setSavingSections] = useState<SavingState>(
    createBusinessEventSourceSavingState
  )
  const [undoingSections, setUndoingSections] = useState<SavingState>(
    createBusinessEventSourceSavingState
  )
  const [undoPatches, setUndoPatches] = useState<UndoPatchState>(
    createBusinessEventSourceUndoPatchState
  )
  ...
  const referencingRules = useMemo(
    () => rules.filter((rule) => rule.sourceCode === committedSource.code),
    ...
    <>
      ...
      >
        ...
        <CollapsibleContent className='overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down'>
          ...
          <Tabs defaultValue='actions' className='px-6 pt-2 pb-6'>
            ...
            <TabsContent value='actions' className='mt-0'>
              <BusinessEventSourceActionSection
                ...
                onAdd={() =>
                  applyDraft((prev) => appendBusinessEventAction(prev))
                }
                onSave={() => void saveSection('actions')}
                onUndo={
                  undoPatches.actions
                    ? () => void undoSection('actions')
                    : undefined
                }
            ...
            <TabsContent value='statuses' className='mt-0'>
              <BusinessEventSourceStatusSection
                ...
                onUndo={
                  undoPatches.statuses
                    ? () => void undoSection('statuses')
                    : undefined
                }
            ...
            <TabsContent value='resolvers' className='mt-0'>
              <BusinessEventSourceResolverSection
                ...
                onDelete={(index) =>
                  applyDraft((prev) =>
                    removeBusinessDynamicResolverAt(prev, index)
                  )
                }
              />
            </TabsContent>
            ...
            <TabsContent value='base' className='mt-0'>
              <BusinessEventSourceBaseSection
                ...
                changeSummary={getBusinessEventSourceGeneralDiffSummary(
                  diff.general
                )}
          ...
          <BusinessEventSourceEditorOverlay
            ...
            onAddField={() =>
              applyDraft((prev) => appendBusinessEventField(prev))
            }
            onUpdateField={(index, updates) =>
              applyDraft((prev) =>
                updateBusinessEventFieldAt(prev, index, updates)
              )
            }
            onDeleteField={(index) =>
