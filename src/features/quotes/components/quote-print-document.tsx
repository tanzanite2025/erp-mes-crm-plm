    ...
    <div className='quote-print-sheet mx-auto w-full bg-white text-[12px] leading-6 text-black'>
      ...
      <div className='border-b-2 border-black pb-4'>
        <div className='flex items-start justify-between gap-4'>
          <div>
            <div className='text-[10px] font-semibold tracking-[0.35em] text-black/55 uppercase'>
              Quote Document / Customer Copy
            </div>
            <div className='mt-1 text-[15px] leading-5 font-bold tracking-[0.12em]'>
              报价单
            </div>
            <div className='mt-1 text-[11px] text-black/65'>
              适用于对客沟通、打印留档与 PDF 交付。
            </div>
          </div>
          <div className='flex-1 pt-2 text-center'>
            <h1 className='text-2xl font-bold tracking-[0.22em]'>QUOTATION</h1>
            <p className='mt-1 text-[11px] tracking-[0.18em] text-black/55'>
              正式报价 / FOR CUSTOMER REVIEW
            </p>
          </div>
      ...
      <section className='mt-6'>
        <div className='border-b border-black pb-2'>
          <h2 className='text-base font-bold tracking-[0.08em]'>
            报价基础信息
          </h2>
        </div>
        <div className='mt-4 grid grid-cols-2 gap-x-8 gap-y-3 text-sm'>
          <div>
            <span className='font-bold'>报价名称：</span>
            {detail.orderName || '--'}
          </div>
          <div>
            <span className='font-bold'>客户编号：</span>
            {detail.customerId || '--'}
          </div>
          <div>
            <span className='font-bold'>报价类型：</span>
            {detail.type || '--'}
          </div>
          <div>
            <span className='font-bold'>报价状态：</span>
            {detail.status || '--'}
          </div>
          <div>
            <span className='font-bold'>下单日期：</span>
            {detail.orderDate || '--'}
          </div>
          <div>
            <span className='font-bold'>交期：</span>
            {detail.deliveryDate || '--'}
          </div>
          <div>
          ...
          <div>
            <span className='font-bold'>账期：</span>
        ...
        <div className='quote-print-avoid-break mt-4 grid gap-4 lg:grid-cols-[1fr_1fr]'>
          <div className='border border-black p-4'>
            <div className='grid grid-cols-3 gap-4 text-sm'>
              <div>
                <div className='text-[11px] tracking-[0.18em] text-black/55 uppercase'>
                  报价金额
                </div>
      ...
      <section className='mt-8'>
        <div className='border-b border-black pb-2'>
          ...
          <p className='mt-1 text-[10px] leading-5 text-black/70'>
            以下明细用于客户确认产品型号、规格、数量、价格与备注说明，打印版式适配
            A4 纵向输出。
          </p>
        </div>
          ...
          <tbody>
            {detail.lines.map((line) => (
              <tr key={line.id} className='quote-print-avoid-break'>
                ...
                <td className='border border-black p-2 align-top font-mono text-[11px]'>
                  {line.productCode || '--'}
                </td>
                ...
                <td className='border border-black p-2 text-right align-top font-semibold'>
                  {line.amount}
                </td>
                <td className='border border-black p-2 align-top'>
                  {line.note || '--'}
        ...
        <div className='mt-10 border-t border-dashed border-black/40 pt-3 text-[10px] leading-5 text-black/65'>
          <div className='flex items-start justify-between gap-4'>
            <span className='flex-1'>
              本单据为系统生成的报价打印件，金额、明细与客户联系方式以系统当前记录为准。
            </span>
