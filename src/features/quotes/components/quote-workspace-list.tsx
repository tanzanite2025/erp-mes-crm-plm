...
export function QuoteWorkspaceList({
  ...
  return (
    <Card className='overflow-hidden rounded-[24px] border-dashed border-muted/50 bg-muted/5 p-1 shadow-inner'>
      <div className='overflow-hidden rounded-[20px] bg-background/60'>
        <div className='flex flex-col gap-3 border-b border-dashed border-border/60 px-5 py-4 md:flex-row md:items-center md:justify-between md:px-6'>
          <div className='space-y-2'>
            <div className='text-base font-black tracking-tight italic'>
              报价单列表
            </div>
            <div className='flex flex-wrap items-center gap-2 text-[11px] font-bold text-muted-foreground'>
              ...
              <Badge
                variant='outline'
                className='rounded-full px-2 py-0 text-[10px] font-black'
              >
                {resultsLabel}
              </Badge>
            </div>
          ...
          <div className='flex items-center gap-2'>
            <Button
              className='h-9 rounded-2xl px-3 text-xs font-black'
              size='sm'
              variant='outline'
            >
        ...
        <Table>
          <TableHeader className='bg-muted/40'>
            <TableRow className='border-none hover:bg-transparent'>
              <TableHead className='h-10 px-5 py-0 text-[10px] font-black tracking-widest text-muted-foreground/40 uppercase'>
                报价编号
              </TableHead>
              <TableHead className='h-10 py-0 text-[10px] font-black tracking-widest text-muted-foreground/40 uppercase'>
                客户
              </TableHead>
              <TableHead className='h-10 py-0 text-[10px] font-black tracking-widest text-muted-foreground/40 uppercase'>
                类型
              ...
              <TableHead className='h-10 py-0 pr-5 text-right text-[10px] font-black tracking-widest text-muted-foreground/40 uppercase'>
                操作
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading ? (
              <TableRow>
                <TableCell
                  colSpan={7}
                  className='h-24 text-center text-[10px] font-black tracking-widest text-muted-foreground/40 uppercase'
                >
                  正在加载报价列表
                </TableCell>
              </TableRow>
              ...
              rows.map((row) => (
                ...
                >
                  <TableCell className='px-5 py-3 text-xs font-bold'>
                    {row.id}
                  </TableCell>
              ...
              <TableRow>
                <TableCell colSpan={7} className='h-28 px-6 text-center'>
                  <div className='flex flex-col items-center gap-3'>
                    ...
                    <Button
                      size='sm'
                      className='h-9 rounded-2xl px-4 text-xs font-black'
