...
function requireNonEmptyString(
  ...
  if (typeof value !== 'string' || value.trim() === '') {
    throw new Error(
      `[INVALID_RESPONSE] ${context} expected ${field} to be a non-empty string.`
    )
  }
  return value.trim()
}
...
function requireNumber(value: unknown, context: string, field: string): number {
  if (typeof value !== 'number' || !Number.isFinite(value)) {
    throw new Error(
      `[INVALID_RESPONSE] ${context} expected ${field} to be a finite number.`
    )
  }
  return value
}
...
function normalizeCustomerSegment(
  ...
  if (typeof value !== 'string') {
    throw new Error(
      `[INVALID_RESPONSE] ${context} expected customerSegment to be a string.`
    )
  }
  ...
  throw new Error(
    `[INVALID_RESPONSE] ${context} expected customerSegment to be one of vip | long-term | new.`
  )
}
...
function normalizeQuoteType(value: unknown, context: string): QuoteSummaryType {
  if (typeof value !== 'string') {
    throw new Error(
      `[INVALID_RESPONSE] ${context} expected quoteType to be a string.`
    )
  }
  const normalized = value
    ...
    .toLowerCase()
    .replace(/[\s_-]/g, '')
  if (['wholesale', 'bulk', 'dealer', 'outsource', 'toll'].includes(normalized))
    return 'wholesale'
  if (
    ['sample', 'sam', 'smp', 'rd', 'rnd', 'r&d', 'trial'].includes(normalized)
  )
    return 'sample'
  if (
    [
    ...
    return 'retail'
  throw new Error(
    `[INVALID_RESPONSE] ${context} expected quoteType to be one of retail | wholesale | sample.`
  )
}
...
function normalizeStatus(value: unknown, context: string): QuoteSummaryStatus {
  if (typeof value !== 'string') {
    throw new Error(
      `[INVALID_RESPONSE] ${context} expected status to be a string.`
    )
  }
  const normalized = value.trim().toLowerCase()
  ...
  throw new Error(
    `[INVALID_RESPONSE] ${context} expected status to be one of draft | pending | converted | voided.`
  )
}
...
function formatAmountLabel(
  ...
  if (typeof amountLabel === 'string' && amountLabel.trim())
    return amountLabel.trim()
  if (typeof value === 'number' && Number.isFinite(value))
    return `¥ ${value.toFixed(2)}`
  if (typeof value === 'string' && value.trim()) return value.trim()
  throw new Error(
    `[INVALID_RESPONSE] ${context} expected amount or amountLabel to be present.`
  )
}

export function toQuoteSummaryContract(
  dto: QuoteListItemApiDTO,
  index: number
): QuoteSummary {
  const context = `QuoteApiAdapter.toQuoteSummaryContract[${index}]`
  const quoteNo = requireNonEmptyString(
    dto.quoteNo ?? dto.code,
    context,
    'quoteNo'
  )
  ...
  return {
    ...
    quoteNo,
    customerName: requireNonEmptyString(
      dto.customerName ?? dto.customer,
      context,
      'customerName'
    ),
    customerSegment: normalizeCustomerSegment(
      dto.customerSegment ?? dto.segment,
      context
    ),
    ...
    itemCount:
      ...
      (() => {
        throw new Error(
          `[INVALID_RESPONSE] ${context} expected itemCount or lineCount to be present.`
        )
      })(),
    ownerName: optionalDisplayName(dto.ownerName ?? dto.owner),
    productSummary: requireNonEmptyString(
      dto.productSummary ?? dto.summary,
      context,
      'productSummary'
    ),
  }
...
export function toQuoteSummaryContracts(
...
export function toQuoteDetailContract(dto: QuoteDetailApiDTO): QuoteDetail {
  ...
  return {
    ...
    quoteNo,
    orderName: firstNonEmptyString(
      [dto.orderName, quoteNo],
      context,
      'orderName'
    ),
    customerName,
    ...
    customerSegment: normalizeCustomerSegment(
      firstNonEmptyString(
        [dto.customerSegment, 'new'],
        context,
        'customerSegment'
      ),
      context
    ),
    type: normalizeQuoteType(
      firstNonEmptyString([dto.type, 'retail'], context, 'type'),
      context
    ),
    status: normalizeStatus(
      firstNonEmptyString([dto.status, 'draft'], context, 'status'),
      context
    ),
    currency: firstNonEmptyString([dto.currency, 'CNY'], context, 'currency'),
    amountLabel: firstNonEmptyString(
      [dto.amountLabel, 'CNY 0.00'],
      context,
      'amountLabel'
    ),
    quantityLabel: firstNonEmptyString(
      [dto.quantityLabel, '0.00'],
      context,
      'quantityLabel'
    ),
    ...
    lines: lines.map((line, index) => {
      ...
      return {
        id: firstNonEmptyString(
          [String(line.id ?? ''), `line-${index + 1}`],
          lineContext,
          'id'
        ),
        lineNo: requireNumber(line.lineNo, lineContext, 'lineNo'),
        productModel: firstNonEmptyString(
          [line.productModel, line.productCode, 'Unnamed product'],
          lineContext,
          'productModel'
        ),
