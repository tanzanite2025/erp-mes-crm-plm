export const orders = {
  title: 'Purchase Order Processing',
  description: 'Strategic procurement, staged delivery monitoring, and supplier performance reconciliation',
  tabOrders: 'Order List',
  tabLogs: 'Statistics & Logs',
  tabReturns: 'Pre-Inbound Returns',
  searchPlaceholder: 'Search order no. / supplier...',
  addOrder: 'Create Purchase Order',
  all: 'All',
  filters: {
    paymentMethod: 'Payment Method',
    paymentTerm: 'Payment Term',
    allPaymentMethods: 'All Payment Methods',
    allPaymentTerms: 'All Payment Terms',
  },
  loading: 'Synchronizing purchase order data...',
  prevPage: 'Prev Page',
  nextPage: 'Next Page',
  page: 'Page',
  detailTitle: 'Purchase Order Detail',
  deleteConfirm: 'Void this purchase order?',
  statusDraft: 'Draft',
  statusSent: 'Ordered',
  statusAwaiting: 'Awaiting Receipt',
  statusReceived: 'Received',
  statusCanceled: 'Canceled',
  masterEmpty: 'No purchase orders found',
  buyer: 'Buyer',
  systemBuyer: 'System',
  overdueWarning: 'Overdue Alert',
  notSet: 'Not set',
  baseCurrency: 'Base: CNY',
  editOrder: 'Edit Order',
  workflowLocked: 'Locked by workflow',
  removePermanently: 'Remove Permanently',
  voidContract: 'Void Contract',
  mobileDate: 'Date',
  mobileRate: 'Rate',
  mobileTotal: 'Total',
  columns: {
    orderStatus: 'Order No. / Status',
    supplierChannel: 'Supplier Channel',
    orderDate: 'Order Date',
    expectedArrival: 'Expected Arrival',
    exchangeRate: 'Exchange Rate',
    paymentMethod: 'Payment Method',
    paymentTerm: 'Payment Term',
    contractAmount: 'Contract Amount',
    actions: 'Actions',
  },
  detailLoading: 'Fetching purchase line details...',
  detailNoSelection: 'Please select a purchase order',
  detailItemsTitle: 'Item Details',
  detailQty: 'Qty',
  detailPrice: 'Price',
  detailAmount: 'Amount',
  detailReceivedQty: 'Received / Ordered',
  detailSideTitle: 'Logistics & Supplier',
  detailDelete: 'Void This Purchase Order',
  detailStats: {
    orderNo: 'Order No.',
    status: 'Status',
    totalAmount: 'Total Amount',
    expectedArrival: 'Expected',
  },
  detailFields: {
    supplier: 'Supplier',
    purchaser: 'Purchaser',
    orderDate: 'Order Date',
    paymentMethod: 'Payment Method',
    paymentMethodUnset: 'Not set',
    paymentTerm: 'Payment Term',
    paymentTermUnset: 'Not set',
  },
  dialogEditTitle: 'Edit Purchase Order',
  dialogCreateTitle: 'Create Material Purchase Contract',
  dialogDescription: 'End-to-end procurement workflow maintenance',
  dialogSyncing: 'Syncing order lines and master data...',
  dialogTotal: 'Estimated Total',
  dialogCancel: 'Cancel',
  dialogSave: 'Save & Sync',
  toasts: {
    returnCreated: 'Purchase return recorded',
  },
  returns: {
    title: 'Pre-Inbound Purchase Returns',
    description: 'Register supplier returns for arrived goods before warehouse inbound so the remaining receivable quantity stays accurate.',
    loading: 'Loading purchase return records...',
    eligibleOrders: 'Eligible Orders',
    pendingLines: 'Pending Return Lines',
    pendingQty: 'Pending Qty',
    availableOrders: 'Orders Awaiting Return Handling',
    availableOrdersDescription: 'Select a purchase order first, then process the rejected quantities line by line.',
    selectedOrder: 'Selected Purchase Order',
    draftSummary: 'Return Draft Summary',
    selectedLines: 'Selected Lines',
    estimatedAmount: 'Estimated Amount',
    issueCategory: 'Issue Category',
    issueCategoryPlaceholder: 'Select an issue category',
    fillAll: 'Fill All',
    clearAll: 'Clear All',
    fillRemaining: 'Fill Remaining',
    clearLine: 'Clear',
    recentRecords: 'Recent Return Records',
    recentRecordsDescription: 'Recent pre-inbound returns, prioritizing the currently selected purchase order.',
    historySearchPlaceholder: 'Search history by purchase order no.',
    viewCurrentOrderHistory: 'Current Order History',
    viewAllHistory: 'All History',
    historyAction: 'View History',
    printAction: 'Print Return Note',
    operator: 'Operator',
    createAction: 'Register Return',
    createTitle: 'Register Purchase Return',
    createDescription: 'Only quantities that have not yet been inbound can be returned here. Inventory will not be affected.',
    order: 'Purchase Order',
    selectOrder: 'Select an eligible purchase order',
    date: 'Return Date',
    reason: 'Return Reason',
    reasonPlaceholder: 'Example: appearance defect / spec mismatch / packaging damage',
    remarks: 'Remarks',
    remarksPlaceholder: 'Optional notes for supplier communication or follow-up handling',
    evidenceTitle: 'Return Photos',
    evidenceHint: 'Take photos on mobile or upload images. They will be printed together with the return note.',
    evidenceEmpty: 'No return photos yet',
    lineEvidenceTitle: 'Line Issue Photos',
    lineEvidenceHint: 'Attach defect, packaging, or quantity issue photos for this line if needed.',
    lineEvidenceEmpty: 'No photos for this line yet',
    cameraAction: 'Take Photo',
    uploadAction: 'Upload Images',
    evidenceLimitReached: 'You can upload up to {{max}} images',
    evidenceUploadFailed: 'Image upload failed. Please try again.',
    photoNote: 'Photo Note',
    photoNotePlaceholder: 'Example: damaged outer carton, blurred batch label',
    photoLocation: 'Photo Location',
    photoLocationPlaceholder: 'Example: front side of carton, left pallet edge',
    photoDefectPart: 'Defect Area',
    photoDefectPartPlaceholder: 'Example: carton corner, label area, material edge',
    lineReason: 'Line Reason',
    lineReasonPlaceholder: 'Optional line-level issue note',
    remainingQty: 'Remaining Receivable',
    alreadyReturned: 'Returned',
    returnQty: 'Return Qty',
    submit: 'Confirm Return',
    totalRecords: 'Return Records',
    totalQty: 'Returned Qty',
    totalAmount: 'Returned Amount',
    empty: 'No purchase return records',
    emptyDescription: 'If goods are rejected during arrival inspection before inbound, register the returned quantity here.',
    noEligibleOrders: 'There are currently no purchase orders eligible for pre-inbound return.',
    tip: 'This action only reduces the purchase order receivable balance. It does not create negative inventory or an inventory return transaction.',
  },
  headerFields: {
    orderNo: 'Order No.',
    supplier: 'Supplier',
    supplierPlaceholder: 'Select supplier',
    currency: 'Currency',
    currencyPlaceholder: 'Select currency',
    exchangeRate: 'Settlement Rate',
    exchangeRateAuto: 'Loaded from finance rates and locked from manual edits',
    exchangeRateBase: 'Base Currency: {{base}}',
    exchangeRatePair: 'Current quote: 1 {{currency}} = {{rate}} {{base}}',
    exchangeRateBaseLocked: '{{currency}} is the base currency, so the rate is fixed at 1.0000',
    status: 'Status',
    expectedArrival: 'Expected Arrival',
    paymentMethod: 'Payment Method',
    paymentMethodPlaceholder: 'Select payment method',
    paymentTerm: 'Payment Term',
    paymentTermPlaceholder: 'Select payment term',
    purchaser: 'Purchaser',
    note: 'Note',
    notePlaceholder: 'Enter contract notes, delivery requirements, or inspection standards...',
  },
  linesEditor: {
    title: 'Purchase Material Lines',
    addLine: 'Add Line',
    materialName: 'Material',
    materialPlaceholder: 'Select material archive...',
    materialSearchPlaceholder: 'Search by name, spec, or code...',
    materialEmpty: 'No matching materials found',
    codeSpec: 'Code / Spec',
    codeSpecPlaceholder: 'Material code or spec',
    qty: 'Qty',
    unit: 'Unit',
    unitPlaceholder: 'Unit',
    unitSearchPlaceholder: 'Search unit name, code, or category...',
    unitEmpty: 'No matching units found',
    price: 'Price',
    subtotal: 'Subtotal',
  },
} as const
