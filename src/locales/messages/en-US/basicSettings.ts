export const basicSettings = {
  ...
  securityPage: {
    ...
    authCardDescription:
      'Controls the global verification password for topology renaming, process adjustment, and node overflow.',
  ...
  appearanceMapping: {
    title: 'Configure Appearance Code Mapping',
    description:
      'Define the physical appearance represented by digits 1-9 in the barcode. These rules act as a global master-data standard.',
    fields: {
  ...
  linearBarcode: {
    page: {
      ...
      subtitle:
        'LINEAR_BARCODE_PROTOCOL / Maintain the persisted Code128 wheel-barcode protocol for date, model, appearance, hole code, and 4-digit sequence.',
      badges: {
    ...
    table: {
      ...
      segments: {
        year: { name: 'Year (YY)', desc: 'Last two digits of the production year, fixed 2-digit encoding.' },
        month: { name: 'Month (M)', desc: '1-9 for Jan-Sep, 0 for Oct, N for Nov, and D for Dec.' },
        day: { name: 'Day (DD)', desc: 'Calendar day, fixed range 01-31.' },
        model: { name: 'Model', desc: '2-digit model code aligned with the engineering product archive.' },
        appearance: { name: 'Appearance', desc: 'Single-digit appearance mapping that reuses the shared appearance dictionary.' },
        holes: { name: 'Hole Code', desc: 'Uses R/D plus a 2-digit hole count, for example R14 or D18.' },
        serial: { name: 'Serial Number', desc: '4-digit serial issued by the shared business numbering rule.' },
      },
    },
    simulation: {
      title: 'Dynamic Linear Barcode Simulation',
      subtitle: 'Year + Month + Day + Model + Appearance + Hole Code + Sequence',
      codeLabel: 'CODE',
      form: {
        year: 'YEAR (YY)',
        month: 'MONTH (M)',
    ...
    footer: {
      ...
      description:
        'This page defines the standard 15-character wheel barcode structure: year + month + day + model + appearance + hole code + serial. The serial reuses numbering rule {{key}} so PDA and scan-ingest clients share the same persisted defaults.',
    },
    resetDialog: {
  ...
  dmNumbering: {
    page: {
      title: 'DM Traceability Rule Center',
      subtitle: 'TRACEABILITY_PROTOCOL / Define and simulate high-resolution DataMatrix/QR atomic rules',
      badges: {
        mirrorActive: 'MIRROR_MODE_ACTIVE',
        payload: 'Current Payload: 14-Digit Core Sequence',
      },
      actions: {
        reset: 'RESET_PROTOCOL',
    ...
    table: {
      ...
      segments: {
        year: { name: 'Year (YY)', desc: 'Last two digits of the year, fixed 2-digit encoding.' },
        month: { name: 'Month (3-bit)', desc: '1-9 for Jan-Sep, 0, N, D for Oct, Nov, Dec.' },
        model: { name: 'Product Model', desc: '2-digit model code from the enterprise product dictionary.' },
        appearance: { name: 'Appearance', desc: 'Values 1-9 representing different finishes or textures.' },
        category: { name: 'Category', desc: 'Primary business attribute code from master data.' },
        holes: { name: 'Holes', desc: 'Number of physical holes, fixed 2-digit encoding.' },
        serial: { name: 'Serial Number', desc: '5-digit Base-36 compressed sequence per model.' },
      },
    },
    simulation: {
      title: 'Dynamic Business Simulation Output',
      placeholder: 'Parsing Engine Not Ready',
      subtitle: 'Human-readable production intent metadata',
      codeLabel: 'CIPHERTEXT',
      qrCodeOutput: '(QR CODE OUTPUT)',
      form: {
        year: 'YEAR (YY)',
        month: 'MONTH (3-BIT)',
        category: 'CATEGORY (7-BIT)',
        model: 'MODEL (4-5 CHAR)',
        appearance: 'APPEARANCE (1 CHAR)',
        holes: 'HOLE_COUNT (2 CHAR)',
        serial: 'SERIAL (5-CHAR BASE-36)',
        serialCount: 'COUNT: {{count}}',
        applySerial: 'APPLY',
        notIssued: 'NOT_ISSUED',
        specialPrefix: 'PREFIX: DRAIN_HOLE',
        enableHPrefix: 'ENABLE H PREFIX',
        suffixWheel: 'SUFFIX: WHEEL_ORIENTATION',
        wheelOptions: { F: 'FRONT (F)', R: 'REAR (R)', H: 'MIX (H)' },
        suffixScope: 'SUFFIX: ADAPTATION_SCOPE',
        scopePlaceholder: 'Enter scope code (e.g. AM/BD)...',
      },
      validator: {
        title: 'Validation Parser',
        titleSuffix: '(VALIDATOR)',
        waiting: 'Waiting for simulation data...',
        success: 'Local master data mapping verified',
      },
      scannerGuide: {
        title: 'Scanner Test Guide',
        text: 'Target: Use any QR-capable device (camera/app) to read the current payload.',
      },
      batchSN: 'PRODUCTION BATCH SN',
      verifiedStandard: 'VERIFIED STANDARD',
    },
    resetDialog: {
      title: 'WARP_RESET / RESET RULES',
      description: 'This action will erase all custom segment descriptions and restore the 14-DIGIT_DM_PATTERN.',
      verifyPrompt: 'Enter verification text to confirm:',
      verifyTarget: '重置DM号码',
      placeholder: 'VERIFY_TEXT_HERE...',
      discard: 'DISCARD_ACTION',
      commit: 'COMMIT_RESET_PROTOCOL',
    ...
    toasts: {
      resetSuccess: 'Rules reset to system defaults.',
