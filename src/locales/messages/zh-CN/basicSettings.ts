export const basicSettings = {
  ...
  appearanceMapping: {
    title: '配置外观编码映射',
    description: '定义 1-9 数字在条形码中所代表的物理外观形态。此规则将作为全局主数据标准。',
    fields: {
      label: '外观简称',
      description: '业务详细描述',
    },
  ...
  dmNumbering: {
    page: {
      title: '二维码追溯规则中心',
      subtitle: 'TRACEABILITY_PROTOCOL / 定义与仿真高分辨率 DataMatrix/QR 编码原子规则，构建物理到数字的映射链路',
      badges: {
        mirrorActive: '镜像模式已激活',
        payload: '当前负荷: 14位核心序列',
      },
      actions: {
        reset: '重置规则',
        publish: '保存并发布',
    ...
    table: {
      ...
      segments: {
        year: { name: '年份 (YY)', desc: '公元年份后两位，固定 2 位数字编码。' },
        month: { name: '月份 (3位)', desc: '1-9月用原数字，10-12月分别使用 0, N, D 标识。' },
        model: { name: '产品型号', desc: '对应企业内部产品数据字典中的 2 位型号代码。' },
        appearance: { name: '外观方案', desc: '数值 1-9 代表不同的涂装或编织纹理映射。' },
        category: { name: '二级分类', desc: '对应物料主数据中的一级业务属性代码。' },
        holes: { name: '孔数', desc: '轮圈物理孔位数量，固定 2 位数字编码。' },
        serial: { name: '流水号', desc: '单型号独立计数的 5 位 Base-36 压缩编码。' },
      },
    },
    simulation: {
      title: '业务模型动态仿真输出',
      placeholder: '解析引擎未就绪',
      subtitle: '人机可读生产意图元数据',
      codeLabel: '编码密文',
      qrCodeOutput: '(二维码输出)',
      form: {
        year: '年份 (YY)',
        ...
        appearance: '外观方案 (1位)',
        holes: '产品孔数 (2位)',
        serial: '5位流水号 (Base-36)',
        serialCount: '计数: {{count}}',
        applySerial: '申请',
        notIssued: '未发放',
        specialPrefix: '特殊前缀: 泄水孔结构',
        enableHPrefix: '启用 H 前缀',
        suffixWheel: '后缀标识: 轮向位置',
        wheelOptions: { F: '前 (F)', R: '后 (R)', H: '混 (H)' },
        suffixScope: '后缀标识: 适应范围',
        scopePlaceholder: '输入范围代码 (如 AM/BD)...',
      },
      validator: {
        title: '业务解析比对验证',
        titleSuffix: '(比对验证)',
        waiting: '等待仿真数据输入...',
        success: '已映射本地存档数据同步校验通过',
      },
      scannerGuide: {
        title: '扫码测试指南',
        text: '验证目标：使用任何能识别 QR 码的设备（如手机相机/微信），读取到当前编码即可。',
      },
      batchSN: '生产批次序列号',
    ...
    resetDialog: {
      title: '重置编码规则',
      description: '重置操作将立即抹除所有自定义段落描述，恢复至系统初始的 14-DIGIT_DM_PATTERN 范式。',
      verifyPrompt: '请输入验证标识以确认操作：',
      verifyTarget: '重置DM号码',
