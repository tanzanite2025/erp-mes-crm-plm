...
## `sales` 第一阶段 TDO 化可执行实施清单专项（2026-04-08，待确认）

### 一、当前现状
上一份 `sales-phase1-tdo-alignment-plan.md` 已经给出第一阶段的完整方案、边界与分层方向。

但在进入代码改造前，还需要一份更短、更明确、更偏执行顺序的审批稿，用于回答：

1. 第一步具体先改哪条链；
2. 第二步需要动哪些文件；
3. 每个阶段的风险点与验证口径是什么；
4. 哪些动作必须等用户批准后才能真正开始。

### 二、本轮目标
本轮只做一件事：

1. 从现有 `sales` 第一阶段方案文档中压缩出一份可执行实施清单；
2. 按 Phase 列出目标、涉及文件、预期结果、风险点与验证口径；
3. 使该清单成为进入代码改造前的直接审批稿。

### 三、本轮明确不做
1. 不修改任何业务代码；
2. 不重复展开长篇架构说明；
3. 不把该清单扩写成全域统一实施路线图；
4. 不在未批准前启动 `sales` 第一阶段代码改造。

### 四、建议文档落点
建议路径：

`docs/architecture/workflow/sales-phase1-tdo-execution-checklist.md`

选择该路径的原因：

1. 与 `current-domain-topology-map.md`、`sales-phase1-tdo-alignment-plan.md` 构成“现状 -> 方案 -> 执行清单”三件套；
2. 保持同一专题目录，便于审批与后续查阅；
3. 文件名直接体现其用途是实施清单，而不是再次泛化成方案文档。
...
2. 实施前不变量
3. Phase 1：建立事务入口协议
4. Phase 2：前端 `claim` 迁移到 transaction 链
5. Phase 3：副作用收口与最小审计落位
6. Phase 4：验证与回退口径
7. 待用户批准后执行的事项
...
3. 待创建：`docs/architecture/workflow/sales-phase1-tdo-execution-checklist.md`

### 七、风险与注意事项
1. 如果执行清单仍然过于抽象，将无法作为真正的实施审批稿；
2. 如果执行清单写得过细到代码 diff 级别，又会与正式实施阶段混淆；
3. 必须在清单末尾明确：未经用户批准，不开始业务代码改造。

### 八、确认点
进入代码改造前，需要你确认：

1. 是否接受建议路径 `docs/architecture/workflow/sales-phase1-tdo-execution-checklist.md`；
2. 是否接受该清单作为 `sales` 第一阶段正式实施顺序；
3. 你明确批准后，我才进入业务代码修改阶段。

## `sales` 第一阶段 TDO 化改造方案文档专项（2026-04-08，待确认）

### 一、当前现状
在上一份“当前各域拓扑图”文档中，已经明确 `sales` 域当前处于以下状态：

1. 前端已具备 `delta` 传输基础；
2. 销售单创建已接入 workflow instance；
3. `claim`、状态推进等关键动作仍主要走字段 patch 链路；
4. 部分业务副作用仍停留在前端 Hook / Service 中。

这意味着 `sales` 已具备成为第一批 TDO 化样板域的条件，但还缺少一份独立、可执行、贴合现有文件结构的第一阶段改造方案文档。

### 二、本轮目标
本轮只做一件事：

1. 新增一份独立 Markdown 方案文档；
2. 面向 `sales` 域输出第一阶段 TDO 化改造方案；
3. 明确第一阶段的最小目标、样板动作、拟改文件、职责收口方向、风险点与验证口径；
4. 作为后续真正进入代码改造前的专项实施基线。

### 三、本轮明确不做
1. 不在本轮修改任何业务代码；
2. 不将本轮扩写成全域统一 TDO 蓝图；
3. 不直接进入 `purchase / inventory` 的实施方案展开；
4. 不在文档中输出具体代码 patch 或命令执行步骤。

### 四、建议文档落点
建议路径：

`docs/architecture/workflow/sales-phase1-tdo-alignment-plan.md`

选择该路径的原因：

1. 与 `current-domain-topology-map.md` 形成“现状 -> 方案”配套关系；
2. 继续沿用 `workflow` 专题目录，避免方案文档散落；
3. 文件名直接体现 `sales`、`phase1`、`tdo` 三个关键信息，便于后续继续扩展第二阶段或其他域方案。
...
1. `task.md`
2. `implementation_plan.md`
3. 待创建：`docs/architecture/workflow/sales-phase1-tdo-alignment-plan.md`

### 七、风险与注意事项
1. 若本轮方案文档过早泛化到全域，会削弱 `sales` 样板域的可执行性；
2. 若方案文档直接写成代码级实现清单，会与后续正式审批稿和实施稿边界混淆；
3. 若命名不清晰，后续第二阶段或 `purchase` 方案文档将难以持续并排管理。

### 八、确认点
进入文档创建前，需要你确认：

1. 是否接受建议路径 `docs/architecture/workflow/sales-phase1-tdo-alignment-plan.md`；
2. 是否确认本轮文档主体聚焦第一阶段方案，而不是展开到全域；
3. 是否允许文末增加“后续阶段预留”章节；
4. 确认后我再正式创建该独立 Markdown 文件。

## 当前各域拓扑图文档沉淀专项（2026-04-08，待确认）

### 一、当前现状
基于已完成的代码级分析，当前仓库内与 `SDRTS + Workflow + TDO` 对齐最相关的四个域，已经可以整理出一张明确的“现状拓扑图”：
...
这份信息目前只存在于对话分析中，尚未沉淀为仓库内可复用、可引用、可持续更新的独立文档。

### 二、本轮目标
本轮只做一件事：

1. 新增一份独立 Markdown 文档；
2. 系统化沉淀四个域当前的：
   - 数据流
   - 副作用流
   - 工作流接点
3. 使其成为后续 `SDRTS + Workflow + TDO` 收敛方案的“当前现状基线文档”；
4. 在文末补充一节“后续收敛方向”，作为从现状拓扑通向下一步方案的过渡入口。

### 三、本轮明确不做
1. 不修改任何业务代码；
2. 不在本轮把该文档扩写成未来实施方案；
3. 不顺手新增第二份重复架构稿；
4. 不将文档内容散落追加到根目录多个零散 Markdown 文件中。

### 四、建议文档落点
结合现有仓库目录与“按专题目录化存放架构文档”的要求，建议采用如下路径：

`docs/architecture/workflow/current-domain-topology-map.md`

选择该路径的原因：

1. 该文档本质属于架构现状图谱，而不是执行总结；
2. 文档内容聚焦工作流接点与事务链路，归入 `workflow` 子目录更利于后续扩展；
3. 可避免继续在仓库根目录堆叠新的分析稿。
...
1. `task.md`
2. `implementation_plan.md`
3. 待创建：`docs/architecture/workflow/current-domain-topology-map.md`

### 七、风险与注意事项
1. 若文档同时混入“现状图”和“未来改造方案”，会再次造成职责混杂；
2. 若文档放在根目录，会继续加剧分析类文件散落；
3. 若本轮未经确认直接创建文件，可能与用户希望的目录落点不一致。

### 八、确认点
进入文档创建前，需要你确认：

1. 路径已确认：`docs/architecture/workflow/current-domain-topology-map.md`；
2. 内容边界已确认：以“当前现状拓扑”为主体，文末允许补充“后续收敛方向”；
3. 下一步可按确认结果正式创建该独立 Markdown 文件。
