### 1. warehouse 第四阶段补充：最小评估 `EXECUTE_FAILED`
...
在“执行审计字段最小补齐”完成后，当前调账执行链已经具备以下可见性基础：

1. `approvedBy / approvedAt`
2. `executedBy / executedAt`
3. 执行成功后能明确看到执行人和执行时间

当前尚未具备的，是“执行失败”的正式持久化表达。

现在需要判断的关键问题不是“失败会不会发生”，而是：

1. 失败是否已经形成足够强的治理缺口
2. 是否值得为此引入一个最小状态 `EXECUTE_FAILED`
3. 如果引入，这个状态是否有明确、安全的落库时机

#### 1.2 本轮目标

本轮目标是形成清晰判断：

1. 当前是否应引入 `EXECUTE_FAILED`
2. 如果不引入，现有“错误返回 + 审计可见性”是否足够
3. 如果引入，最小联动范围应控制在什么程度

#### 1.3 建议评估标准

##### A. 失败是否具有“可追踪业务价值”

只有当执行失败不仅是技术错误，而且对业务用户后续操作有持续影响时，新增状态才有价值。

判断点包括：

1. 用户是否需要在稍后再次回到页面继续处理失败记录
2. 失败记录是否需要在列表中被筛选、标识或跟踪
3. 失败后是否存在明确的后续动作，例如重试或人工介入

如果这些需求并不强，单纯因为“失败存在”而新增状态，收益可能不高。

##### B. 失败落库时机是否可安全定义

`EXECUTE_FAILED` 只有在失败落库时机清晰时才适合引入。

需要明确：

1. 是在事务开始前预写状态，还是失败后回写状态
2. 如果执行逻辑运行在单个事务内，失败后事务回滚，失败状态如何持久化
3. 是否会因为记录失败状态而破坏当前事务语义

如果这一点不能清晰定义，贸然引入状态会引入更大复杂度。

##### C. 当前可见性补齐后，是否仍存在明显治理盲区

在已补齐 `executedBy / executedAt` 之后，需要再判断：

1. 当前用户是否仍然无法理解一次失败执行发生了什么
2. 是否仍无法区分“从未执行”与“尝试过但失败”
3. 是否已出现必须要在页面层保留失败痕迹的业务诉求

如果这些盲区依然明显，则 `EXECUTE_FAILED` 的必要性会上升。

#### 1.4 暂不引入 `EXECUTE_FAILED` 的条件

如果排查后发现以下特征成立，则更适合暂不引入：

1. 执行失败目前主要是罕见技术错误
2. 用户无需持续追踪失败记录，只需即时得知错误并重新尝试
3. 当前事务模型下，失败状态难以安全持久化
4. 页面层没有筛选、统计、追踪失败执行的实际需求

在这种情况下，继续使用“错误返回 + toast + 审计字段可见性”会更稳妥。

#### 1.5 建议引入 `EXECUTE_FAILED` 的触发条件

如果排查后发现以下任一信号较强，则可考虑最小引入：

1. 用户需要明确看到某笔调账“尝试执行过但失败”
2. 失败后需要人工介入，而不仅是立即重试
3. 调账记录列表需要区分“未执行”与“执行失败”
4. 后端已有可安全落失败状态的位置，不会破坏现有事务语义

#### 1.6 若引入，最小联动范围建议

如果后续判断应该引入 `EXECUTE_FAILED`，最小改动范围应控制为：

1. 后端模型状态枚举增加 `EXECUTE_FAILED`
2. API DTO / 前端 contract / adapter 同步增加该状态
3. 页面状态文案与 badge 展示同步支持
4. 失败持久化只记录最小必要信息，不同时引入重试/回滚状态

也就是说，只做“失败可见”，不一步走到完整执行状态机。

#### 1.7 当前倾向性判断

基于当前已知情况，我当前更倾向于：

1. **先审慎评估，不预设一定引入 `EXECUTE_FAILED`**
2. 只有在确认失败需要被持续追踪，且失败落库时机清晰时，才值得最小引入
3. 否则继续保持“执行失败即时反馈 + 审计可见性补齐”更稳妥

#### 1.8 本轮排查范围建议

若进入执行，本轮应重点排查：

1. 后端 `ExecuteAdjustment` 的失败点与事务边界
2. 当前失败是否全部在事务内回滚导致无持久化窗口
3. 页面层是否已经存在对失败记录持续展示的真实需求
4. 当前错误返回是否足以支撑用户恢复流程

#### 1.9 当前阶段结论

最小评估 `EXECUTE_FAILED` 的正确顺序，不是直接加状态，而是先确认“失败是否值得追踪”与“失败是否能安全落库”。如果这两点都成立，再进入最小引入；否则当前更适合保持现状，不扩张状态模型。

### 2. warehouse 第四阶段补充：执行审计字段最小补齐规划
...
### 4. warehouse 第四阶段补充：审批回流执行链边界规划（方向 B）
...
### 5. warehouse 第四阶段补充：判断 `adjustment` 轻量入口是否演进为正式独立边界（方向 A2）
...
### 6. warehouse 第四阶段补充：调整历史与执行边界规划（方向 A）
...
### 8. warehouse 第四阶段补充：inventory / stocktake 旧兼容层一致性清理规划
...
### 9. warehouse 第四阶段补充：stocktake 第二批消费方切换规划
...
### 10. warehouse 第四阶段：stocktake 子域拆分规划
...
### 11. warehouse 第三阶段补充：inventory 第一批兜底一致性清理规划
...
### 12. warehouse 第三阶段补充：inventory 第二批消费方切换规划
...
### 13. warehouse 第三阶段补充：inventory-maintenance-service 收口规划
...
### 14. warehouse 第三阶段：inventory 子域拆分规划
...
### 15. warehouse 第二阶段补充：warehouse-export-service 历史 lint 清理规划
...
### 16. warehouse 第二阶段补充：shipment 间接依赖清理规划
