...
### 1. plan：`/personal-workbench/capture` 标题/描述收口为新建拍照/录像页
...
当前 `/personal-workbench/capture` 的交互职责已经基本收口为“独立新建采集页”，但顶部标题和说明文案仍残留“个人快捷采集 / 一键拍照 / 一键录视频 / 独立新建采集入口”等混合表述，尚未完全匹配“新建拍照页 / 新建录像页”的产品心智。

#### 1.2 当前排查结论

当前实查结果如下：

1. 页面顶部主标题当前为“一键拍照 / 一键录视频”
2. 页面说明标题当前为“独立新建采集入口”
3. 当前页面实际上已经不再读取历史草稿队列，因此更准确的表述应直接围绕“新建记录”展开

#### 1.3 本轮目标

1. 让 `/personal-workbench/capture` 的页面文案明确体现“独立新建”
2. 进一步消除与“个人缓冲区”之间的心智混淆
3. 保持当前采集与编辑链路不变，只做页面标题/描述层的收口

#### 1.4 推荐实施方向

1. 根据 `mode` 为 `photo | video` 分别输出更明确的新建页标题
2. 将描述文案从“快捷采集 / 采集入口”收口为“新建拍照记录 / 新建录像记录”
3. 保持其它交互逻辑、桥接逻辑与编辑逻辑不变
...
1. 不要顺手改动 capture 页业务逻辑
2. 不要把这次小范围文案收口扩散到 quick-action drawer 或 personal-workbench 主页
...
1. `mode=photo` 时页面标题/描述明确体现“新建拍照记录”
2. `mode=video` 时页面标题/描述明确体现“新建录像记录”
3. `pnpm exec tsc --noEmit` 通过
4. 定向 eslint 通过

#### 1.9 结论

这项问题已经不是交互链路问题，而是页面文案尚未完全跟上最新职责边界。下一步应只对 `/personal-workbench/capture` 做最小范围的标题/描述收口，让用户一进入就明确感知这是“新建拍照页 / 新建录像页”，而不是任何形式的缓冲区工作台。

### 1. plan：侧边栏方案A——请假管理 / 荣誉榜并回组织人事分组
...
当前侧边栏中，`组织人事` 菜单项位于“组织人事”分组下，而 `请假管理` 与 `荣誉榜` 被放在“服务中心”分组下。但三者的路由都归属于 `/personnel*` 路径树，信息架构与路由归属出现了不一致。

#### 1.2 当前排查结论

当前实查结果如下：

1. `src/components/layout/data/sidebar-data.ts` 将 `/personnel` 配置在 `orgPersonnel` 分组，将 `/personnel/leave` 和 `/personnel/stats` 配置在 `serviceCenter` 分组
2. `src/routes/_authenticated/personnel/*` 证明 `请假管理` 与 `荣誉榜` 实际上都属于组织人事域
3. `src/components/layout/nav-group.tsx` 的 `checkIsActive(...)` 对 `/personnel` 使用前缀命中，因此访问 `/personnel/stats` 时，`组织人事` 与 `荣誉榜` 会同时显示 active
4. 若采用方案A，只需收口侧边栏分组配置，即可让信息架构与路由归属更一致

#### 1.3 本轮目标

1. 将 `请假管理 / 荣誉榜` 从“服务中心”并回“组织人事”分组
2. 让侧边栏分组与 `/personnel*` 路由归属保持一致
3. 以最小修改降低当前双高亮带来的认知违和感

#### 1.4 推荐实施方向

1. 修改 `src/components/layout/data/sidebar-data.ts`
2. 在 `orgPersonnel` 分组下增加：
   - `/personnel/leave`
   - `/personnel/stats`
3. 从 `serviceCenter` 分组移除上述两项
4. 本轮不重写 `checkIsActive(...)`，先通过信息架构收口解决当前主要违和感
...
1. 不要顺手改动路由文件
2. 不要把本轮扩散成“服务中心”整组删除或重命名，除非后续另行规划
3. 需要确认是否仍保留空的 `serviceCenter` 分组，还是在本轮一并移除该空分组

#### 1.7 非目标边界

本轮不做：

1. 不修改 `checkIsActive(...)` 的底层命中算法
2. 不调整 `组织人事` 模块内部 tabs
3. 不迁移 `/personnel/*` 路由
...
1. 侧边栏中 `请假管理 / 荣誉榜` 出现在“组织人事”分组下
2. “服务中心” 若无其它菜单项，不应残留空分组
3. `pnpm exec tsc --noEmit` 通过
4. 定向 eslint 通过

#### 1.9 结论

方案A的核心不是改路由，而是修正侧边栏信息架构，使菜单分组与实际路由归属重新一致。这样可以在最小变更下消除“人事域页面却被服务中心承载”的违和感，并降低当前双高亮问题带来的认知噪音。

### 1. plan：侧边栏分类卡片 / 文字层级色收口
...
当前侧边栏在全部分类展开后，分组标题与菜单项文字层级过于接近，不利于快速区分“分类卡片”与“具体菜单项”。用户希望分类本身拥有独立层级色，但必须继续对齐当前明暗主题系统，而不是引入硬编码颜色破坏 sidebar 主题变量体系。
